...
6. **Google DeepMind 发布 Gemini Robotics 2 物理 AI** — X：Google DeepMind (@GoogleDeepMind)（7/30 23:02）
...
7. **Gemini Robotics ER 2：用视频理解、任务编排与多机器人协作赋能机器人** — Google DeepMind：Blog（RSS）（7/30 23:00）
...
9. **Google DeepMind 在 Flow Music 中推出 Lyria 3.5，提升音乐性、歌词、人声与创作控制** — Google DeepMind：Blog（RSS）（7/30 00:02）
...
10. **OpenAI 推出两款新转录模型 API** — X：OpenAI Developers (@OpenAIDevs)（7/29 04:26）
...
11. **Microsoft 发布 MAI-Cyber-1-Flash：5B 活跃参数的网络安全模型，驱动 MDASH 在 CyberGym 上达到 95.95%** — MarkTechPost（RSS）（7/28 16:33）
...
12. **FeyNoBg 发布：开源自动背景去除模型，在四项基准上达到 SOTA** — Hacker News 热门（buzzing.cc 中文翻译）（7/28 12:57）
...
13. **Kimi K3 开放日：模型权重、技术报告和关键 Infra 技术同步开放** — 公众号：月之暗面（Kimi）（7/27 23:34）
...
14. **Kimi K3 开源：2.8T MoE 模型与技术报告** — X：Kimi.ai (@Kimi_Moonshot)（7/27 23:14）
...
## 产品发布
15. **Replit Design 推出数百设计模板** — X：Replit (@Replit)（7/31 08:00）
   再也不用从空白页开始了。
...
17. **Google Earth 集成 Nano Banana 2 图像生成** — X：Google AI (@GoogleAI)（7/31 02:53）
...
18. **OpenRouter 下调 GPT-5.6 Terra/Luna 价格** — X：OpenRouter (@OpenRouter)（7/31 01:32）
...
19. **GitHub Copilot 应用新增堆叠会话与拉取请求功能** — GitHub Blog（7/31 01:30）
...
21. **llm-chat-completions-server 0.1a0 发布** — Simon Willison 博客（7/30 23:43）
...
22. **Token Saver：用本地混合 RAG 将 Claude PDF token 消耗削减 92%-99% 的开源 MCP 扩展** — MarkTechPost（RSS）（7/30 15:43）
...
29. **在 M1 Max 上运行 2.8T 参数的 Kimi K3：Deltafin 项目实现 0.0687 token/s 推理** — Hacker News 热门（buzzing.cc 中文翻译）（7/29 10:49）
...
30. **OpenAI 发布 Codex 安全 CLI 与 SDK** — X：Tibo (@thsottiaux)（7/29 07:05）
...
31. **Gemini API Managed Agents 默认升级为 3.6 Flash，新增环境钩子与免费套餐** — Google Blog：AI（RSS）（7/29 00:00）
...
32. **Perplexity 推出 Windows 版个人电脑智能体** — X：Perplexity (@perplexity_ai)（7/28 22:00）
...
33. **火山引擎上线豆包搜索服务，为AI Agent提供实时可信搜索能力** — 公众号：火山引擎（7/28 15:51）
...
34. **Kimi 发布视觉感知基准 PerceptionBench** — X：Kimi.ai (@Kimi_Moonshot)（7/28 02:45）
...
35. **Kimi K3 开源分布式智能体环境 AgentENV** — X：Kimi.ai (@Kimi_Moonshot)（7/27 23:25）
...
## 行业动态
36. **欧盟《人工智能法》新增透明度要求，8 月 2 日起正式执行** — IT之家（RSS）（7/31 19:40）
...
37. **Anthropic 承认三款 Claude 模型逃出测试环境攻击真实系统** — The Decoder：AI News（RSS）（7/31 18:57）
...
38. **国家发改委：人工智能相关行业保持 30% 以上高增长，全国智能算力规模达去年同期 2.8 倍** — IT之家（RSS）（7/31 14:38）
...
39. **国家发改委：将加快《人工智能法》立法进程** — IT之家（RSS）（7/31 10:48）
...
40. **Anthropic 披露 Claude 在安全评估中入侵真实系统** — X：Anthropic (@AnthropicAI)（7/31 07:02）
...
41. **法官称特朗普政府仍缺乏证据将Anthropic列为供应链风险** — TechCrunch：AI（RSS）（7/31 04:26）
...
42. **RadixArk 与 Google Cloud 合作，将完整 SGLang 功能引入 TPU** — LMSYS：Blog（Chatbot Arena 团队）（7/30 23:50）
...
43. **FCC 禁止进口中国新型机器人与联网逆变器** — The Decoder：AI News（RSS）（7/30 20:47）
...
44. **Claude Opus 5 在模拟售货机任务中展现欺骗与背叛，创下新纪录** — TechCrunch：AI（RSS）（7/30 02:45）
...
45. **SpaceXAI 起诉明尼苏达州，反对"AI 脱衣"应用禁令** — IT之家（RSS）（7/29 20:12）
...
46. **OpenAI 失控 AI 智能体不止攻击了 Hugging Face，还入侵了多家公司** — The Verge：AI（RSS）（7/29 19:54）
...
47. **1100多名AI员工联名呼吁美国政府控制AI发展速度，OpenAI CEO奥尔特曼表态支持** — IT之家（RSS）（7/29 08:20）
...
48. **Anthropic 支持 AI 发展节奏请愿** — X：Anthropic (@AnthropicAI)（7/29 06:17）
...
49. **OpenAI 失控模型二次入侵 Modal 客户** — X：AI Safety Memes (@AISafetyMemes)（7/29 05:55）
...
50. **Hugging Face 公开自主智能体网络攻击详情** — X：Clément Delangue（Hugging Face CEO） (@ClementDelangue)（7/29 04:27）
...
51. **Andrew Ng 创办 LearnVector，用 AI 实现一对一学习** — X：Andrew Ng（DeepLearning.AI 创始人） (@AndrewYNg)（7/29 04:19）
...
52. **德里高等法院裁定 OpenAI 利用 ANI 内容训练 AI 未侵犯版权** — IT之家（RSS）（7/28 14:21）
...
53. **Google AI Overviews 搜索结果出现率升至43%** — TechCrunch：AI（RSS）（7/27 23:57）
...
## 论文研究
54. **面壁智能ALIGN：自动对齐智能体与环境接口** — X：面壁智能 OpenBMB (@OpenBMB)（7/31 21:00）
...
55. **Show HN：将 DeepSeek 整合到 GPT-OSS 中不会带来审查机制** — Hacker News 热门（buzzing.cc 中文翻译）（7/31 12:57）
...
56. **腾讯混元Hyra破解50年数学难题** — X：腾讯混元 (@TencentHunyuan)（7/30 10:33）
...
57. **大语言模型的显著性偏差：常识推理中的知识压制而非缺失** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
58. **BM25 在大规模语料中胜出：检索增强生成范式的规模扩展研究** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
59. **PhiZero：围绕"物理语言"构建的世界模型** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
60. **Science One Framework：通过 Chain-of-Evidence 实现可验证的自主科研框架** — Google Research：Blog（网页）（7/30 00:00）
...
61. **AI 智能体能否进行开放式 AI 研究？两项案例的早期证据** — HuggingFace Daily Papers（社区热门论文）（7/29 08:00）
...
62. **Claude 发现加密算法弱点研究发布** — X：Anthropic (@AnthropicAI)（7/29 01:16）
...
63. **Miles 在 Blackwell 架构上实现端到端 MXFP8 与逐 token NVFP4 强化学习方案** — LMSYS：Blog（Chatbot Arena 团队）（7/29 00:00）
...
64. **Kimi Linear：一种表现力强且高效的注意力架构** — Hacker News 热门（buzzing.cc 中文翻译）（7/28 23:21）
...
65. **GPT-Red：通过大规模自对弈实现自动化红队测试** — HuggingFace Daily Papers（社区热门论文）（7/28 08:00）
...
## 技巧与观点
66. **教程：用 Antigravity SDK 与 Google Cloud 构建自主财务审计智能体团队** — Google AI：DEV 作者专属（RSS）（7/31 19:07）
...
67. **OpenAI 总裁布罗克曼承认新版 ChatGPT 桌面应用"有点乱"，目标年底实现"零标签"** — IT之家（RSS）（7/30 10:49）
...
68. **揭秘 AI 智能体入侵 Hugging Face 全过程：4 天半执行 17600 次操作** — IT之家（RSS）（7/30 07:29）
...
69. **算力价格未来可能上涨 10 倍以上** — Dwarkesh Patel：Podcast & Blog（RSS）（7/29 23:01）
...
70. **启用两项 API 设置使 GPT-5.6 在 ARC-AGI-3 基准测试得分提升三倍** — OpenAI：官网动态（RSS · 排除企业/客户案例）（7/29 23:00）
...
71. **我的Claude账号被封了** — 公众号：数字生命卡兹克（7/29 08:06）
...
72. **微软转售前沿：Azure 年营收破千亿但增速被 Google Cloud 反超** — Tomer Tunguz 博客（VC 分析）（7/29 08:00）
...
73. **GPT-5.6 如何融合前沿智能与效率** — OpenAI：官网动态（RSS · 排除企业/客户案例）（7/29 08:00）
...
74. **OpenRouter 推出专用 LangChain 集成包，支持 400+ 模型与自动故障切换** — OpenRouter：Announcements（RSS）（7/29 08:00）
...
75. **OpenAI 呼吁为前沿AI发展设定节奏** — X：OpenAI (@OpenAI)（7/29 04:56）
...
76. **Sam Altman 态度转变：AI 发展或需"减速"以让社会做好准备** — TechCrunch：AI（RSS）（7/29 04:17）
...
77. **Google Search 的 AI Mode 推出 5 项新功能，帮你规划线下生活** — Google Blog：AI（RSS）（7/28 21:00）
...
78. **如何评估不同 LLM 提供商在延迟、吞吐量和正常运行时间上的性能** — OpenRouter：Announcements（RSS）（7/28 08:00）
...
79. **OpenAI 称越来越多员工用 ChatGPT 做其他岗位的工作** — The Decoder：AI News（RSS）（7/28 03:08）
...
80. **用Claude和Python构建技能驱动的金融分析智能体** — MarkTechPost（RSS）（7/28 02:08）
...
81. **GitHub Copilot 发布"Harness"工作流：用单一工具完成原型、规划、实现与代码审查** — GitHub Blog（7/28 02:00）
...
82. **GitHub Copilot app 入门指南：多 Agent 会话工作区与 Canvas 预览** — GitHub Blog（7/28 00:00）
