...
3. **DeepSeek-V4-Flash 正式版 API 上线公测** — DeepSeek：API 更新日志（7/31 00:00）
...
4. **Google DeepMind 发布 Gemini Robotics 2 物理 AI** — X：Google DeepMind (@GoogleDeepMind)（7/30 23:02）
...
6. **DeepSeek V4 Flash 正式版发布：后训练重做，九项 Agent 测试全面超越 V4 Pro Preview** — 公众号：数字生命卡兹克（7/31 14:41）
...
7. **GPT-5.6 如何推进性价比前沿** — OpenAI：官网动态（RSS · 排除企业/客户案例）（7/30 18:00）
   OpenAI 为 GPT-5.6 的 Luna 和 Terra 版本推出更低定价，以更高效的模型帮助企业大规模部署 AI 工作流。
   https://openai.com/index/advancing-the-price-performance-frontier-with-gpt-5-6

8. **Inkling-Small 发布，276B 参数性能持平原版** — X：Thinking Machines (@thinkymachines)（7/31 01:47）
...
9. **DeepSeek-V4-Flash API公测上线，Agent能力大幅升级** — X：DeepSeek (@deepseek_ai)（7/31 14:56）
...
## 产品发布
11. **OpenRouter 推出 Ori Eval：用你的数据证明哪款模型最适合你的项目** — OpenRouter：Announcements（RSS）（7/31 08:00）
...
12. **Token Saver：用本地混合 RAG 将 Claude PDF token 消耗削减 92%-99% 的开源 MCP 扩展** — MarkTechPost（RSS）（7/30 15:43）
...
13. **llm-chat-completions-server 0.1a0 发布** — Simon Willison 博客（7/30 23:43）
...
14. **Gemini Spark 集成 Chrome 自动浏览功能** — X：Gemini (@GeminiApp)（7/31 04:15）
...
15. **OpenRouter 下调 GPT-5.6 Terra/Luna 价格** — X：OpenRouter (@OpenRouter)（7/31 01:32）
...
17. **WorkBuddy 深度打通腾讯文档，打造 Agent 时代的第三代办公协同** — 公众号：数字生命卡兹克（7/30 12:30）
...
18. **Perplexity Computer 推出 Projects 功能** — X：Aravind Srinivas（Perplexity CEO） (@AravSrinivas)（7/31 00:54）
...
19. **百度搭子一镜Skill：一句话生成数字人口播视频** — 公众号：百度智能云（文心）（7/31 21:42）
...
20. **Replit Design 推出数百设计模板** — X：Replit (@Replit)（7/31 08:00）
   再也不用从空白页开始了。
...
## 行业动态
22. **国家发改委：将加快《人工智能法》立法进程** — IT之家（RSS）（7/31 10:48）
...
23. **Anthropic 承认三款 Claude 模型逃出测试环境攻击真实系统** — The Decoder：AI News（RSS）（7/31 18:57）
...
24. **国家发改委：人工智能相关行业保持 30% 以上高增长，全国智能算力规模达去年同期 2.8 倍** — IT之家（RSS）（7/31 14:38）
...
25. **法官称特朗普政府仍缺乏证据将Anthropic列为供应链风险** — TechCrunch：AI（RSS）（7/31 04:26）
...
26. **欧盟《人工智能法》新增透明度要求，8 月 2 日起正式执行** — IT之家（RSS）（7/31 19:40）
...
27. **FCC 禁止进口中国新型机器人与联网逆变器** — The Decoder：AI News（RSS）（7/30 20:47）
...
28. **Anthropic 披露 Claude 在安全评估中入侵真实系统** — X：Anthropic (@AnthropicAI)（7/31 07:02）
...
29. **RadixArk 与 Google Cloud 合作，将完整 SGLang 功能引入 TPU** — LMSYS：Blog（Chatbot Arena 团队）（7/30 23:50）
...
## 论文研究
30. **腾讯混元 Hyra 攻克加法组合学 50 年未解难题** — 公众号：腾讯混元（7/31 12:10）
...
31. **Zero-Mem：为 LLM 智能体实现零 token 记忆操作** — HuggingFace Daily Papers（社区热门论文）（7/31 08:00）
...
32. **DiffusionGemma 技术报告：基于离散扩散的高效文本生成模型** — HuggingFace Daily Papers（社区热门论文）（7/31 08:00）
   DiffusionGemma 是一个实验性开源权重语言模型，通过离散扩散并行迭代精炼 256 个 token 的块，而非逐 token 解码，从而避免自回归模型的顺序解码瓶颈。
   https://arxiv.org/abs/2608.00146

33. **小红书 dots 团队发布 VibeLifeBench 与 VibeSearchBench：七个最强模型无一及格** — 公众号：小红书技术（dots.llm）（7/31 18:00）
...
34. **Show HN：将 DeepSeek 整合到 GPT-OSS 中不会带来审查机制** — Hacker News 热门（buzzing.cc 中文翻译）（7/31 12:57）
...
35. **面壁智能ALIGN：自动对齐智能体与环境接口** — X：面壁智能 OpenBMB (@OpenBMB)（7/31 21:00）
...
36. **大语言模型的显著性偏差：常识推理中的知识压制而非缺失** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
37. **腾讯混元Hyra破解50年数学难题** — X：腾讯混元 (@TencentHunyuan)（7/30 10:33）
...
38. **PhiZero：围绕"物理语言"构建的世界模型** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
39. **删除回避：LLM 代码编辑中的系统性缺陷与缓解之道** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
40. **BM25 在大规模语料中胜出：检索增强生成范式的规模扩展研究** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
## 技巧与观点
41. **实测 ego lite：给 Codex 浏览器自动化加到 2.5 倍速** — 公众号：卡尔的AI沃茨（7/30 18:04）
...
42. **OpenAI 总裁布罗克曼承认新版 ChatGPT 桌面应用"有点乱"，目标年底实现"零标签"** — IT之家（RSS）（7/30 10:49）
...
43. **教程：用 Antigravity SDK 与 Google Cloud 构建自主财务审计智能体团队** — Google AI：DEV 作者专属（RSS）（7/31 19:07）
