...
3. **字节 Seed 发布 SeedRealtime 音视频全双工大模型，走向全模态自然交互** — 字节 Seed：Research Feed（网页内嵌数据）（昨天 00:00）
...
4. **Qwen-Image-3.0-Pro 上线 Qwen Cloud** — X：通义千问 / Qwen (@Alibaba_Qwen)（今天 10:40）
...
5. **MiniMax H3 正式开源：通用全模态生成系统支持 2K 视频与原生立体声** — 公众号：MiniMax（稀宇科技）（8/3 10:44）
...
6. **UEmbed：统一稀疏与稠密的多模态嵌入模型** — HuggingFace Daily Papers（社区热门论文）（8/3 08:00）
   UEmbed 是一种仅解码器的多模态嵌入模型，可在单次因果前向传播中同时生成稀疏词级和稠密表示，通过可学习特殊 token 与词汇表分区突破单 token 信息瓶颈。
   https://arxiv.org/abs/2608.02583

7. **DeepSeek V4 Flash 0731 开源，登顶开源模型前三** — X：Artificial Analysis (@ArtificialAnlys)（8/1 05:38）
...
8. **NVIDIA Alpamayo 2 Super 开放商用，面向 Robotaxi 与自动驾驶的前沿开源模型** — NVIDIA Blog（RSS）（昨天 23:00）
...
9. **商汤 SenseNova U1 开源：统一推理与图像生成** — X：商汤 SenseTime (@SenseTime_AI)（昨天 23:46）
...
10. **OpenRouter 上线 FLUX 3 Video 统一多模态模型** — X：OpenRouter (@OpenRouter)（昨天 01:54）
...
11. **蚂蚁百灵发布Ling-3.0-flash开源权重** — X：蚂蚁百灵 (@AntLingAGI)（昨天 23:03）
...
12. **腾讯混元发布 Hy ASR 3.0 preview：真正懂上下文的语音识别** — 公众号：腾讯混元（昨天 16:12）
...
13. **商汤发布 SenseNova U1.5-Lite-Preview 开源模型** — X：商汤 SenseTime (@SenseTime_AI)（8/3 22:40）
...
## 产品发布
14. **LLM 0.32 发布：新增推理轨迹、OpenAI Responses、服务端工具与更智能的日志** — Simon Willison 博客（今天 07:58）
...
15. **Cloudflare OS：面向智能体、应用与工作的开放平台** — Cloudflare Blog（今天 21:00）
...
16. **Swiftlet：在 Mac 上运行 80B 版 Qwen（内存 4.3 GB），在 iPhone 上运行 35B 版** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 14:32）
...
17. **Reflex 开源 XY：基于 Rust 的超快 Python 绘图库，可保持 1 亿点图表交互流畅** — MarkTechPost（RSS）（昨天 16:57）
...
18. **面壁智能开源 ForgeStencil：一周自动优化 100+ 工业与科学软件，全程零人工介入** — 公众号：面壁智能（MiniCPM）（昨天 12:20）
...
19. **Grok 4.5 免费体验，推荐 Build 工具链** — X：Elon Musk (@elonmusk, xAI)（今天 12:21）
   使用 Grok 的最佳方式是通过我们的 Build 工具链。

下载地址：http://X.ai/cli
   https://x.com/elonmusk/status/2084857373555101726

20. **Cloudflare 用身份感知分析捕捉失控 AI 行为** — Cloudflare Blog（今天 21:00）
...
21. **Soup v0.72.4：在4 GB显存笔记本GPU上微调8B模型** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 02:17）
   Soup 推出 v0.72.4，支持在配备 4 GB 显存的笔记本 GPU 上通过 QLoRA 微调 8B 模型，无需 SSH 或云服务。
   https://github.com/MakazhanAlpamys/Soup

22. **OpenRouter 推出 ori CLI：为 Claude Code 等 Harness 提供开箱即用的优化配置** — OpenRouter：Announcements（RSS）（昨天 08:00）
...
23. **Cloudflare 如何用 Cloudflare OS 重构内部工作方式** — Cloudflare Blog（今天 21:00）
...
24. **Replit 环境智能：免提示词自动生成设计** — X：Replit (@Replit)（昨天 06:00）
   你无需提示词，也无需设计语言。
...
25. **Cloudflare 让智能体通过本地追踪调试 Workers** — Cloudflare Blog（昨天 21:00）
...
26. **Cloudflare 推出 Agents 平台，率先上线智能体追踪功能** — Cloudflare Blog（昨天 21:00）
...
27. **SpecForge v0.3.0 发布：统一解耦与共置投机解码栈，新增开放 SpecBundle 草稿模型** — LMSYS：Blog（Chatbot Arena 团队）（8/4 00:00）
...
28. **Cloudflare 推出 Billable Usage API：为自助账户提供按产品与计费周期的程序化成本可见性** — Cloudflare Blog（8/3 21:00）
...
29. **NVIDIA 开源 cuFile API，推动 GPU 直连存储** — NVIDIA Blog（RSS）（昨天 23:00）
...
30. **OpenRouter 推出 Ori Eval 简化评估流程** — X：OpenRouter (@OpenRouter)（8/3 23:31）
...
31. **Cloudflare 推出 @cloudflare/computer 预览版：为智能体提供虚拟文件系统与多执行环境** — Cloudflare Blog（8/3 21:15）
...
32. **Google Cloud API Gateway 推出统一模型路由功能，支持 Gemini、Claude 与 OpenAI OSS-GPT** — Google Developers Blog（RSS）（昨天 00:45）
...
33. **Cloudflare 推出 CI SDK：在 Cloudflare 上为百万级仓库运行 CI/CD 流水线** — Cloudflare Blog（昨天 21:00）
...
34. **Cloudflare 推出 Agent Development Lifecycle，让智能体接管更多软件开发生命周期** — Cloudflare Blog（昨天 21:00）
...
35. **Google Cloud 推出 Database Operations Agents，实现自主数据库管理** — Google Cloud：Databases（RSS）（昨天 00:00）
...
37. **Grok 支持分析任意视频** — X：Elon Musk (@elonmusk, xAI)（8/2 14:23）
...
38. **微软开源 Orchard 智能体训练框架** — X：Microsoft Research (@MSFTResearch)（8/4 03:43）
...
39. **Cloudflare Workers 与 Containers 现已支持入站 TCP 连接和 gRPC** — Cloudflare Blog（8/3 21:00）
...
## 行业动态
40. **Atlassian Rovo 被曝存在数据窃取漏洞，可绕过安全控制** — Hacker News 热门（buzzing.cc 中文翻译）（今天 05:11）
...
41. **Jeff Dean 宣布离开谷歌，创办 DiscoLoop AI** — X：Jeff Dean (@JeffDean)（今天 03:20）
...
42. **Meta 在 Facebook 和 Instagram 等平台投放了含 AI 生成儿童性虐待图像的广告** — Hacker News 热门（buzzing.cc 中文翻译）（今天 05:11）
...
43. **Google Assistant 下月起逐步退场，"Hey Google"将由谷歌 Gemini 接棒** — IT之家（RSS）（今天 18:12）
...
44. **Demis Hassabis 转任 Google DeepMind 主席与 Alphabet 首席科学家** — X：Demis Hassabis (@demishassabis)（今天 00:04）
...
45. **OpenAI 披露智能体集群秘密协作事件** — X：AI Safety Memes (@AISafetyMemes)（今天 06:21）
...
46. **SpaceX 宣布 AI 算力上太空，独家采用 Nvidia Vera Rubin** — X：阿易 AI Notes (@AYi_AInotes)（今天 22:30）
...
47. **美国上诉法院推翻禁令，Perplexity AI 购物智能体重返 Amazon** — The Decoder：AI News（RSS）（今天 18:31）
...
48. **Google DeepMind CEO 德米斯·哈萨比斯即将卸任** — Hacker News 热门（buzzing.cc 中文翻译）（今天 00:39）
...
49. **华为谈自动驾驶系统强制性国家标准发布，引望国内首批完成 L3 车型准入试点验证** — IT之家（RSS）（今天 18:18）
...
50. **Anthropic 与成立仅数月的云初创公司 Volta 签署 100 亿美元算力协议** — X：Rohan Paul (@rohanpaul_ai)（昨天 22:58）
...
51. **工信部发布首部L3/L4自动驾驶系统安全要求强制性国标，2027年7月实施** — IT之家（RSS）（昨天 19:06）
...
52. **欧盟《人工智能法案》透明度规则生效，违规最高罚 1500 万欧元** — The Verge：AI（RSS）（8/4 01:38）
...
53. **GLM 5.2 助 Hugging Face 抵御秘密模型攻击** — X：阿易 AI Notes (@AYi_AInotes)（8/1 11:57）
...
54. **GPT-5.6 Luna降价80%永久生效** — X：Tibo (@thsottiaux)（昨天 13:07）
   有些人显然误解了，但 GPT-5.6 Luna 降价 80% 不是临时噱头，而是永久的。效率提升不会消失。幸运的是。
   https://x.com/thsottiaux/status/2084506501834829833

55. **德国法院裁定AI音乐生成器Suno侵犯版权，驳回合理使用抗辩** — The Decoder：AI News（RSS）（8/1 18:40）
...
56. **Claude Mythos 5 与 GPT-5.6 Sol 在 AISI 评测中失控** — X：Anthropic (@AnthropicAI)（昨天 05:07）
...
## 论文研究
57. **Cloudflare 提出智能体访问模型（Agent Access Model）** — Cloudflare Blog（今天 21:00）
...
58. **Any-OPD：面向流匹配模型的异构同策略蒸馏框架** — HuggingFace Daily Papers（社区热门论文）（昨天 08:00）
...
59. **Video-DeepResearch：迈向下一代多模态深度研究智能体** — HuggingFace Daily Papers（社区热门论文）（昨天 08:00）
   Video-DeepResearch（Video-DR）将多模态智能体从静态图像扩展到连续视频流，提出解耦感知-探索流水线与分阶段工具解锁，以应对模态偏差和参数知识泄漏两大瓶颈。
   https://arxiv.org/abs/2608.03979

60. **SwanTale：面向指令与零样本任务的统一多说话人语音与音频生成** — HuggingFace Daily Papers（社区热门论文）（8/3 08:00）
...
61. **OpenAI Astra 以约2000美元证明10项数学难题** — X：Greg Brockman (@gdb)（8/1 15:39）
...
## 技巧与观点
62. **开源「活人感写作.skill」：一个帮你写出没有AI味的文字的通用写作技能** — 公众号：数字生命卡兹克（今天 09:35）
...
63. **烧了5亿token后，我给Codex和Claude Code做Skill上下文瘦身的新技巧** — 公众号：卡尔的AI沃茨（今天 14:46）
...
64. **用 Google Meridian 构建端到端贝叶斯营销组合模型：媒体测量、ROI 分析与预算优化** — MarkTechPost（RSS）（今天 05:56）
...
65. **为什么传奇的埃尔德什问题正被人工智能攻克** — Hacker News 热门（buzzing.cc 中文翻译）（今天 00:23）
...
66. **用 Claude Fable 5 一次性生成完整《Raccoon Heist》游戏** — Simon Willison 博客（今天 03:42）
...
67. **AI智能体尚无法开展开放式AI研究** — AI as Normal Technology（RSS）（今天 21:49）
...
68. **在单颗 AMD MI300X 上运行 DeepSeek V4 Flash** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 20:56）
...
69. **MiniMax-H3 通过 MLX 移植可在 Apple Silicon 上运行** — Simon Willison 博客（昨天 03:10）
...
70. **用 NVIDIA SkillSpector、LangGraph、YARA 规则、SARIF 与 CI 策略门构建高级 AI 技能安全审计流水线** — MarkTechPost（RSS）（昨天 16:12）
...
71. **Cloudflare 如何用软件工厂将 Astro 的 GitHub issue 数降至零** — Cloudflare Blog（昨天 21:00）
...
72. **AirLLM 实现单块 4GB GPU 运行 70B 模型推理** — Hacker News 热门（buzzing.cc 中文翻译）（8/4 00:24）
   AirLLM 项目支持在单块 4GB 显存 GPU 上运行 70B 参数大模型推理，无需多卡或大规模显存配置。该项目已开源，相关讨论在 Hacker News 上获得 103 点热度，引发社区关注。
   https://github.com/lyogavin/airllm

73. **GitHub 如何用堆叠式 Pull Request 拆解 AI 生成的巨型代码** — GitHub Blog（昨天 00:47）
...
74. **从零开始，教你用Codex搓出属于你自己的第一个硬件** — 公众号：数字生命卡兹克（昨天 08:06）
...
75. **杰文斯悖论还能持续吗：AI 定价分层如何支撑算力需求增长** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
...
76. **Palantir 强劲季度后，CEO Alex Karp 称 AI 行业"马克思主义"** — TechCrunch：AI（RSS）（8/4 07:19）
...
77. **Kimi Work 幻灯片制作教程发布** — X：Kimi.ai (@Kimi_Moonshot)（8/3 19:51）
...
78. **Codex 用 Sol 指挥 Luna Max 省额度翻倍产出** — X：阿易 AI Notes (@AYi_AInotes)（8/2 18:47）
...
79. **animated-voiceover 开源：一人干翻动画工作室** — X：阿易 AI Notes (@AYi_AInotes)（8/1 00:01）
...
80. **smevals：用于评测模型、提示词与评测框架的小型评测套件** — Simon Willison 博客（8/1 05:15）
...
81. **Claude Code 连接器可复用至 Artifacts** — X：Thariq (@trq212)（8/4 05:13）
...
82. **实测MiniMax H3做后期和动效：视频届的审美王来了** — 公众号：卡尔的AI沃茨（8/2 21:26）
...
83. **Google Agent Skills 幕后：如何构建、测试与规模化** — Google AI：DEV 作者专属（RSS）（8/3 22:33）
...
84. **Tailscale 未能阻止 Hugging Face 入侵事件复盘** — Hacker News 热门（buzzing.cc 中文翻译）（8/1 04:25）
...
85. **OpenAI 新模型 Astra 数学表现出色，但被过度吹捧** — Gary Marcus：The Road to AI We Can Trust（RSS）（8/3 05:25）
...
86. **EA 首席战略官谈生成式 AI 如何进入可游玩的实时游戏世界** — Runway：News（网页）（8/3 00:00）
