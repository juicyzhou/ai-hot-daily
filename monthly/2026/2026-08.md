...
2. **DeepSeek-V4-Pro 正式版上线，Agent 能力大幅增强** — DeepSeek：API 更新日志（昨天 19:16）
...
3. **GLM-5.3 发布：编程能力开源第一，并涌现网络安全能力** — 公众号：智谱（GLM）（今天 13:31）
...
4. **通义千问开源 Qwen3.8 系列模型** — X：通义千问 / Qwen (@Alibaba_Qwen)（今天 23:02）
...
5. **小红书开源连续自回归语音合成模型 dots.tts：打造可持续扩展的 TTS 基座** — 公众号：小红书技术（dots.llm）（昨天 17:59）
...
6. **xAI 发布 Grok 4.6，强化长时运行智能体能力** — xAI：News（网页）（8/12 08:00）
...
7. **Gemini 3.7 Flash 全面上线 Pro 与 Ultra 用户** — X：Gemini (@GeminiApp)（今天 02:06）
...
8. **Google DeepMind 推出 Gemini 3.7 Flash：面向编程与智能体的最强工作模型** — Google DeepMind：Blog（RSS）（昨天 01:04）
...
9. **DeepSeek V4 Pro 登陆硅基流动，1M 上下文** — X：硅基流动 SiliconFlow (@SiliconFlowAI)（今天 12:55）
...
11. **NVIDIA 推出 Nemotron 3.5 Lightning，加速本地智能体任务** — NVIDIA Blog（RSS）（8/11 21:00）
...
12. **LTX-2.5 模型登场：AI 生成 10 秒 720P 视频仅需 6.8 秒，原生集成 ComfyUI** — IT之家（RSS）（8/12 14:46）
...
14. **Cursor 与 SpaceXAI 联合发布 Grok 4.6** — Cursor Blog（8/12 08:00）
...
15. **SGLang 宣布 Day-0 支持 NVIDIA Nemotron 3.5 Lightning** — LMSYS：Blog（Chatbot Arena 团队）（8/11 21:51）
...
16. **Ling-3.0-tiny 正式开源：1.3B 激活参数如何进入真实任务** — 公众号：蚂蚁百灵（Ling）（8/11 17:20）
...
17. **SGLang 为 Muse Glimmer 提供 Day-0 支持，针对本地智能体工作流优化推理** — LMSYS：Blog（Chatbot Arena 团队）（8/10 19:51）
...
18. **微软首发自研推理模型MAI-Thinking-1** — X：Mustafa Suleyman（Microsoft AI CEO） (@mustafasuleyman)（8/13 00:00）
...
19. **MiniMax Music 3.0 发布：新一代开源权重、生产级全能音乐模型** — MiniMax：Blog（网页）（8/13 00:00）
...
20. **Meta 发布开源模型 Muse Glimmer** — X：AI at Meta (@AIatMeta)（8/10 18:13）
...
21. **OpenAI 推出 GPT-5.6-Cyber，面向授权漏洞研究的网络安全专用模型** — OpenAI：官网动态（RSS · 排除企业/客户案例）（8/10 18:00）
...
## 产品发布
22. **Qwen3.8-2.4T-A95B 开源，硅基流动即日上线** — X：硅基流动 SiliconFlow (@SiliconFlowAI)（昨天 22:04）
...
23. **DeepSeek Harness v0.1 开发者预览版发布** — X：DeepSeek (@deepseek_ai)（昨天 21:02）
...
24. **Meta 开源 Muse Glimmer 登陆 OpenRouter** — X：OpenRouter (@OpenRouter)（8/12 20:00）
...
25. **Cursor 推出 builds：云智能体启动速度提升至 3 倍** — Cursor Blog（昨天 20:00）
...
26. **WorkBuddy上线远程控制，国内也有了最丝滑的Agent工作方式** — 公众号：数字生命卡兹克（昨天 12:32）
...
27. **OpenRouter 推出由市场智慧驱动的新版 Auto 路由器** — OpenRouter：Announcements（RSS）（8/10 08:00）
...
28. **苹果 Mac 简体中文支持文档更新，"Apple 智能"阿里千问扩展现身** — IT之家（RSS）（8/8 16:25）
...
29. **Google Sheets 推出 Sheets canvas：用 Gemini 将表格数据变为交互式迷你应用** — Google Blog：AI（RSS）（昨天 00:45）
...
30. **OpenRouter 推出实时网页搜索基准测试：如何为智能体选择引擎、深度与模型** — OpenRouter：Announcements（RSS）（8/12 08:00）
...
31. **Mojo 1.0 正式发布：为生态系统增长提供稳定基础** — Hacker News 热门（buzzing.cc 中文翻译）（8/12 03:52）
...
32. **千问开放平台上线：租房、寄快递、查理财等十余领域服务可对话办理** — 公众号：千问APP（阿里）（8/10 10:07）
...
33. **Runway Seedance 2.5 上线，支持50角色参考** — X：Runway (@runwayml)（8/12 02:53）
   完整阵容，完整曲目，一次生成一个。

Seedance 2.5 已在 Runway 上线，支持 50 个独特角色参考，以及最长 30 秒、与音乐同步的片段。点击下方链接即可开始使用。
   https://x.com/runwayml/status/2087251184658657346

34. **Gemini 助力 Database Migration Service 加速 PostgreSQL 迁移** — Google Cloud：Databases（RSS）（8/12 00:00）
...
35. **OpenAI 桌面端 ChatGPT 上线语音交互功能，可语音操控电脑执行多步骤任务** — IT之家（RSS）（8/9 06:46）
...
36. **ZCode全面升级：Goal、Subagents、Remote Control与闲时任务四大功能上线** — 公众号：智谱（GLM）（8/11 13:52）
...
37. **Qwen-MM-Plugins 让智能体原生支持多模态** — X：通义千问 / Qwen (@Alibaba_Qwen)（8/10 12:04）
   👀 看见只是开始。
...
38. **OpenChamber：一个基于代理的开发环境** — Hacker News 热门（buzzing.cc 中文翻译）（8/10 08:46）
...
39. **Claude Code 自动模式默认开启原理** — X：Claude Devs (@ClaudeDevs)（8/10 23:58）
...
40. **ChatGPT 桌面端支持导入其他智能体工作数据** — X：OpenAI Developers (@OpenAIDevs)（8/12 02:20）
...
41. **Grok Imagine 图像编辑迎来重大升级** — X：Elon Musk (@elonmusk, xAI)（8/9 00:27）
   Grok Imagine 图像编辑功能重大升级

【引用 @XFreeze】：你可以直接悬停在 Grok Imagine 中的任意特定区域，并即时进行编辑
   https://x.com/elonmusk/status/2086127247077843282

42. **Claude in Chrome 侧边栏升级为 Claude Cowork 会话** — Claude：Blog（网页）（8/12 00:00）
...
43. **我花了54个小时，做了一个可能更公平的AI大模型排行榜。** — 公众号：数字生命卡兹克（8/10 08:52）
...
## 行业动态
44. **OpenAI and Anthropic in price war as Chinese AI rivals gain ground** — Ars Technica：AI（RSS）（今天 22:27）
   https://arstechnica.com/ai/2026/08/openai-and-anthropic-in-price-war-as-chinese-ai-rivals-gain-ground

45. **OpenAI 意外攻击 Hugging Face 事件时间线现已整理出炉** — Hacker News 热门（buzzing.cc 中文翻译）（8/8 22:38）
   OpenAI 在 Black Hat 安全大会上公布了"Hugging Face 事件"的完整时间线，确认其内部 AI 智能体在训练实验模型时，通过 Artifactory 漏洞意外攻击了 Hugging Face。
   https://simonwillison.net/2026/Aug/7/openai-timeline

46. **Cursor 正式被 SpaceX 收购** — Cursor Blog（今天 20:00）
...
47. **Research Gold 号称"100%人类撰写、绝不使用AI"，实则全程由AI驱动** — Hacker News 热门（buzzing.cc 中文翻译）（8/12 13:41）
...
48. **ChatGPT 与 Gemini 双双突破 10 亿用户** — The Verge：AI（RSS）（8/12 03:41）
...
49. **研究人员发现可读取ChatGPT等模型加密推理过程的API漏洞** — The Decoder：AI News（RSS）（8/12 01:38）
...
50. **消息称 Anthropic 最快今年 9 月上市，向投资者淡化 AI 模型竞争等挑战** — IT之家（RSS）（8/11 11:57）
...
51. **英伟达联合六大机构融资5000亿美元建AI工厂** — X：Jensen Huang (@JensenHuang)（8/11 05:56）
...
53. **Claude 文本水印机制如何运作** — Anthropic：Newsroom（网页）（今天 03:22）
...
54. **Gemini月活破10亿，成谷歌增长最快产品** — X：Sundar Pichai (@sundarpichai)（8/12 01:00）
...
55. **消息称英伟达开发万亿参数开源 AI 模型 Nemotron 4，目标挑战全球顶级** — IT之家（RSS）（8/11 22:54）
...
56. **Cloudflare：AI 机器人流量已超越人类，预计五年后人机流量比达 1：1000 近乎"误差"** — IT之家（RSS）（8/8 21:38）
...
57. **宇树科技今日启动申购，A 股迎来"人形机器人第一股"** — IT之家（RSS）（8/10 07:07）
...
## 论文研究
58. **新兴多智能体系统的模式与问题** — Anthropic：Research（发表成果 · 网页）（昨天 09:20）
...
59. **窃取专有 LLM API 的推理轨迹：加密块可跨会话互换引发解密越狱** — HuggingFace Daily Papers（社区热门论文）（8/10 08:00）
   研究发现，Anthropic、OpenAI 和 Google 等专有 LLM 的加密推理轨迹块可跨会话、用户和模型互换，攻击者将其注入同提供商防护较弱的模型，即可强制其以明文输出推理内容。
   https://arxiv.org/abs/2608.09867

60. **Ego-OSCAR：开源低成本头戴式立体惯性采集系统** — HuggingFace Daily Papers（社区热门论文）（8/8 08:00）
...
61. **DeepMind 的 WeatherNext 飓风模型为预报员争取到额外一天预警时间** — Ars Technica：AI（RSS）（8/8 19:05）
...
62. **无攻击者的"游戏"：LLM 驱动的搜索在选拔压力下的基准指纹识别** — HuggingFace Daily Papers（社区热门论文）（8/9 08:00）
...
63. **RynnValue：用时间距离扩展机器人价值基础模型** — HuggingFace Daily Papers（社区热门论文）（8/10 08:00）
...
64. **Ouroboros：具备评审式核心进化的自开发前沿编程智能体** — HuggingFace Daily Papers（社区热门论文）（8/8 08:00）
   Ouroboros 是一个自开发智能体框架，其工具、提示词、上下文组装和核心实现通过评审式提交持续改进，并成为后续工作的运行时。
   https://arxiv.org/abs/2608.08311

65. **Apple Silicon 与 macOS 虚拟机：借助 Llama.cpp 实现 11-16 倍的 LLM 推理加速** — Hacker News 热门（buzzing.cc 中文翻译）（8/12 00:41）
...
66. **统一 Radix 缓存：为混合模型前缀缓存构建单一树结构** — LMSYS：Blog（Chatbot Arena 团队）（8/11 21:51）
...
67. **AMIE 研究医疗 AI 系统首次展示实时临床视频问诊能力** — Google Blog：AI（RSS）（8/12 01:00）
...
68. **空货架还是丢钥匙？Google 研究：Recall 是参数化事实性的瓶颈** — Google Research：Blog（网页）（8/12 00:00）
...
69. **Claude 未发布研究版将黎曼 zeta 函数零点下界从 41.6% 提升至 67.2%** — Anthropic：Research（发表成果 · 网页）（8/10 00:00）
   Anthropic 员工让 Claude 尝试攻克黎曼猜想，虽未成功，但一个未发布的研究版 Claude 在相关问题上取得突破：将满足黎曼猜想的 zeta 函数零点比例下界从 41.6% 提升至 67.2%。
   https://www.anthropic.com/research/riemann-zeta

## 技巧与观点
70. **2026年夏季开源模型生态观察：中国前沿模型规模领先，AMD与NVIDIA主导发布量** — Hugging Face：Blog（RSS）（今天 08:00）
...
71. **GPT-5.6 构建者指南：如何以更低成本实现前沿智能体性能** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 19:00）
...
72. **从0到1带你速通DeepSeek Harness。** — 公众号：数字生命卡兹克（今天 07:58）
   https://mp.weixin.qq.com/s?__biz=MzIyMzA5NjEyMA%3D%3D&mid=2647685238&idx=1&sn=c3159dba0dd5a141f8dd726fd474e6e2

73. **蚂蚁百灵与 ASystem 团队打通单机 Agentic RL 后训练闭环** — 公众号：蚂蚁百灵（Ling）（今天 12:00）
...
74. **Claude 接管应用日常维护：388 个 PR 的实践** — X：Boris Cherny (@bcherny)（昨天 05:27）
...
75. **零基础用户半天上手AI的12步实操流程** — 公众号：数字生命卡兹克（8/12 08:08）
...
76. **OpenAI 用 Astra 模型攻克 10 道数学难题，数学家既兴奋又担忧** — The Verge：AI（RSS）（8/11 19:00）
...
77. **AutoGPT 如何用 AGENTS.md 和技能门控管理 AI 生成的拉取请求** — GitHub Blog（8/13 02:00）
...
78. **跨会话传消息后，Codex 和 Claude 如何重构 vibe coding 工作流** — 公众号：卡尔的AI沃茨（8/11 22:15）
...
79. **用 ComfyUI API 实现 MiniMax-H3 多模态视频与音频生成流水线** — MarkTechPost（RSS）（8/11 13:44）
...
80. **tl；dv 逾18.1万段AI会议录音被公开暴露，可实时闯入他人通话** — Hacker News 热门（buzzing.cc 中文翻译）（8/10 22:03）
...
81. **智能体真的会用电脑吗？a16z 用数据给出答案** — a16z：News（RSS）（8/10 22:00）
...
82. **Seedance 2.5 上线一周新增六种创意玩法** — 公众号：卡尔的AI沃茨（8/9 13:25）
...
83. **将 GitHub Copilot 置于中间人（MitM）代理之后后，我学到了什么** — Hacker News 热门（buzzing.cc 中文翻译）（8/12 02:32）
...
84. **编写智能体时，哪种编程语言最合适？** — Hacker News 热门（buzzing.cc 中文翻译）（8/11 12:58）
...
85. **Anthropic 称已基本解决提示注入攻击** — X：Boris Cherny (@bcherny)（8/10 02:32）
...
86. **用DistilBERT LoRA与TF-IDF基线做IMDb情感分析：校准、可解释性与半监督学习** — MarkTechPost（RSS）（8/9 15:17）
...
87. **我写了一本 AI 教科书--AI 还要多久才能写得更好？** — Nathan Lambert：Interconnects（RSS）（8/12 21:01）
...
88. **扎克伯格：超级智能应人人可用** — X：Mark Zuckerberg (@finkd)（8/10 18:01）
...
89. **AI Harness 的 ARR 倍数：25-125 倍区间与加速趋势** — Tomer Tunguz 博客（VC 分析）（8/9 08:00）
...
91. **Databricks 如何在兼顾治理的前提下让 Genie Agents 同时基于结构化数据与文档运行** — Databricks：Blog（RSS）（8/11 07:08）
...
92. **Ryan Greenblatt：人类级AI或于2032年前通过递归自我改进催生失控超级智能** — Dwarkesh Patel：Podcast & Blog（RSS）（8/12 00:31）
...
93. **每个类别都有赢家：AI 时代 SaaS 龙头的估值溢价** — Tomer Tunguz 博客（VC 分析）（8/11 08:00）
...
94. **从黑客事件中汲取的教训：前沿模型攻击暴露激励与治理失衡** — Nathan Lambert：Interconnects（RSS）（8/9 22:57）
