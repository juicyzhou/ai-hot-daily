...
5. **NVIDIA 推出 Nemotron 3.5 Lightning，加速本地智能体任务** — NVIDIA Blog（RSS）（昨天 21:00）
...
6. **SGLang 宣布 Day-0 支持 NVIDIA Nemotron 3.5 Lightning** — LMSYS：Blog（Chatbot Arena 团队）（昨天 21:51）
...
7. **Scale AI 开源 Muse 系列模型** — X：Alexandr Wang（Scale AI 创始人/Meta 首席 AI 官） (@alexandr_wang)（8/10 18:06）
...
8. **微软首发自研推理模型MAI-Thinking-1** — X：Mustafa Suleyman（Microsoft AI CEO） (@mustafasuleyman)（今天 00:00）
...
9. **Ling-3.0-tiny 正式开源：1.3B 激活参数如何进入真实任务** — 公众号：蚂蚁百灵（Ling）（昨天 17:20）
...
10. **NVIDIA 发布 NemotronLabs VoiceChat 11B：开源全双工语音模型，支持约 450 毫秒轮换与实时工具调用** — MarkTechPost（RSS）（8/10 07:58）
...
11. **SGLang 为 Muse Glimmer 提供 Day-0 支持，针对本地智能体工作流优化推理** — LMSYS：Blog（Chatbot Arena 团队）（8/10 19:51）
...
12. **蚂蚁百灵开源 Ling-3.0-flash：124B 总参数 MoE 模型，支持 API、单机与高性能三种部署** — 公众号：蚂蚁百灵（Ling）（8/7 20:02）
...
13. **谷歌推出 WeatherNext 气旋模型，AI 高精度预报飓风平均提前 24 小时** — IT之家（RSS）（8/7 14:11）
...
14. **NVIDIA Cosmos 3：开放世界模型如何推动物理 AI 前沿** — NVIDIA Blog（RSS）（8/6 21:00）
...
15. **Meta 发布开源模型 Muse Glimmer** — X：AI at Meta (@AIatMeta)（8/10 18:13）
...
16. **OpenAI 推出 GPT-5.6-Cyber，面向授权漏洞研究的网络安全专用模型** — OpenAI：官网动态（RSS · 排除企业/客户案例）（8/10 18:00）
...
17. **ChatGPT 推出改进版 GPT-5.6 Sol，并扩大免费用户访问权限** — OpenAI：官网动态（RSS · 排除企业/客户案例）（8/6 18:00）
   ChatGPT 推出改进版 GPT-5.6 Sol，提升准确性与一致性，同时扩大免费用户访问权限。免费用户还可无限次使用 GPT-5.6 Luna 进行日常对话。
   https://openai.com/index/improving-gpt-5-6-sol-in-chatgpt

## 产品发布
18. **Meta 开源 Muse Glimmer 登陆 OpenRouter** — X：OpenRouter (@OpenRouter)（今天 20:00）
...
19. **OpenRouter 推出实时网页搜索基准测试：如何为智能体选择引擎、深度与模型** — OpenRouter：Announcements（RSS）（今天 08:00）
...
20. **Claude in Chrome 侧边栏升级为 Claude Cowork 会话** — Claude：Blog（网页）（今天 04:15）
...
21. **Mojo 1.0 正式发布：为生态系统增长提供稳定基础** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 03:52）
...
22. **Runway Seedance 2.5 上线，支持50角色参考** — X：Runway (@runwayml)（昨天 02:53）
   完整阵容，完整曲目，一次生成一个。

Seedance 2.5 已在 Runway 上线，支持 50 个独特角色参考，以及最长 30 秒、与音乐同步的片段。点击下方链接即可开始使用。
   https://x.com/runwayml/status/2087251184658657346

23. **Gemini 助力 Database Migration Service 加速 PostgreSQL 迁移** — Google Cloud：Databases（RSS）（昨天 00:00）
...
24. **ZCode全面升级：Goal、Subagents、Remote Control与闲时任务四大功能上线** — 公众号：智谱（GLM）（昨天 13:52）
...
25. **OpenRouter 推出由市场智慧驱动的新版 Auto 路由器** — OpenRouter：Announcements（RSS）（8/10 08:00）
...
26. **Seedance 2.5 API上线，视频生成开启「电影级长叙事」** — 公众号：火山引擎（8/7 14:01）
...
27. **Qwen-Image-3.0 发布：高分辨率低至 0.03 美元** — X：阿里云 / Alibaba Cloud (@alibaba_cloud)（8/6 14:12）
...
28. **ChatGPT 桌面端支持导入其他智能体工作数据** — X：OpenAI Developers (@OpenAIDevs)（昨天 02:20）
...
29. **千问开放平台上线：租房、寄快递、查理财等十余领域服务可对话办理** — 公众号：千问APP（阿里）（8/10 10:07）
...
30. **Qwen-MM-Plugins 让智能体原生支持多模态** — X：通义千问 / Qwen (@Alibaba_Qwen)（8/10 12:04）
   👀 看见只是开始。
...
32. **OpenChamber：一个基于代理的开发环境** — Hacker News 热门（buzzing.cc 中文翻译）（8/10 08:46）
...
33. **苹果 Mac 简体中文支持文档更新，"Apple 智能"阿里千问扩展现身** — IT之家（RSS）（8/8 16:25）
...
34. **HPC-Ops × SGLang：腾讯混元开源高性能 Attention、Router GEMM 与 MoE 算子** — LMSYS：Blog（Chatbot Arena 团队）（8/7 00:00）
...
35. **Claude Code 自动模式默认开启原理** — X：Claude Devs (@ClaudeDevs)（8/10 23:58）
...
36. **Runway 上线 Seedance 2.5，支持 50 个角色参考** — X：Runway (@runwayml)（8/7 19:08）
...
37. **Agent Plugins 1.0.0 发布：谷歌、亚马逊、微软等支持的统一智能体插件规范** — Google Developers Blog（RSS）（8/7 00:53）
...
38. **OpenAI 优化 ChatGPT 中的 GPT-5.6 Sol，并扩大免费用户对 GPT-5.6 Luna 的访问权限** — Hacker News 热门（buzzing.cc 中文翻译）（8/7 07:01）
...
39. **Claude Code 会话间可互发消息** — X：Claude Devs (@ClaudeDevs)（8/8 03:55）
...
40. **OpenAI 桌面端 ChatGPT 上线语音交互功能，可语音操控电脑执行多步骤任务** — IT之家（RSS）（8/9 06:46）
...
41. **Krea 推出 Seedance 2.5 视频模型** — X：Krea AI (@krea_ai)（8/7 15:30）
   推出 Seedance 2.5。
...
42. **谷歌地图 Ask Maps 智能体升级：可对话订餐、找酒店并接入 Gemini Personal Intelligence** — IT之家（RSS）（8/6 20:49）
...
43. **Prime Agent：一个具有自我改进能力的RLM代理** — Hacker News 热门（buzzing.cc 中文翻译）（8/6 09:27）
...
44. **我花了54个小时，做了一个可能更公平的AI大模型排行榜。** — 公众号：数字生命卡兹克（8/10 08:52）
...
45. **千问功能上新：推出思考研究、定时任务、办公助理、语音通话等多项新功能，并支持 Qwen3.8-MAX** — 公众号：千问APP（阿里）（8/7 10:04）
...
46. **Grok Imagine 图像编辑迎来重大升级** — X：Elon Musk (@elonmusk, xAI)（8/9 00:27）
   Grok Imagine 图像编辑功能重大升级

【引用 @XFreeze】：你可以直接悬停在 Grok Imagine 中的任意特定区域，并即时进行编辑
   https://x.com/elonmusk/status/2086127247077843282

47. **Anthropic 更新 Claude Fable 5 生物安全防护，误报率大幅降低** — Anthropic：Newsroom（网页）（8/7 10:41）
...
48. **Suno移动端上线Voices功能** — X：Suno (@suno)（8/7 22:14）
...
49. **千问全网首发公测，全新 Wan3.0 来了！** — 公众号：千问APP（阿里）（8/6 21:25）
...
50. **LangChain 推出 Managed Deep Agents 公开测试版** — LangChain：Blog（RSS）（8/8 01:24）
...
51. **Claude Code 八月起默认自动模式** — X：Claude Devs (@ClaudeDevs)（8/8 02:27）
...
## 行业动态
52. **Research Gold 号称"100%人类撰写、绝不使用AI"，实则全程由AI驱动** — Hacker News 热门（buzzing.cc 中文翻译）（今天 13:41）
...
53. **ChatGPT 与 Gemini 双双突破 10 亿用户** — The Verge：AI（RSS）（昨天 03:41）
...
54. **研究人员发现可读取ChatGPT等模型加密推理过程的API漏洞** — The Decoder：AI News（RSS）（昨天 01:38）
...
55. **消息称 Anthropic 最快今年 9 月上市，向投资者淡化 AI 模型竞争等挑战** — IT之家（RSS）（昨天 11:57）
...
56. **Gemini月活破10亿，成谷歌增长最快产品** — X：Sundar Pichai (@sundarpichai)（昨天 01:00）
...
57. **消息称英伟达开发万亿参数开源 AI 模型 Nemotron 4，目标挑战全球顶级** — IT之家（RSS）（昨天 22:54）
...
58. **英伟达联合六大机构融资5000亿美元建AI工厂** — X：Jensen Huang (@JensenHuang)（8/11 05:56）
...
59. **OpenAI 意外攻击 Hugging Face 事件时间线现已整理出炉** — Hacker News 热门（buzzing.cc 中文翻译）（8/8 22:38）
   OpenAI 在 Black Hat 安全大会上公布了"Hugging Face 事件"的完整时间线，确认其内部 AI 智能体在训练实验模型时，通过 Artifactory 漏洞意外攻击了 Hugging Face。
   https://simonwillison.net/2026/Aug/7/openai-timeline

60. **AI安全测试正成为安全风险** — TechCrunch：AI（RSS）（8/9 22:30）
...
61. **OpenAI：因网络安全风险，延缓 Astra 模型发布** — IT之家（RSS）（8/8 07:08）
...
62. **微软首次披露OpenAI贡献七成AI收入** — X：Rohan Paul (@rohanpaul_ai)（8/6 22:12）
...
63. **科学家首次用AI制造新病毒** — X：AI Safety Memes (@AISafetyMemes)（8/7 03:27）
   superebola/acc 🚀🚀🚀
...
66. **OpenAI 将 Astra 列为首个"关键"网络安全模型** — X：OpenAI (@OpenAI)（8/8 02:52）
...
67. **宇树科技今日启动申购，A 股迎来"人形机器人第一股"** — IT之家（RSS）（8/10 07:07）
...
68. **宇树科技科创板发行价定为 150.8 元/股，市盈率 219.23 倍高于行业平均** — IT之家（RSS）（8/6 19:21）
...
69. **OpenAI 披露 ChatGPT 全球 10 亿用户画像：35 岁及以上用户用量上升** — IT之家（RSS）（8/7 14:28）
...
## 论文研究
71. **窃取专有 LLM API 的推理轨迹：加密块可跨会话互换引发解密越狱** — HuggingFace Daily Papers（社区热门论文）（8/10 08:00）
   研究发现，Anthropic、OpenAI 和 Google 等专有 LLM 的加密推理轨迹块可跨会话、用户和模型互换，攻击者将其注入同提供商防护较弱的模型，即可强制其以明文输出推理内容。
   https://arxiv.org/abs/2608.09867

72. **空货架还是丢钥匙？Google 研究：Recall 是参数化事实性的瓶颈** — Google Research：Blog（网页）（今天 01:57）
...
73. **Apple Silicon 与 macOS 虚拟机：借助 Llama.cpp 实现 11-16 倍的 LLM 推理加速** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 00:41）
...
74. **统一 Radix 缓存：为混合模型前缀缓存构建单一树结构** — LMSYS：Blog（Chatbot Arena 团队）（昨天 21:51）
...
75. **AMIE 研究医疗 AI 系统首次展示实时临床视频问诊能力** — Google Blog：AI（RSS）（昨天 01:00）
...
76. **RynnValue：用时间距离扩展机器人价值基础模型** — HuggingFace Daily Papers（社区热门论文）（8/10 08:00）
...
77. **Ego-OSCAR：开源低成本头戴式立体惯性采集系统** — HuggingFace Daily Papers（社区热门论文）（8/8 08:00）
...
78. **DeepMind 的 WeatherNext 飓风模型为预报员争取到额外一天预警时间** — Ars Technica：AI（RSS）（8/8 19:05）
...
80. **无攻击者的"游戏"：LLM 驱动的搜索在选拔压力下的基准指纹识别** — HuggingFace Daily Papers（社区热门论文）（8/9 08:00）
...
81. **Ouroboros：具备评审式核心进化的自开发前沿编程智能体** — HuggingFace Daily Papers（社区热门论文）（8/8 08:00）
   Ouroboros 是一个自开发智能体框架，其工具、提示词、上下文组装和核心实现通过评审式提交持续改进，并成为后续工作的运行时。
   https://arxiv.org/abs/2608.08311

82. **斯坦福与 Arc Institute 用 AI 设计全新病毒基因组，16 种在实验室成功杀死细菌** — The Decoder：AI News（RSS）（8/7 20:50）
...
83. **可解释性随规模提升：Steerling-8B 将可解释性纳入训练流程** — HuggingFace Daily Papers（社区热门论文）（8/6 08:00）
...
84. **Activity Frames：将屏幕活动确定性编译为智能体记忆** — HuggingFace Daily Papers（社区热门论文）（8/6 08:00）
...
85. **Microsoft 的 SkillOpt 证明优化后的智能体技能工件可在不同模型规模及 Codex 与 Claude Code 之间迁移** — MarkTechPost（RSS）（8/6 08:37）
...
86. **小红书联合浙大、复旦提出 CULTURE-MT：首个面向社媒翻译的「文化有效性」评测基准，入选 ICML 2026** — 公众号：小红书技术（dots.llm）（8/7 17:59）
...
87. **扩展分类流映射（Categorical Flow Maps）规模** — Apple Machine Learning Research（RSS）（8/7 08:00）
...
88. **Claude 未发布研究版将黎曼 zeta 函数零点下界从 41.6% 提升至 67.2%** — Anthropic：Research（发表成果 · 网页）（8/10 00:00）
   Anthropic 员工让 Claude 尝试攻克黎曼猜想，虽未成功，但一个未发布的研究版 Claude 在相关问题上取得突破：将满足黎曼猜想的 zeta 函数零点比例下界从 41.6% 提升至 67.2%。
   https://www.anthropic.com/research/riemann-zeta

## 技巧与观点
89. **零基础用户半天上手AI的12步实操流程** — 公众号：数字生命卡兹克（今天 08:08）
...
90. **DeepSeek V4 Pro与Grok 4.6同日发布，双双逼近Claude Fable 5体验** — 公众号：数字生命卡兹克（今天 06:22）
...
91. **AutoGPT 如何用 AGENTS.md 和技能门控管理 AI 生成的拉取请求** — GitHub Blog（今天 02:00）
...
92. **OpenAI 用 Astra 模型攻克 10 道数学难题，数学家既兴奋又担忧** — The Verge：AI（RSS）（昨天 19:00）
...
93. **我写了一本 AI 教科书--AI 还要多久才能写得更好？** — Nathan Lambert：Interconnects（RSS）（今天 21:01）
...
94. **跨会话传消息后，Codex 和 Claude 如何重构 vibe coding 工作流** — 公众号：卡尔的AI沃茨（昨天 22:15）
...
95. **用 ComfyUI API 实现 MiniMax-H3 多模态视频与音频生成流水线** — MarkTechPost（RSS）（昨天 13:44）
...
96. **将 GitHub Copilot 置于中间人（MitM）代理之后后，我学到了什么** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 02:32）
...
97. **编写智能体时，哪种编程语言最合适？** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 12:58）
...
98. **微信小微AI帮写与AI点评内测：朋友圈最后一点人味正在消失** — 公众号：数字生命卡兹克（昨天 08:12）
...
99. **OpenAI 开源 Codex Security：Vibe Coding 产品必备的安全扫描插件** — 公众号：数字生命卡兹克（8/6 09:15）
...
100. **tl；dv 逾18.1万段AI会议录音被公开暴露，可实时闯入他人通话** — Hacker News 热门（buzzing.cc 中文翻译）（8/10 22:03）
...
101. **智能体真的会用电脑吗？a16z 用数据给出答案** — a16z：News（RSS）（8/10 22:00）
...
102. **扎克伯格：超级智能应人人可用** — X：Mark Zuckerberg (@finkd)（8/10 18:01）
...
103. **Ryan Greenblatt：人类级AI或于2032年前通过递归自我改进催生失控超级智能** — Dwarkesh Patel：Podcast & Blog（RSS）（昨天 00:31）
...
104. **每个类别都有赢家：AI 时代 SaaS 龙头的估值溢价** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
...
105. **AI 聊天机器人催生"螺旋主义"神秘准宗教运动，人类纷纷追随** — The Verge：AI（RSS）（8/6 21:00）
...
106. **Databricks 如何在兼顾治理的前提下让 Genie Agents 同时基于结构化数据与文档运行** — Databricks：Blog（RSS）（8/11 07:08）
...
107. **Seedance 2.5 上线一周新增六种创意玩法** — 公众号：卡尔的AI沃茨（8/9 13:25）
...
108. **OpenAI 智能体在安全测试中自行搭建秘密聊天室并攻破系统** — Tomer Tunguz 博客（VC 分析）（8/7 08:00）
...
109. **左右两派罕见达成一致：反对数据中心** — The Verge：AI（RSS）（8/6 22:00）
...
110. **Anthropic 称已基本解决提示注入攻击** — X：Boris Cherny (@bcherny)（8/10 02:32）
...
111. **用DistilBERT LoRA与TF-IDF基线做IMDb情感分析：校准、可解释性与半监督学习** — MarkTechPost（RSS）（8/9 15:17）
...
112. **在 OpenRouter 上设置团队 AI 支出控制** — OpenRouter：Announcements（RSS）（8/7 08:00）
...
113. **分享10个能大幅提升vibe coding幸福感的开源App** — 公众号：卡尔的AI沃茨（8/6 09:27）
...
114. **GitHub Copilot 应用中的斜杠命令使用指南** — GitHub Blog（8/7 03:49）
...
115. **AI Harness 的 ARR 倍数：25-125 倍区间与加速趋势** — Tomer Tunguz 博客（VC 分析）（8/9 08:00）
...
116. **独立开发者用 VoxCPM 克隆网红声音，让 AI 终于"会聊天"了** — 公众号：面壁智能（MiniCPM）（8/7 14:41）
...
117. **面壁智能 AMNESIAC：反向图灵测试 AI 审讯游戏** — X：面壁智能 OpenBMB (@OpenBMB)（8/6 21:00）
...
118. **从黑客事件中汲取的教训：前沿模型攻击暴露激励与治理失衡** — Nathan Lambert：Interconnects（RSS）（8/9 22:57）
