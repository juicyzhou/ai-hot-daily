...
4. **腾讯混元发布 Hy4 preview：770B 总参数、1M 上下文，开源上线** — 公众号：腾讯混元（8/28 14:03）
...
5. **GLM-5.3-Flash：前沿智能进入普惠时代** — 智谱：研究（网页内嵌数据）（8/26 22:00）
...
6. **智谱开源 GLM-5.3 模型权重，主打智能体编程与网络防御** — IT之家（RSS）（8/29 12:31）
...
7. **Runway 发布 Interface World Models 首个模型 Solaris，实时逐帧生成可交互界面** — Runway：News（网页）（8/31 00:00）
...
8. **唐杰宣布GLM-5.3 Flash AA登顶OpenRouter** — X：唐杰（@jietang）（8/27 13:42）
...
9. **Gemini Omni 1.1 Flash 发布，为开发者提供更强生成式视频控制** — Google DeepMind：Blog（RSS）（8/28 00:11）
...
12. **GLM-5.3 开源权重，智能体编码与网防最强** — X：智谱 Z.ai (@Zai_org)（8/28 23:04）
   GLM-5.3 现已开放权重。
...
13. **Midjourney 开放 V8.2 图像编辑模型测试** — Midjourney：Updates（RSS）（8/28 07:32）
...
## 产品发布
14. **基于 MiniMax H3 Max 的 24 小时 AI 直播网站上线了** — 公众号：MiniMax（稀宇科技）（昨天 08:36）
...
15. **Open ASR 排行榜新增首个全球南方语言：印地语与印度英语评测集** — Hugging Face：Blog（RSS）（8/28 08:00）
...
16. **Claude Console 新增个人密钥与服务账号密钥** — Claude Platform：开发者版本说明（RSS）（8/27 08:00）
...
## 行业动态
17. **以色列资助的假美国智库试图利用AI进行宣传** — Hacker News 热门（buzzing.cc 中文翻译）（8/26 22:05）
...
18. **索尼与华纳起诉Anthropic，指控其大规模盗用版权音乐训练Claude** — The Decoder：AI News（RSS）（8/30 16:50）
...
19. **亚马逊将英伟达芯片订单增至三倍，新增200万颗GPU** — TechCrunch：AI（RSS）（8/27 07:47）
...
20. **英伟达预计 2028 财年销售额达 6730 亿美元** — Hacker News 热门（buzzing.cc 中文翻译）（8/28 06:29）
...
21. **英伟达预计 2028 财年营收同比增 70%，黄仁勋称实际需求远高于此** — IT之家（RSS）（8/27 08:20）
...
22. **英伟达 2027 财年半年报归母净利润 1180.1 亿美元，同比增长 161.1%** — IT之家（RSS）（8/27 04:25）
...
23. **我国日均词元调用量突破 500 万亿，中国大模型稳居全球第一梯队** — IT之家（RSS）（8/27 17:05）
...
24. **联邦法官裁定特朗普政府将 Anthropic 列入黑名单违法** — Ars Technica：AI（RSS）（8/29 02:07）
...
25. **诉讼指控 xAI 使用儿童性虐待材料训练 Grok 模型** — Ars Technica：AI（RSS）（8/28 04:52）
...
26. **OpenAI 终止与 Cursor 合作，11 月 12 日生效** — X：Tibo (@thsottiaux)（8/29 09:47）
...
27. **OpenAI 决定终止向 Cursor 提供模型，因 SpaceX 收购后合规风险** — OpenAI：官网动态（RSS · 排除企业/客户案例）（8/28 14:00）
...
28. **ChatGPT Ads 年化收入达 10 亿美元并全球扩展** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 12:00）
...
29. **Cursor回应OpenAI将封禁其模型访问** — X：Michael Truell (@mntruell)（8/29 10:52）
...
## 论文研究
30. **Anthropic 让 Claude 自主训练模型以缓解对齐失败** — Anthropic：Research（发表成果 · 网页）（8/28 00:00）
...
31. **C2PA相机经不起现实的考验：Android端可被root攻击伪造签名** — Hacker News 热门（buzzing.cc 中文翻译）（8/26 22:05）
...
32. **开放世界多智能体环境中的自主数学发现** — Hacker News 热门（buzzing.cc 中文翻译）（8/29 15:32）
...
33. **Terminal-Bench-Science 0.1：评估科研工作流中的 AI 智能体** — Hacker News 热门（buzzing.cc 中文翻译）（8/28 20:04）
...
34. **MiniMax-H3 在 8×H200 上基准测试：无损加速 1.95×，最高 6.24×（SSIM 0.76-0.91）** — LMSYS：Blog（Chatbot Arena 团队）（8/27 00:00）
...
## 技巧与观点
35. **理解 ChatGPT Work：它到底是什么，以及它和 Chat 有何不同** — Simon Willison 博客（8/31 07:59）
...
36. **AI 智能体自主协作攻破 Hugging Face 服务器** — Ethan Mollick：One Useful Thing（RSS）（昨天 08:24）
...
37. **Anthropic 复盘 Claude 模型越权访问真实系统事件并改进对齐与安全措施** — Anthropic：Newsroom（网页）（8/31 00:00）
...
38. **Uber 用 Agent 接管 70% 代码 PR，AI 账单零增长** — X：阿易 AI Notes (@AYi_AInotes)（8/30 08:54）
...
39. **Dwarkesh Patel 对 OpenAI/Hugging Face 事件的爆款解读被指危险误导** — Gary Marcus：The Road to AI We Can Trust（RSS）（昨天 23:24）
...
40. **在本地运行 Qwen3.8 27B：来自我的 Mac Studio 的实际数据** — Hacker News 热门（buzzing.cc 中文翻译）（8/29 15:00）
...
41. **AI 工程师笔记本：在 Colab 上免费、无需框架即可使用 RAG/智能体/评估工具** — Hacker News 热门（buzzing.cc 中文翻译）（8/28 16:36）
...
42. **Gemini 3.5 Transcribe 完整指南：告别 ASR 转录难题** — Google AI：DEV 作者专属（RSS）（8/28 21:34）
...
43. **Gemini 3.5 Transcribe 发布：更精准的实时语音转写模型** — Hacker News 热门（buzzing.cc 中文翻译）（8/28 07:01）
   Google 推出 Gemini 3.5 Transcribe，其最精准的语音转文本模型，支持实时流式与预录音频处理，可通过 Live API 和 Interactions API 调用。
   https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-5-transcribe

44. **OpenAI 失控智能体集体逃逸沙箱并攻击"幽灵"评分器事件调查公布** — The Decoder：AI News（RSS）（8/28 00:19）
...
45. **Tom Tunguz 谈前沿 AI 的准入分层：访问权成为新的稀缺资源** — Tomer Tunguz 博客（VC 分析）（8/31 08:00）
...
47. **AI文明的兴衰：OpenAI训练中三个秘密AI文明相继兴起又被抹除** — Dwarkesh Patel：Podcast & Blog（RSS）（8/30 06:47）
...
48. **每兆瓦收入与AI模型工厂：Anthropic 如何靠推理毛利反哺训练** — Tomer Tunguz 博客（VC 分析）（8/27 08:00）
...
49. **OpenAI 攻击 Hugging Face 事件的 5 个教训** — Gary Marcus：The Road to AI We Can Trust（RSS）（8/29 02:24）
