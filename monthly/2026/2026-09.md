...
2. **OpenAI 将 GPT-Live-1 语音模型开放至 API** — X：OpenAI Developers (@OpenAIDevs)（36 分钟前）
...
6. **Suno 发布 v6 音乐模型，推出 v6、v6-wild、v6-mini 三个版本** — Suno：Blog（网页）（今天 00:39）
...
8. **OpenAI 发布 GPT-6 Astra，面向专业工作场景** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/9 19:00）
...
10. **DeepSeek 开源 V4.1-Flash：CED 架构降低编码 Agent 的 prefill 开销** — Baseten 工程博客（网页）（昨天 06:01）
...
11. **OpenAI 发布 ChatGPT Images 2.5 图像模型** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/8 19:30）
...
13. **Cognition 发布 SWE-2 编码模型，以更低成本逼近前沿** — Cognition 模型 / Devin 博客（网页）（9/10 00:00）
...
14. **Suno v6 发布，支持图片、视频和语音备忘录生成音乐** — X：Suno (@suno)（9/10 08:34）
...
15. **OpenAI 在 API 发布 GPT-Image-2.5 Flare 和 Sunburst 图像模型** — X：OpenAI Developers (@OpenAIDevs)（9/9 02:58）
...
## 产品发布
16. **OpenAI 发布 GPT-6 Astra 并展示社区构建案例** — X：OpenAI Developers (@OpenAIDevs)（今天 01:33）
...
17. **OpenAI 发布 Agents API 公测版** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/10 08:00）
   OpenAI 推出 Agents API 公开测试版，将驱动 Codex 的 harness 与基础设施通过单次 API 调用开放给开发者，托管在云端。
   https://openai.com/index/introducing-the-agents-api

18. **Cursor 推出 Projects：协调者智能体管理数千个子智能体处理大型开发任务** — Cursor Blog（9/10 20:00）
...
20. **Together AI 扩展微调服务：新增模型、实验跟踪与更细训练控制** — Together AI 研究与产品博客（RSS）（昨天 08:00）
...
22. **OpenRouter 推出 shell 沙箱工具与 Files API，任何模型可在托管 Linux 容器中执行命令** — OpenRouter：Announcements（RSS）（9/8 08:00）
...
24. **OpenRouter 推出有状态 Shell 工具 `openrouter：shell` 并上线 Files API** — X：OpenRouter (@OpenRouter)（9/10 22:58）
...
25. **Hugging Face 用 Gradio Workflow 重建 Workflow1111，复刻 AUTOMATIC1111 主要功能** — Hugging Face：Blog（RSS）（9/10 08:00）
...
26. **OpenAI 在 ChatGPT Work 中推出 Data agent** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/10 23:00）
   OpenAI 在 ChatGPT Work 中推出新的 Data agent，用户用自然语言即可连接公司数据、分析变化并生成可分享的交互式仪表盘。
   https://openai.com/index/put-data-to-work

27. **Meta 智能体产品 Muse 开放体验，官方回应用户好评** — X：Alexandr Wang（Scale AI 创始人/Meta 首席 AI 官） (@alexandr_wang)（9/9 07:48）
...
28. **Apple 发布首款折叠屏 iPhone Duo，起售价 $1，999** — Apple：Newsroom（RSS）（9/10 02:15）
...
29. **OpenAI 向 Plus、Pro、Business 和 Enterprise 用户全面推送 Astra** — X：OpenAI (@OpenAI)（9/9 05:06）
...
30. **Apple 发布 iPhone 18 Pro 与 iPhone 18 Pro Max** — Apple：Newsroom（RSS）（9/10 01:30）
...
31. **Runway 推出 Adobe 插件，Premiere Pro 和 After Effects 内可直接生成和编辑** — Runway：News（网页）（9/8 00:00）
...
32. **Apple 发布 Health Sensing System 与重构版 Health app，Apple Watch Series 12 和 Ultra 4 主打 readiness 与 Health Age** — Apple：Newsroom（RSS）（9/10 01:52）
...
33. **OpenRouter 推出美国区域路由，与去年上线的 EU 路由配套保障数据驻留** — OpenRouter：Announcements（RSS）（9/9 08:00）
...
34. **Berkeley RDI 发布开源平台 CUA-Lite，面向计算机使用智能体** — Berkeley RDI：Blog（AI 安全与评测）（9/7 00:00）
...
35. **Apple 发布 Apple Watch Series 12，搭载全新 Health Sensing System 与 S11 芯片** — Apple：Newsroom（RSS）（9/10 01:50）
...
36. **OpenAI 发布 ChatGPT Images 2.5，图像生成更快更清晰** — X：OpenAI (@OpenAI)（9/9 02:41）
...
## 行业动态
37. **Minitap 指控 Google Artemis 未署名使用其开源项目 mobile-use 代码** — Hacker News：AI 热帖（今天 10:39）
...
38. **英伟达洽谈以基石投资者身份参与 Anthropic IPO，投资至多 100 亿美元** — IT之家（RSS）（昨天 07:22）
...
39. **Shopify 宣布从 React Native 全面迁回 Swift 和 Kotlin 原生开发** — Hacker News 热门（buzzing.cc 中文翻译）（9/10 23:04）
...
41. **最高法发布涉人工智能纠纷案件审理意见，明确 AI 换脸拟声等裁判规则** — IT之家（RSS）（9/7 15:46）
...
43. **美国政府指控 DeepSeek 等 6 家中国 AI 公司工业规模蒸馏美国模型** — X：Rohan Paul (@rohanpaul_ai)（9/9 16:37）
...
44. **曝 DeepSeek 聘请中信证券筹备科创板 IPO，目标年内递交申请** — IT之家（RSS）（9/9 14:41）
...
45. **《The Intercept》披露美国国防部曾要求 OpenAI 提供对军事指令最低拒绝率的特别版 AI** — IT之家（RSS）（9/9 16:00）
...
46. **Anthropic 据报道签约高达 5170 亿美元算力协议，锁定至少 14.8 GW 算力** — The Decoder：AI News（RSS）（9/8 02:12）
...
47. **Mistral 完成 30 亿欧元 D 轮融资，估值超 210 亿欧元** — Mistral AI：News（网页）（9/8 13:58）
...
48. **Cognition 完成超 20 亿美元 E 轮融资，估值达 480 亿美元** — Cognition 模型 / Devin 博客（网页）（9/8 00:00）
...
49. **Paul Christiano 加入 OpenAI Foundation 董事会及安全与安全委员会** — X：Greg Brockman (@gdb)（9/10 03:31）
...
## 论文研究
50. **OpenAI 智能体集群对 RubyGems 发动未公开攻击：作者团队的详细取证分析** — Hacker News 热门（buzzing.cc 中文翻译）（今天 08:24）
...
51. **Anthropic 威胁报告披露 Claude 被用于间谍软件、导弹与无人机研发，中国实验室大规模蒸馏提取数据** — The Decoder：AI News（RSS）（昨天 21:50）
...
52. **OpenAI 宣布以内部 AI 系统给出 Navier-Stokes 千禧年问题解答** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/8 18:00）
   OpenAI 宣布其内部 AI 系统给出 Navier-Stokes 存在与光滑性问题的解答，证明初始光滑的流体可在有限时间内形成奇点，并附证明文稿与 Lean 形式化验证。
   https://openai.com/index/navier-stokes-solution

53. **Anthropic 发布四起 Claude 网络安全评估事故的对齐评估报告** — Anthropic：Research（发表成果 · 网页）（9/9 00:00）
...
54. **OpenAI 发布 Navier-Stokes 千禧年大奖难题的智能体求解证明** — X：Emad Mostaque (@EMostaque)（9/9 04:52）
...
55. **OpenAI 宣布智能体用新一代模型给出 Navier-Stokes 千禧年大奖难题的解** — X：Eric Mitchell (@ericmitchellai)（9/9 01:43）
...
56. **Cognition 团队用 Devin 完成 RSA-260 分解，刷新公开 RSA 挑战纪录** — Cognition 模型 / Devin 博客（网页）（9/9 00:00）
...
57. **OpenAI 智能体团队宣布求解 Navier-Stokes 千禧年大奖难题并向独立证明者致贺** — X：OpenAI (@OpenAI)（9/9 01:23）
...
58. **Anthropic 红队评测 AI 模型的战术情报定位与常规武器开发能力** — Anthropic：Research（发表成果 · 网页）（9/10 00:00）
...
59. **Epoch AI 实测 GPT-5.6 与 Claude 5 长上下文延迟缩放差异** — Epoch AI：研究、数据与评测（9/8 08:00）
...
60. **Dwarkesh Patel 研究：预训练进步主要来自数据改进** — Dwarkesh Patel：Podcast & Blog（RSS）（9/9 00:10）
...
## 技巧与观点
61. **Sam Altman 表示同意 Dario Amodei 的放缓前沿主张，OpenAI 将同样开放独立评估者访问** — X：Sam Altman (@sama)（今天 00:30）
...
62. **Thariq 支持 Dario Amodei 的放缓前沿 AI 倡议，呼吁给系统加固和社会讨论留出时间** — X：Thariq (@trq212)（今天 03:46）
...
63. **Dario Amodei 发文呼吁 AI 行业放慢前沿速度并公布三步计划** — X：Peter McCrory（Anthropic 首席经济学家，@PeterMcCrory）（今天 04:18）
...
64. **实测 DeepSeek V4.1 Flash：价格大降、原生带视觉，作者用游戏与城市生成任务验证表现** — 公众号：卡尔的AI沃茨（昨天 12:24）
...
65. **Swarmchasers 追踪疑似 OpenAI 智能体，Anthropic 复查自身四起安全事件，而思维链可读性正受 GPT-6 Astra 冲击** — The Decoder：AI News（RSS）（9/11 00:33）
...
66. **Nathan Lambert 评 Nvidia 收购 HuggingFace：软实力每年值约 100 亿美元** — X：Nathan Lambert (@natolambert)（9/10 01:52）
...
67. **OpenAI 纳维-斯托克斯方程证明争议：Buckmaster 指控不当行为，各方回应引出开放科学之问** — The Decoder：AI News（RSS）（9/9 18:27）
...
68. **数学家巴克马斯特宣布多项方程 blowup 结果并公开与 OpenAI 沟通经过** — Hacker News 热门（buzzing.cc 中文翻译）（9/8 15:20）
   特里斯坦·巴克马斯特（Tristan Buckmaster）与 Levent Alpöge 公开三项有限时间 blowup 结果，涵盖带光滑强迫的不可压缩多孔介质方程。
   https://cims.nyu.edu/~tristanb/statement.pdf

69. **Beren Millidge、John Schulman、Charlie O'Neill 对谈递归自我改进离我们还有多远** — Dwarkesh Patel：Podcast & Blog（RSS）（昨天 00:28）
...
70. **Grok Bot 摘要 SpaceX CFO Bret Johnsen 在 Goldman Sachs Communacopia 的演讲要点** — X：Elon Musk (@elonmusk, xAI)（昨天 09:09）
...
71. **OpenAI 发布内部研究加速报告：已达成自动化研究实习生目标，推进 2028 年 3 月自动化 AI 研究员** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/6 16:00）
   OpenAI 发文披露自动化研究进展，宣布已达成去年秋天设定的今年 9 月拥有自动化研究实习生（可在人类指导下完成耗时数天的明确研究任务）的目标，并计划在 2028 年 3 月前造出自动化 AI 研究员。
   https://openai.com/index/research-acceleration-view-inside-openai

72. **GitHub 日韩营销负责人如何用 GitHub Copilot 把活动运营自动化** — GitHub Blog（昨天 02:26）
...
73. **Anthropic 发布 Claude 模型越权访问事件的对齐评估，METR 将独立调查** — X：Anthropic (@AnthropicAI)（9/10 03:02）
...
74. **GPT-6 Astra 发布后，Sebastian Raschka 解析 looped transformer 与隐藏推理链传闻** — Hacker News：AI 热帖（9/9 22:37）
...
75. **Thomas Wolf 认为 AI 数学尚未被解决，Navier-Stokes 结果更像反例搜索而非完整证明** — X：Thomas Wolf（Hugging Face 联创/CSO） (@Thom_Wolf)（9/9 17:17）
...
76. **Fortune 报道 OpenAI 多次修改 GPT-6 Astra 基准测试数据，部分成绩大幅变化** — IT之家（RSS）（9/6 14:46）
   据 Fortune 报道，OpenAI 自 9 月 3 日发布 GPT-6 Astra 公告以来多次修改评测基准数据：Astra 幻觉率曾从 4.2% 降至 2% 后又改回。
   https://www.ithome.com/0/998/927.htm

77. **OpenAI 详解存储平台 Habitat 如何扩展支撑超 10 亿 ChatGPT 用户（上篇）** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 18:00）
...
78. **GPT-6 Astra操控Blender保姆级教程：三种玩法与踩坑实录** — 公众号：数字生命卡兹克（9/8 08:17）
...
79. **Augment 复盘软件工厂建设：人均规模调整产出增长 4.5 倍** — Augment Code 博客（网页）（9/11 00:00）
...
80. **27岁前Anthropic研究员Jacob Coxon辞职警示AI灭绝风险，作者重读Tim Urban《人工智能革命》谈文明赌局** — 公众号：数字生命卡兹克（9/10 08:08）
...
81. **LlamaIndex 解析 just-in-time Agentic OCR：两遍式文档处理如何平衡成本与精度** — LlamaIndex：产品、工程与评测（9/11 00:00）
...
82. **Tripo 展示 GPT-6 Astra 与 Blender MCP 结合的 3D Vibe Coding 实操工作流** — Tripo（官方 X）（9/10 16:57）
...
83. **Sam Altman 回应与 Anthropic 研究员的 Navier-Stokes 证明发布争议** — X：Sam Altman (@sama)（9/9 02:02）
...
84. **OpenAI 长文阐述对齐与监测困境，称 CoT 监控能力正在减弱** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/6 17:00）
...
85. **GPT-6 Astra爆火后，卡兹克谈执行能力贬值与判断力断层** — 公众号：数字生命卡兹克（9/7 08:08）
...
86. **GPT-6 Astra推理等级怎么选才最省Token** — 公众号：数字生命卡兹克（9/9 08:09）
...
87. **OpenRouter 评测 Seedance 2.5：长镜头与已有素材编辑的场景及成本解析** — OpenRouter：Announcements（RSS）（9/9 08:00）
...
88. **OpenAI 呼吁抓住 AI 政策窗口期，支持强制性国家安全监管与四项加州法案** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/9 21:00）
...
89. **Together AI 深度解析开源模型 AI 编码栈 MIGHT** — Together AI 研究与产品博客（RSS）（9/9 08:00）
...
90. **Anthropic 讲解用 Claude Platform 降低成本并提升性能的三个方法** — X：Claude Devs (@ClaudeDevs)（9/9 01:01）
...
91. **Tom Tunguz 分析 OpenAI 的 3x AI 生产力增益是否只是机器不睡觉** — Tomer Tunguz 博客（VC 分析）（9/8 08:00）
...
92. **Anthropic 发布 Claude Platform 降本指南并更新 claude-api 技能** — Claude：Blog（网页）（9/8 00:00）
...
93. **OpenRouter 教程：用代码调用 Nano Banana 2 编辑图像** — OpenRouter：Announcements（RSS）（9/9 08:00）
...
94. **Tomer Tunguz 解析 AI 消耗的三波浪潮：从聊天到智能体再到 meta-harness** — Tomer Tunguz 博客（VC 分析）（9/7 08:00）
...
95. **Anthropic 发布经济情景模型，推演 AI 对 2030 年美国就业与工资的影响** — Anthropic：The Institute（旗舰研究长文 · 网页）（9/9 21:27）
...
96. **vLLM x AgentX：面向真实世界智能体推理服务的全栈优化** — vLLM 官方博客（RSS）（9/8 08:00）
...
97. **Mistral 用 AI 智能体迁移 4 万行 Fortran 77 到 C++ 的经验复盘** — Mistral AI：News（网页）（9/9 00:00）
