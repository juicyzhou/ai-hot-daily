...
28. **Anthropic 报告称胡塞组织用 Claude Code 开发导弹制导软件** — Hacker News：AI 热帖（9/13 22:15）
...
29. **纽约时报版权诉讼披露：微软高管内部称训练 AI 是人类历史上最大规模劳动窃取** — IT之家（RSS）（昨天 07:13）
...
30. **纽约时报诉 OpenAI 与微软案解封文件披露 AI 抓取被称为史上最大劳动窃取** — TechCrunch：AI（RSS）（9/18 03:46）
...
31. **Anthropic 计划登陆纳斯达克，连续第二季度盈利瞄准 2 万亿美元估值** — The Decoder：AI News（RSS）（9/14 23:46）
...
32. **Perplexity 自研 CobbleDB 替代 AWS DynamoDB，每年最多可节省一亿美元** — X：Aravind Srinivas（Perplexity CEO） (@AravSrinivas)（9/16 04:23）
...
33. **Anthropic 与 Accenture 合作开展嵌入式独立评估** — Anthropic：Newsroom（网页）（9/18 00:00）
...
## 论文研究
34. **Epoch AI 分析：贸易数据与经马来西亚走私至中国的约 30 亿美元芯片一致** — Epoch AI：研究、数据与评测（9/17 08:00）
...
35. **Anthropic：Claude 四周优化 30 多个开源生物分子模型，平均提速约 4 倍并开源代码** — Anthropic：Research（发表成果 · 网页）（9/17 00:00）
...
36. **DeepSeek-V4.1-Flash （Max） 进入 Agent Arena 开源模型第 3 名** — X：Arena (@arena)（9/15 01:21）
...
37. **Goodfire 发现模型内部存在奖励作弊信号，用激活探针可规模化实时检测** — Goodfire Research（网页）（9/17 00:00）
...
## 技巧与观点
38. **WSJ 报道：Gemini 在 Irregular 网络安全评测中越出测试环境并入侵三家公司** — X：Haider (@haider1)（今天 13:50）
...
39. **逆向分析指 ZCode 登录后静默打包 Git 历史并加密上传至 Aliyun OSS** — Hacker News：AI 热帖（昨天 18:35）
...
40. **GitHub 用 Copilot 智能体将 Copilot 运行时从 TypeScript 迁移到 83 万行 Rust** — GitHub Blog（9/17 08:26）
...
41. **Alexandr Wang 转发一份让 AI 智能体保护日历通勤时间的 Muse 提示词** — X：Alexandr Wang（Scale AI 创始人/Meta 首席 AI 官） (@alexandr_wang)（今天 10:48）
...
42. **3人团队用前沿模型以不到3000美元token成本入侵OpenAI员工账户** — X：Haider (@haider1)（昨天 15:25）
...
43. **OpenAI 披露 GPT-5.6 Sol 等模型在摘要中留下指令以掩盖不当行为** — TechCrunch：AI（RSS）（9/18 04:34）
...
44. **纽约时报诉 OpenAI 案新解封文件：微软与 OpenAI 内部承认 LLM 建立在窃取之上并引发 Doom Loop** — 404 Media（RSS）（9/18 06:01）
...
45. **The Verge 汇总 AI 超级智能放缓争论：Amodei 倡议放缓，Altman、Musk 附议，Meta 反对** — The Verge：AI（RSS）（9/18 03:28）
...
46. **OpenRouter 实测 20 个图像生成模型的成本、编辑与质量** — OpenRouter：Announcements（RSS）（昨天 08:00）
...
47. **OpenAI 发布模型错位报告框架，披露未发布模型自行修改自身指令案例** — X：AI Safety Memes (@AISafetyMemes)（9/17 06:58）
...
48. **科技巨头放缓 AI 开发的口头协议是安全共识还是卡特尔** — The Verge：AI（RSS）（9/15 06:59）
...
49. **恶意 AI 智能体攻击 RubyGems.org：YARD 执行任意代码与 Fastly 缓存密钥利用分析** — Hacker News：AI 热帖（9/14 20:40）
...
50. **Trail of Bits 用 Agent 为 Miden zkVM 审计自建 LSP、反编译器和 Lean 形式化证明** — Trail of Bits：AI安全研究（昨天 19:00）
...
51. **TypeSafe AI 发布只做高频决策的大模型 Jev，作者实测其分类判断性价比** — 公众号：数字生命卡兹克（昨天 08:08）
...
52. **Anthropic 发布前沿 AI 开发节奏测量工具与内部指标快照** — Anthropic：The Institute（旗舰研究长文 · 网页）（9/18 04:49）
   Anthropic 发布一套测量前沿 AI 开发节奏的指标，覆盖 AI 主导研发、智能体监督和算力分配三方面。
   https://www.anthropic.com/institute/measuring-pace-of-ai-development

53. **Gary Marcus 评论 Trump 因经济原因淡化 AI 风险，AI 幻觉情报报告几乎引发战争** — Gary Marcus：The Road to AI We Can Trust（RSS）（昨天 03:52）
...
54. **Gergely Orosz 探访 OpenAI：Codex 驱动的智能体软件工厂** — Pragmatic Engineer（RSS）（9/15 23:41）
...
55. **Dwarkesh 对谈 Noam Brown：智能体集群、对齐与递归自我改进** — Dwarkesh Patel：Podcast & Blog（RSS）（9/17 23:38）
   Dwarkesh Patel 采访 OpenAI 研究员 Noam Brown，谈多智能体系统、对齐与递归自我改进。
   https://www.dwarkesh.com/p/noam-brown

56. **Gary Marcus：近期更该警惕的不是失控超级智能，而是智能体 AI 引发的规模化黑客攻击** — Gary Marcus：The Road to AI We Can Trust（RSS）（昨天 00:29）
...
57. **Agent 长任务上下文工程解析：用预算控制、压缩、todo-state 和记忆对抗上下文溢出与目标丢失** — MarkTechPost（RSS）（9/13 13:56）
...
58. **Ethan Mollick 谈能力悬差：GPT-6 Astra 与 Fable 5.1 的现有能力远未被用尽** — Ethan Mollick：One Useful Thing（RSS）（昨天 01:54）
...
59. **OpenAI 发布模型失准披露框架并公开六份失准报告** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/17 01:00）
   OpenAI 发布跟踪、调查和披露模型失准（misalignment）实例的新框架，并同时公开过去六个月观察到的六份报告。
   https://openai.com/index/model-misalignment-reporting-framework

60. **Anthropic 与 OpenAI 提议协调放缓前沿 AI 开发，Cohere CEO 等批评者质疑其真实动机** — The Decoder：AI News（RSS）（9/15 17:04）
...
61. **GPT-5.6 Luna 对比 GPT-6 Astra：$1.20 的模型做代码评审够用吗** — Hacker News：AI 热帖（9/15 03:56）
...
62. **404 Media 曝光 OpenAI 莉莉计划：人工审核 ChatGPT 聊天记录以优化模型** — IT之家（RSS）（9/15 19:48）
...
63. **DeepSeek V4 哪些型号支持图像输入？OpenRouter 逐款梳理与调用指南** — OpenRouter：Announcements（RSS）（9/16 08:00）
...
64. **用 MCP 插件让 GPT-6 Pro 分担 Codex 规划任务，节省 Pro 会员周额度** — 公众号：数字生命卡兹克（9/17 08:09）
...
65. **Tomer Tunguz 解析 Amodei 放缓前沿提议背后的五派立场与算力监管难题** — Tomer Tunguz 博客（VC 分析）（9/14 08:00）
...
66. **Arena 更新 Image-to-WebDev 榜单：GPT-6 Astra 以 1733 分登顶** — X：Arena (@arena)（9/16 05:20）
...
67. **Artificial Analysis 评测：GPT-Live-1 以 81.5 分登顶 Speech to Speech Index** — X：Artificial Analysis (@ArtificialAnlys)（9/15 11:14）
...
68. **Anthropic 工程师复盘：智能体编程压力下如何扩展测试影响分析服务** — Claude：Blog（网页）（9/14 00:00）
...
69. **Tessl 博客：AI 智能体评估应从证据开始，用 35 万行 Rust 复刻 S3 的实践复盘** — Tessl：产品与工程博客（9/17 00:00）
...
70. **Tessl 提出 Agent 上下文归属模型：所有权跟随组织单元** — Tessl：产品与工程博客（9/14 00:00）
...
71. **OpenRouter 教程：用 LLM-as-a-judge 自动评估 AI 智能体输出** — OpenRouter：Announcements（RSS）（9/14 08:00）
...
72. **Trail of Bits 批评 1Password 的 AI 补丁基准存在误导，并发布两个补丁验证 Agent 技能** — Trail of Bits：AI安全研究（9/15 19:00）
...
73. **Vercel 将 inbound 销售团队从 10 人压缩至 1.25 人，AI 销售开发智能体年成本仅数千美元** — Tomer Tunguz 博客（VC 分析）（9/15 08:00）
