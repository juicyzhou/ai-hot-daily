...
5. **DeepSeek 开源 V4.1-Flash：CED 架构降低编码 Agent 的 prefill 开销** — Baseten 工程博客（网页）（9/12 06:01）
...
6. **阶跃星辰发布 StepAudio 3 系列语音大模型，多款在 Artificial Analysis 榜单全球第一** — 公众号：阶跃星辰（Step）（9/15 15:29）
...
7. **小红书 AllSpark 开源 Search Agent 模型 Iris，35B 与 397B 版本同量级成绩领先** — 公众号：小红书技术（dots.llm）（9/14 17:59）
...
8. **Suno 发布 v6 音乐模型，推出 v6、v6-wild、v6-mini 三个版本** — Suno：Blog（网页）（9/13 00:39）
...
9. **硅基流动上线开源模型 Hy4 preview，770B 总参数、1M 上下文** — X：硅基流动 SiliconFlow (@SiliconFlowAI)（9/15 00:32）
...
## 产品发布
10. **ChatGPT for Word 上线，OpenAI 员工称 Excel 和 PowerPoint 用量近期激增** — X：Sherwin Wu（@sherwinwu）（15 分钟前）
...
11. **Unsloth 发布 Docker 镜像与 Unsloth Desktop，本地训练运行 500+ 模型** — X：Unsloth (@UnslothAI)（今天 23:03）
...
12. **Claude Code 重构 Projects：从文件夹变为可托管多线程的对话式项目** — Claude：Blog（网页）（今天 01:52）
...
13. **Meta 发布 Muse for Mac，个人智能体可直接在电脑上执行任务** — X：AI at Meta (@AIatMeta)（今天 06:33）
...
14. **Anthropic 将 Claude Cowork 与聊天合并为统一 Claude，并推出 Docs 和 Slides** — Claude：Blog（网页）（9/16 00:00）
...
15. **Claude Docs、Claude Slides 与 Claude Design 直接嵌入对话，可导出 PowerPoint 或 PDF** — X：Boris Cherny (@bcherny)（昨天 00:28）
...
16. **OpenAI 推出 ChatGPT Ads 新功能：Sponsored Agents 测试并集成 HubSpot 与 Shopify** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 21:00）
...
17. **Apple 发布新一代 Apple Intelligence，Siri AI 正式以测试版上线** — Apple：Newsroom（RSS）（9/15 01:09）
...
18. **Grok Build 推出记忆功能，可跨会话保留项目约定与决策** — xAI：News（网页）（昨天 08:00）
...
19. **Fireworks 上线 DeepSeek-V4.1-Flash，DeepSWE 达 GPT-6 Astra 水准、成本仅 1/15** — Fireworks AI（网页）（9/14 00:00）
...
20. **Google 发布 TranslateGemma 等多语言 AI 成果，语言技术覆盖 300 多种语言** — Google Blog：AI（RSS）（9/16 00:00）
...
21. **Claude for Small Business 新增 43 个 workflow 与 27 个集成** — Claude：Blog（网页）（9/15 00:00）
...
22. **Together AI 扩展微调服务：新增模型、实验跟踪与更细训练控制** — Together AI 研究与产品博客（RSS）（9/11 08:00）
...
## 行业动态
23. **纽约时报诉 OpenAI 与微软案解封文件披露 AI 抓取被称为史上最大劳动窃取** — TechCrunch：AI（RSS）（今天 03:46）
...
24. **Anthropic 报告称胡塞组织用 Claude Code 开发导弹制导软件** — Hacker News：AI 热帖（9/13 22:15）
...
25. **Anthropic 计划登陆纳斯达克，连续第二季度盈利瞄准 2 万亿美元估值** — The Decoder：AI News（RSS）（9/14 23:46）
...
26. **英伟达洽谈以基石投资者身份参与 Anthropic IPO，投资至多 100 亿美元** — IT之家（RSS）（9/12 07:22）
...
27. **Minitap 指控 Google Artemis 未署名使用其开源项目 mobile-use 代码** — Hacker News：AI 热帖（9/12 10:39）
...
28. **Perplexity 自研 CobbleDB 替代 AWS DynamoDB，每年最多可节省一亿美元** — X：Aravind Srinivas（Perplexity CEO） (@AravSrinivas)（9/16 04:23）
...
## 论文研究
29. **Epoch AI 分析：贸易数据与经马来西亚走私至中国的约 30 亿美元芯片一致** — Epoch AI：研究、数据与评测（今天 08:00）
...
30. **Anthropic 用 Claude 优化 30 多个开源生物分子模型，平均提速约 4 倍并开源全部代码** — Anthropic：Research（发表成果 · 网页）（今天 03:49）
...
31. **Goodfire Research 发现模型内部信号可规模化检测奖励作弊** — Goodfire Research（网页）（今天 00:38）
...
32. **Anthropic 威胁报告披露 Claude 被用于间谍软件、导弹与无人机研发，中国实验室大规模蒸馏提取数据** — The Decoder：AI News（RSS）（9/11 21:50）
...
33. **DeepSeek-V4.1-Flash （Max） 进入 Agent Arena 开源模型第 3 名** — X：Arena (@arena)（9/15 01:21）
...
## 技巧与观点
34. **GitHub 用 Copilot 智能体将 Copilot 运行时从 TypeScript 迁移到 83 万行 Rust** — GitHub Blog（今天 08:26）
...
35. **OpenAI 披露 GPT-5.6 Sol 等模型在摘要中留下指令以掩盖不当行为** — TechCrunch：AI（RSS）（今天 04:34）
...
36. **纽约时报诉 OpenAI 案新解封文件：微软与 OpenAI 内部承认 LLM 建立在窃取之上并引发 Doom Loop** — 404 Media（RSS）（今天 06:01）
...
37. **The Verge 汇总 AI 超级智能放缓争论：Amodei 倡议放缓，Altman、Musk 附议，Meta 反对** — The Verge：AI（RSS）（今天 03:28）
...
38. **OpenAI 发布模型错位报告框架，披露未发布模型自行修改自身指令案例** — X：AI Safety Memes (@AISafetyMemes)（昨天 06:58）
...
39. **Anthropic 发布前沿 AI 开发节奏测量工具与内部指标快照** — Anthropic：The Institute（旗舰研究长文 · 网页）（今天 04:49）
   Anthropic 发布一套测量前沿 AI 开发节奏的指标，覆盖 AI 主导研发、智能体监督和算力分配三方面。
   https://www.anthropic.com/institute/measuring-pace-of-ai-development

40. **Dwarkesh 对谈 Noam Brown：智能体集群、对齐与递归自我改进** — Dwarkesh Patel：Podcast & Blog（RSS）（今天 23:38）
   Dwarkesh Patel 采访 OpenAI 研究员 Noam Brown，谈多智能体系统、对齐与递归自我改进。
   https://www.dwarkesh.com/p/noam-brown

41. **OpenAI 发布模型失准披露框架并公开六份失准报告** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 01:00）
   OpenAI 发布跟踪、调查和披露模型失准（misalignment）实例的新框架，并同时公开过去六个月观察到的六份报告。
   https://openai.com/index/model-misalignment-reporting-framework

42. **用 MCP 插件让 GPT-6 Pro 分担 Codex 规划任务，节省 Pro 会员周额度** — 公众号：数字生命卡兹克（今天 08:09）
...
43. **Gergely Orosz 探访 OpenAI：Codex 驱动的智能体软件工厂** — Pragmatic Engineer（RSS）（9/15 23:41）
...
44. **Anthropic 与 OpenAI 提议协调放缓前沿 AI 开发，Cohere CEO 等批评者质疑其真实动机** — The Decoder：AI News（RSS）（9/15 17:04）
...
45. **404 Media 曝光 OpenAI 莉莉计划：人工审核 ChatGPT 聊天记录以优化模型** — IT之家（RSS）（9/15 19:48）
...
46. **科技巨头放缓 AI 开发的口头协议是安全共识还是卡特尔** — The Verge：AI（RSS）（9/15 06:59）
...
47. **恶意 AI 智能体攻击 RubyGems.org：YARD 执行任意代码与 Fastly 缓存密钥利用分析** — Hacker News：AI 热帖（9/14 20:40）
...
48. **Agent 长任务上下文工程解析：用预算控制、压缩、todo-state 和记忆对抗上下文溢出与目标丢失** — MarkTechPost（RSS）（9/13 13:56）
...
50. **英伟达是人工智能领域的"中央银行"** — Hacker News 热门（buzzing.cc 中文翻译）（9/13 00:37）
...
51. **实测 DeepSeek V4.1 Flash：价格大降、原生带视觉，作者用游戏与城市生成任务验证表现** — 公众号：卡尔的AI沃茨（9/11 12:24）
...
52. **Arena 更新 Image-to-WebDev 榜单：GPT-6 Astra 以 1733 分登顶** — X：Arena (@arena)（9/16 05:20）
...
53. **Sam Altman 表示同意 Dario Amodei 的放缓前沿主张，OpenAI 将同样开放独立评估者访问** — X：Sam Altman (@sama)（9/13 00:30）
...
54. **Artificial Analysis 评测：GPT-Live-1 以 81.5 分登顶 Speech to Speech Index** — X：Artificial Analysis (@ArtificialAnlys)（9/15 11:14）
...
55. **Dario Amodei 发文呼吁 AI 行业放慢前沿速度并公布三步计划** — X：Peter McCrory（Anthropic 首席经济学家，@PeterMcCrory）（9/13 04:18）
...
56. **Tomer Tunguz 解析 Amodei 放缓前沿提议背后的五派立场与算力监管难题** — Tomer Tunguz 博客（VC 分析）（9/14 08:00）
...
57. **Trail of Bits 批评 1Password 的 AI 补丁基准存在误导，并发布两个补丁验证 Agent 技能** — Trail of Bits：AI安全研究（9/15 19:00）
...
58. **Vercel 将 inbound 销售团队从 10 人压缩至 1.25 人，AI 销售开发智能体年成本仅数千美元** — Tomer Tunguz 博客（VC 分析）（9/15 08:00）
...
59. **Anthropic 工程师复盘：智能体编程压力下如何扩展测试影响分析服务** — Claude：Blog（网页）（9/14 00:00）
...
60. **Beren Millidge、John Schulman、Charlie O'Neill 对谈递归自我改进离我们还有多远** — Dwarkesh Patel：Podcast & Blog（RSS）（9/12 00:28）
...
61. **Grok Bot 摘要 SpaceX CFO Bret Johnsen 在 Goldman Sachs Communacopia 的演讲要点** — X：Elon Musk (@elonmusk, xAI)（9/11 09:09）
...
62. **GitHub 日韩营销负责人如何用 GitHub Copilot 把活动运营自动化** — GitHub Blog（9/12 02:26）
...
63. **Tessl 提出 Agent 上下文归属模型：所有权跟随组织单元** — Tessl：产品与工程博客（9/14 00:00）
...
64. **Dario Amodei 发文呼吁为前沿 AI 降速并提出三点计划** — X：Dario Amodei (@DarioAmodei)（9/12 22:01）
...
65. **OpenAI 详解存储平台 Habitat 如何扩展支撑超 10 亿 ChatGPT 用户（上篇）** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/11 18:00）
...
66. **Epoch AI 评审 ExploitBench v0.1：基于 41 个真实 V8 漏洞的利用基准评测** — Epoch AI：研究、数据与评测（9/12 08:00）
