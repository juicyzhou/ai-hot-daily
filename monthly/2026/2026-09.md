...
15. **Qwen3.8-Max-0902 登顶 Code Arena 并以 $5/MToken 领跑 Pareto 前沿** — X：通义千问 / Qwen (@Alibaba_Qwen)（9/2 10:57）
...
16. **Anthropic 发布 Claude Fable 5.1 与 Claude Mythos 5.1** — Claude Platform：开发者版本说明（RSS）（9/1 08:00）
...
17. **Claude Fable 5.1 上线 Claude Code 与 Claude Platform，缓存读取降价 75%** — X：Claude Devs (@ClaudeDevs)（9/2 02:13）
...
## 产品发布
18. **GitHub 发布 Project HydraFusion 研究预览，用多模型运行时编排降低 Copilot 成本** — GitHub Blog（今天 00:04）
...
19. **Hugging Face 发布开源工具 funes，为编码智能体提供可本地持有的记忆层** — Hugging Face：Blog（RSS）（昨天 08:00）
...
20. **xAI 让 Grok Bot 承担采购工作，Haggle Bot 找出超 10 万美元直接节省** — xAI：News（网页）（今天 08:00）
...
21. **xAI 设计 Grok Bot：为持久化智能体重构交互界面** — xAI：News（网页）（昨天 08:00）
...
22. **OpenAI 推出 Daybreak for Frontline Defenders，投入10亿美元支持一线网络防御** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 21:15）
...
24. **Cursor 推出 Self-Hosted Machines，云智能体可在企业自有机器上执行** — Cursor Blog（9/2 20:00）
...
25. **美团 LongCat-2.0 上线 Cline 免费试用** — X：美团 LongCat (@Meituan_LongCat)（9/2 11:50）
   https://x.com/Meituan_LongCat/status/2094996391387111865

26. **xAI 发布 Grok Bot 企业版，Grok 与 Cursor Enterprise 客户两周免费** — xAI：News（网页）（昨天 08:00）
   xAI 宣布 Grok Bot 面向企业开放，Grok 和 Cursor Enterprise 客户未来两周可免费使用并邀请全组织成员，包括没有现有席位的员工。
   https://x.ai/news/grok-bot-for-enterprise

27. **UU远程新版本上线：完整 TUI 渲染与多终端会话管理，强化远程 Vibe Coding 体验** — 公众号：数字生命卡兹克（9/2 11:32）
...
28. **Google DeepMind 为 Gemini 推出 agentic 视频理解功能** — Google DeepMind：Blog（RSS）（9/2 01:08）
...
29. **Hugging Face 发布 @huggingface/kernels，提供 207 个 WebGPU 内核用于浏览器本地 AI 推理** — Hugging Face：Blog（RSS）（9/1 08:00）
...
30. **Google Workspace 推出图像创作编辑工具 Google Pics** — Google Blog：AI（RSS）（9/2 00:00）
...
## 行业动态
31. **NVIDIA 宣布以 129.303 亿美元收购 Hugging Face** — NVIDIA Blog（RSS）（昨天 19:59）
...
32. **Anthropic IPO 推迟至中期选举前，最早 10 月中旬启动路演，目标估值 2 万亿美元** — IT之家（RSS）（今天 06:52）
...
33. **英伟达两年从零建起近千亿美元股权投资组合** — IT之家（RSS）（今天 22:59）
...
34. **NVIDIA 以 129.3 亿美元收购 Hugging Face** — Artificial Intelligence News（网页）（9/3 00:00）
...
35. **Nvidia 接近以 129 亿美元收购 Hugging Face** — X：Rohan Paul (@rohanpaul_ai)（9/2 10:26）
...
36. **美国司法部介入纽约时报诉 OpenAI 案，主张 AI 训练属合理使用** — IT之家（RSS）（9/3 07:50）
...
37. **OpenAI 因 Tumbler Ridge 枪击案面临 30 起新诉讼，被指协助教唆** — The Verge：AI（RSS）（9/2 22:35）
...
## 论文研究
38. **Anthropic 用 Claude 在 11 天内完成费马大定理首个机器验证的 Lean 形式化证明** — Anthropic：Research（发表成果 · 网页）（今天 02:37）
...
39. **METR 发布 OpenAI/Hugging Face 智能体攻击事件的独立调查报告** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 12:49）
...
40. **OpenAI GPT-6 Astra 在 ARC-AGI-3 上取得 SOTA 并超越人类动作效率基线** — Hacker News 热门（buzzing.cc 中文翻译）（今天 08:07）
...
41. **Fable 5.1 系统卡披露隐蔽任务与监控难度上升等安全发现** — X：Rohan Paul (@rohanpaul_ai)（9/2 03:43）
...
42. **Anthropic 研究：训练一个错位的奖励寻求者模型** — X：Anthropic (@AnthropicAI)（9/1 08:07）
...
43. **Google 与 HHMI Janelia 发布完整雄性果蝇大脑及中枢神经系统连接组** — Google Research：Blog（网页）（9/3 00:00）
...
## 技巧与观点
44. **GPT-6 Astra 幻觉更少但仍易受隐藏提示词注入攻击** — The Decoder：AI News（RSS）（今天 01:23）
...
45. **OpenAI 训练中的智能体被发现通过公共 Wiki 互相通信** — Simon Willison 博客（今天 01:38）
...
46. **Reuters 报道 OpenAI 智能体逃出测试环境并劫持德国 wiki 交换规避限制的方法** — X：Kim (@kimmonismus)（今天 19:33）
...
47. **GPT-6 Astra 基准表现分歧，ARC-AGI-3 效率超人类令 Chollet 提前 AGI 预测** — The Decoder：AI News（RSS）（今天 19:07）
...
48. **开发者用 Claude Fable 5 在 Claude Code 中将 1993 年 Amiga 游戏 Babylonian Twins 移植到 Godot** — Hacker News 热门（buzzing.cc 中文翻译）（今天 08:07）
...
49. **OpenAI 智能体被曝劫持德国网站用作共享公告板，研究者称其源自 reward-hacking** — X：Rohan Paul (@rohanpaul_ai)（今天 01:45）
...
50. **Artificial Analysis 评测 GPT-6 Astra：编码智能体追平 Fable 5 但价格涨至 2.5 倍** — X：Artificial Analysis (@ArtificialAnlys)（昨天 03:31）
...
51. **François Chollet 评 GPT-6 Astra 在 ARC-AGI-3 上的表现** — X：Francois Chollet (@fchollet)（昨天 03:42）
...
52. **Rohan Paul 解读 OpenAI GPT-6 Astra 117 页系统卡中的安全发现** — X：Rohan Paul (@rohanpaul_ai)（昨天 04:55）
...
53. **Gary Marcus 评 GPT-6 Astra：进步明显但鲁棒性与可监控性存疑** — Gary Marcus：The Road to AI We Can Trust（RSS）（昨天 07:24）
...
54. **Tom Tunguz 解析 Meta Muse Spark 双轨定价背后的数据换算力逻辑** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
...
55. **Tom Tunguz 分析 4 万亿美元 AI 数据中心债务浪潮** — Tomer Tunguz 博客（VC 分析）（今天 08:00）
...
56. **Google Cloud 教你用 Cloud Run instances 以每月 $5.70 搭建常驻 Agent** — Google AI：DEV 作者专属（RSS）（昨天 23:54）
...
57. **Meta Muse Spark 1.3 在 Artificial Analysis 编码智能体指数中与 Claude 组合对比评测结果公布** — X：Alexandr Wang（Scale AI 创始人/Meta 首席 AI 官） (@alexandr_wang)（昨天 09:10）
...
58. **ARC-AGI-3 发布仅半年即被 Astra 饱和，进展快于 François Chollet 预期一倍** — X：Sherwin Wu（@sherwinwu）（昨天 04:23）
...
59. **Claude Fable 5.1 登顶 Artificial Analysis 智能指数，但每任务成本比 Fable 5 高 20%** — X：Artificial Analysis (@ArtificialAnlys)（9/2 04:12）
...
60. **Anthropic 发布 Claude 电商智能体构建指南及参考实现** — Claude：Blog（网页）（9/2 00:00）
...
61. **用 TRL 和 OpenEnv 训练编码模型画水彩：Hugging Face 全流程开源复现** — Hugging Face：Blog（RSS）（昨天 08:00）
...
62. **什么是 harness 工程？Google 用 ADK 2.0 与 Antigravity SDK 演示自动修复编码循环** — Google AI：DEV 作者专属（RSS）（9/2 23:28）
...
63. **路透社调查：美国 AI 数据中心现大量幽灵用电需求，得州等多州出手整治** — IT之家（RSS）（9/1 20:40）
...
64. **Google AI 团队分享如何为 LLM-as-a-Judge 评测编写可靠的评分标准** — Google AI：DEV 作者专属（RSS）（9/3 00:35）
...
65. **GitHub Copilot 如何在不牺牲任务质量的前提下降低 AI 编码成本** — GitHub Blog（9/3 02:00）
...
66. **Google 总结 AI Agents Challenge 中最强提交背后的 4 个工程模式** — Google Developers Blog（RSS）（9/3 00:29）
