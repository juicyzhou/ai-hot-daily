...
> 共 28 条精选 · 统计时段 07-27~08-02（北京时间）
...
3. **MiniMax H3 发布并将开源，主打视觉包装与后期特效** — 公众号：数字生命卡兹克（7/31 10:21）
...
5. **DeepSeek-V4-Flash API公测上线，Agent能力大幅升级** — X：DeepSeek (@deepseek_ai)（7/31 14:56）
...
## 产品发布
6. **OpenRouter 推出 Ori Eval：用你的数据证明哪款模型最适合你的项目** — OpenRouter：Announcements（RSS）（7/31 08:00）
...
7. **Grok 支持分析任意视频** — X：Elon Musk (@elonmusk, xAI)（8/2 14:23）
...
## 行业动态
10. **GLM 5.2 助 Hugging Face 抵御秘密模型攻击** — X：阿易 AI Notes (@AYi_AInotes)（8/1 11:57）
...
11. **国家发改委：将加快《人工智能法》立法进程** — IT之家（RSS）（7/31 10:48）
...
12. **Anthropic 承认三款 Claude 模型逃出测试环境攻击真实系统** — The Decoder：AI News（RSS）（7/31 18:57）
...
15. **欧盟《人工智能法》新增透明度要求，8 月 2 日起正式执行** — IT之家（RSS）（7/31 19:40）
...
## 论文研究
16. **腾讯混元 Hyra 攻克加法组合学 50 年未解难题** — 公众号：腾讯混元（7/31 12:10）
...
17. **Zero-Mem：为 LLM 智能体实现零 token 记忆操作** — HuggingFace Daily Papers（社区热门论文）（7/31 08:00）
...
18. **DiffusionGemma 技术报告：基于离散扩散的高效文本生成模型** — HuggingFace Daily Papers（社区热门论文）（7/31 08:00）
   DiffusionGemma 是一个实验性开源权重语言模型，通过离散扩散并行迭代精炼 256 个 token 的块，而非逐 token 解码，从而避免自回归模型的顺序解码瓶颈。
   https://arxiv.org/abs/2608.00146

19. **小红书 dots 团队发布 VibeLifeBench 与 VibeSearchBench：七个最强模型无一及格** — 公众号：小红书技术（dots.llm）（7/31 18:00）
...
20. **Show HN：将 DeepSeek 整合到 GPT-OSS 中不会带来审查机制** — Hacker News 热门（buzzing.cc 中文翻译）（7/31 12:57）
...
21. **面壁智能ALIGN：自动对齐智能体与环境接口** — X：面壁智能 OpenBMB (@OpenBMB)（7/31 21:00）
...
22. **OpenAI Astra 以约2000美元证明10项数学难题** — X：Greg Brockman (@gdb)（8/1 15:39）
...
## 技巧与观点
23. **Codex 用 Sol 指挥 Luna Max 省额度翻倍产出** — X：阿易 AI Notes (@AYi_AInotes)（8/2 18:47）
...
24. **animated-voiceover 开源：一人干翻动画工作室** — X：阿易 AI Notes (@AYi_AInotes)（8/1 00:01）
...
26. **实测MiniMax H3做后期和动效：视频届的审美王来了** — 公众号：卡尔的AI沃茨（8/2 21:26）
...
27. **Tailscale 未能阻止 Hugging Face 入侵事件复盘** — Hacker News 热门（buzzing.cc 中文翻译）（8/1 04:25）
