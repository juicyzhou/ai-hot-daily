...
> 共 39 条精选 · 统计时段 09-07~09-13（北京时间）
...
6. **DeepSeek 开源 V4.1-Flash：CED 架构降低编码 Agent 的 prefill 开销** — Baseten 工程博客（网页）（9/12 06:01）
...
7. **WorkBuddy 上线 DeepSeek V4.1-Flash，免费试用两周** — X：Tencent WorkBuddy (@WorkBuddy_AI)（9/10 23:49）
...
9. **Suno v6 发布，支持图片、视频和语音备忘录生成音乐** — X：Suno (@suno)（9/10 08:34）
...
14. **OpenRouter 发布 Fusion 复合模型，让多个模型辩论后合成最终答案** — OpenRouter：Announcements（RSS）（9/10 08:00）
...
15. **OpenRouter 推出有状态 Shell 工具 `openrouter：shell` 并上线 Files API** — X：OpenRouter (@OpenRouter)（9/10 22:58）
...
16. **Hugging Face 用 Gradio Workflow 重建 Workflow1111，复刻 AUTOMATIC1111 主要功能** — Hugging Face：Blog（RSS）（9/10 08:00）
...
17. **Together AI 扩展微调服务：新增模型、实验跟踪与更细训练控制** — Together AI 研究与产品博客（RSS）（9/11 08:00）
...
18. **OpenAI 在 ChatGPT Work 中推出 Data agent** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/10 23:00）
   OpenAI 在 ChatGPT Work 中推出新的 Data agent，用户用自然语言即可连接公司数据、分析变化并生成可分享的交互式仪表盘。
   https://openai.com/index/put-data-to-work

## 行业动态
19. **Anthropic 报告称胡塞组织用 Claude Code 开发导弹制导软件** — Hacker News：AI 热帖（9/13 22:15）
...
20. **Shopify 宣布从 React Native 全面迁回 Swift 和 Kotlin 原生开发** — Hacker News 热门（buzzing.cc 中文翻译）（9/10 23:04）
   Shopify 宣布将全部移动应用从 React Native 迁回 Swift 和 Kotlin，判断是 LLM 智能体大幅降低了跨平台重复开发成本这一核心假设被改变。
   https://shopify.engineering/back-to-native

21. **Anthropic 报告指控阿里、月之暗面与 DeepSeek 对 Claude 发起蒸馏攻击** — TechCrunch：AI（RSS）（9/11 04:57）
...
22. **英伟达洽谈以基石投资者身份参与 Anthropic IPO，投资至多 100 亿美元** — IT之家（RSS）（9/12 07:22）
...
23. **Minitap 指控 Google Artemis 未署名使用其开源项目 mobile-use 代码** — Hacker News：AI 热帖（9/12 10:39）
...
## 论文研究
24. **Anthropic 威胁报告披露 Claude 被用于间谍软件、导弹与无人机研发，中国实验室大规模蒸馏提取数据** — The Decoder：AI News（RSS）（9/11 21:50）
...
## 技巧与观点
25. **Agent 长任务上下文工程解析：用预算控制、压缩、todo-state 和记忆对抗上下文溢出与目标丢失** — MarkTechPost（RSS）（9/13 13:56）
...
26. **英伟达是人工智能领域的"中央银行"** — Hacker News 热门（buzzing.cc 中文翻译）（9/13 00:37）
...
27. **实测 DeepSeek V4.1 Flash：价格大降、原生带视觉，作者用游戏与城市生成任务验证表现** — 公众号：卡尔的AI沃茨（9/11 12:24）
...
28. **Sam Altman 表示同意 Dario Amodei 的放缓前沿主张，OpenAI 将同样开放独立评估者访问** — X：Sam Altman (@sama)（9/13 00:30）
...
29. **Dario Amodei 发文呼吁 AI 行业放慢前沿速度并公布三步计划** — X：Peter McCrory（Anthropic 首席经济学家，@PeterMcCrory）（9/13 04:18）
...
30. **Augment 复盘软件工厂建设：人均规模调整产出增长 4.5 倍** — Augment Code 博客（网页）（9/11 00:00）
...
31. **Beren Millidge、John Schulman、Charlie O'Neill 对谈递归自我改进离我们还有多远** — Dwarkesh Patel：Podcast & Blog（RSS）（9/12 00:28）
...
32. **Grok Bot 摘要 SpaceX CFO Bret Johnsen 在 Goldman Sachs Communacopia 的演讲要点** — X：Elon Musk (@elonmusk, xAI)（9/11 09:09）
...
33. **27岁前Anthropic研究员Jacob Coxon辞职警示AI灭绝风险，作者重读Tim Urban《人工智能革命》谈文明赌局** — 公众号：数字生命卡兹克（9/10 08:08）
...
34. **LlamaIndex 解析 just-in-time Agentic OCR：两遍式文档处理如何平衡成本与精度** — LlamaIndex：产品、工程与评测（9/11 00:00）
...
35. **Tripo 展示 GPT-6 Astra 与 Blender MCP 结合的 3D Vibe Coding 实操工作流** — Tripo（官方 X）（9/10 16:57）
...
36. **GitHub 日韩营销负责人如何用 GitHub Copilot 把活动运营自动化** — GitHub Blog（9/12 02:26）
...
37. **Dario Amodei 发文呼吁为前沿 AI 降速并提出三点计划** — X：Dario Amodei (@DarioAmodei)（9/12 22:01）
...
38. **OpenAI 详解存储平台 Habitat 如何扩展支撑超 10 亿 ChatGPT 用户（上篇）** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/11 18:00）
...
39. **Hugging Face 用 LoRA 与 Storage Bucket 在 HF Jobs 上跑异步 GRPO，免 NCCL 并提速 3.9 倍** — Hugging Face：Blog（RSS）（9/10 08:00）
