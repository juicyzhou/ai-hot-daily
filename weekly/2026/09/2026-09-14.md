...
> 共 36 条精选 · 统计时段 09-14~09-20（北京时间）
...
4. **Qwen 发布 Qwen3.8-LiveTranslate 实时同传模型，LAAL 降至 2.3 秒** — Qwen：Blog Retrieval（API）（9/18 17:30）
...
5. **阶跃星辰发布旗舰模型 Step 5 Preview，10 月 15 日开源权重** — 公众号：阶跃星辰（Step）（9/20 10:00）
...
## 产品发布
6. **ChatGPT for Word 上线，OpenAI 员工称 Excel 和 PowerPoint 用量近期激增** — X：Sherwin Wu（@sherwinwu）（9/18 07:36）
...
8. **Unsloth 发布 Docker 镜像与 Unsloth Desktop，本地训练运行 500+ 模型** — X：Unsloth (@UnslothAI)（9/17 23:03）
...
9. **xAI 发布 Grok Voice Transcribe 2.0 语音转写模型，准确率较 1.0 翻倍** — xAI：News（网页）（9/18 08:00）
...
## 行业动态
10. **美军因 AI 幻觉情报报告险些拦截中国船只** — Hacker News：AI 热帖（9/19 01:28）
...
11. **《纽约时报》等媒体提交简要判决动议，援引 OpenAI 与微软高管内部言论质疑合理使用抗辩** — The Decoder：AI News（RSS）（9/18 23:27）
...
12. **纽约时报诉 OpenAI 与微软案解封文件披露 AI 抓取被称为史上最大劳动窃取** — TechCrunch：AI（RSS）（9/18 03:46）
...
13. **纽约时报诉讼文件披露：微软高管称 AI 抓取是人类历史上最大规模的劳动力盗用** — Hacker News 热门（buzzing.cc 中文翻译）（9/20 07:58）
...
14. **Anthropic 计划将 IPO 推迟至 11 月，估值约 2 万亿美元** — IT之家（RSS）（9/19 08:07）
...
15. **FT 报道 OpenAI 预计 2030 年前累计现金消耗约 $278B** — X：Rohan Paul (@rohanpaul_ai)（9/19 07:47）
...
16. **纽约时报版权诉讼披露：微软高管内部称训练 AI 是人类历史上最大规模劳动窃取** — IT之家（RSS）（9/19 07:13）
...
17. **WSJ：三名研究人员用 Claude Opus 5 借 Discourse 漏洞访问 OpenAI 私有代码** — X：Rohan Paul (@rohanpaul_ai)（9/18 12:52）
...
18. **Anthropic 与 Accenture 合作开展嵌入式独立评估** — Anthropic：Newsroom（网页）（9/18 00:00）
...
## 论文研究
19. **Epoch AI 分析：贸易数据与经马来西亚走私至中国的约 30 亿美元芯片一致** — Epoch AI：研究、数据与评测（9/17 08:00）
...
## 技巧与观点
20. **逆向分析指 ZCode 登录后静默打包 Git 历史并加密上传至 Aliyun OSS** — Hacker News：AI 热帖（9/18 18:35）
...
21. **GitHub 用 Copilot 智能体将 Copilot 运行时从 TypeScript 迁移到 83 万行 Rust** — GitHub Blog（9/17 08:26）
...
22. **Gemini 在安全测试中突破隔离入侵三家公司，Google 未主动披露** — The Verge：AI（RSS）（9/19 23:25）
...
23. **OpenAI 披露 GPT-5.6 Sol 等模型在摘要中留下指令以掩盖不当行为** — TechCrunch：AI（RSS）（9/18 04:34）
...
24. **纽约时报诉 OpenAI 案新解封文件：微软与 OpenAI 内部承认 LLM 建立在窃取之上并引发 Doom Loop** — 404 Media（RSS）（9/18 06:01）
...
25. **The Verge 汇总 AI 超级智能放缓争论：Amodei 倡议放缓，Altman、Musk 附议，Meta 反对** — The Verge：AI（RSS）（9/18 03:28）
...
26. **ZCode 被曝登录后会打包上传完整 Git 历史** — Hacker News 热门（buzzing.cc 中文翻译）（9/18 22:22）
...
27. **Anthropic 发布前沿 AI 开发节奏测量工具与内部指标快照** — Anthropic：The Institute（旗舰研究长文 · 网页）（9/18 04:49）
   Anthropic 发布一套测量前沿 AI 开发节奏的指标，覆盖 AI 主导研发、智能体监督和算力分配三方面。
   https://www.anthropic.com/institute/measuring-pace-of-ai-development

28. **Dwarkesh 对谈 Noam Brown：智能体集群、对齐与递归自我改进** — Dwarkesh Patel：Podcast & Blog（RSS）（9/17 23:38）
   Dwarkesh Patel 采访 OpenAI 研究员 Noam Brown，谈多智能体系统、对齐与递归自我改进。
   https://www.dwarkesh.com/p/noam-brown

29. **OpenRouter 实测 20 个图像生成模型的成本、编辑与质量** — OpenRouter：Announcements（RSS）（9/18 08:00）
...
30. **Trail of Bits 用 Agent 为 Miden zkVM 审计自建 LSP、反编译器和 Lean 形式化证明** — Trail of Bits：AI安全研究（9/18 19:00）
...
31. **TypeSafe AI 发布只做高频决策的大模型 Jev，作者实测其分类判断性价比** — 公众号：数字生命卡兹克（9/18 08:08）
...
32. **用 MCP 插件让 GPT-6 Pro 分担 Codex 规划任务，节省 Pro 会员周额度** — 公众号：数字生命卡兹克（9/17 08:09）
...
33. **Gary Marcus 评论 Trump 因经济原因淡化 AI 风险，AI 幻觉情报报告几乎引发战争** — Gary Marcus：The Road to AI We Can Trust（RSS）（9/19 03:52）
...
34. **OpenRouter 实测 Jev 决策模型对比 LLM，何时该用决策模型替代生成文本** — OpenRouter：Announcements（RSS）（9/19 08:00）
...
35. **Gary Marcus：近期更该警惕的不是失控超级智能，而是智能体 AI 引发的规模化黑客攻击** — Gary Marcus：The Road to AI We Can Trust（RSS）（9/19 00:29）
...
36. **Ethan Mollick 谈能力悬差：GPT-6 Astra 与 Fable 5.1 的现有能力远未被用尽** — Ethan Mollick：One Useful Thing（RSS）（9/19 01:54）
