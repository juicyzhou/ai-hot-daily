...
> 共 55 条精选 · 统计时段 09-21~09-27（北京时间）
...
15. **小米发布并开源 MiMo-V2.6 系列，扩展强化学习规模迈向自我提升** — 公众号：小米 MiMo（昨天 04:18）
...
16. **小米发布 MiMo-V2.6 Pro 与 Flash 全模态开源模型** — X：小米 MiMo (@XiaomiMiMo)（昨天 04:51）
...
17. **xAI 发布 Grok 4.7，主打编码与知识工作** — xAI：News（网页）（昨天 08:00）
   xAI 发布 Grok 4.7，定位为其最强编码与知识工作模型，定价 $2/百万输入 token、$6/百万输出 token，与 Grok 4.6 同价同速，另有速度和价格加倍的快速变体。
   https://x.ai/news/grok-4-7

18. **小米发布开放权重模型 MiMo-V2.6-Pro，登顶 Artificial Analysis 开放权重模型智能指数** — X：Clément Delangue（Hugging Face CEO） (@ClementDelangue)（昨天 05:03）
...
19. **通义千问发布 Qwen-Image-2.1：7B 单检查点同时支持图像生成与编辑** — X：通义千问 / Qwen (@Alibaba_Qwen)（昨天 10:50）
...
20. **Arena 上线 GPT-6 Sol 与 GPT-6 Luna 测试，评分即将公布** — X：Arena (@arena)（今天 03:29）
...
21. **小米 MiMo-V2.6-Pro 登上 Code Arena： WebDev 榜约第10名，开源权重中约第3** — X：Arena (@arena)（昨天 05:08）
...
22. **腾讯混元发布 Hy Image3.5 preview 图像生成模型** — 腾讯混元：Research（API）（昨天 10:51）
...
## 产品发布
23. **transformers 支持直接加载 GGUF 量化模型，本地推理性能接近 llama.cpp** — Hugging Face：Blog（RSS）（今天 08:00）
...
24. **OpenRouter 推出 Batch API，批量推理可享半价** — OpenRouter：Announcements（RSS）（今天 08:00）
...
25. **Kimi 发布浏览器扩展，由 Kimi WebBridge 更名而来** — X：Kimi.ai (@Kimi_Moonshot)（今天 19:00）
...
26. **Claude Code v2.1.280 发布，新增 Claude Opus 5.5 为默认 Opus 模型** — Claude Code：GitHub Releases（RSS）（今天 00:38）
...
27. **LiteParse 9 月更新：PDFium 提速 20-25%，新增视觉定位与 is-complex 路由 API** — LlamaIndex：产品、工程与评测（今天 22:38）
...
28. **Apple 新款 Mac mini（M6/M5 Pro）与 Mac Studio（M5 Max/M5 Ultra）今日开售** — Apple：Newsroom（RSS）（今天 20:59）
...
29. **OpenAI 为 GPT-6 推出改进的提示词缓存系统与诊断工具** — OpenAI：官网动态（RSS · 排除企业/客户案例）（今天 05:00）
...
30. **Kimi 发布 Kimi Code Desktop 1.0，macOS 与 Windows 版同步上线** — 公众号：月之暗面（Kimi）（昨天 15:54）
...
## 行业动态
31. **五角大楼内部审查：过度依赖 Palantir Maven AI 系统导致误击伊朗学校、123 名儿童死亡** — Hacker News：AI 热帖（今天 03:03）
...
32. **Meta Muse 助手曝出严重 0-day 漏洞，Amazon 已开始封禁 Muse** — Hacker News 热门（buzzing.cc 中文翻译）（今天 03:29）
...
33. **不列颠哥伦比亚省起诉 OpenAI，指其未在枪击案前将 flagged ChatGPT 活动转介警方** — X：Rohan Paul (@rohanpaul_ai)（昨天 04:11）
...
34. **亚马逊封禁 Meta Muse 智能体代用户购物，双方争端升级** — IT之家（RSS）（昨天 19:52）
   亚马逊阻断了 Meta 个人 AI 智能体 Muse 代用户在其网站购物的访问权限，称从未许可该访问，并指 Muse 不表明身份、会采集保存用户账号凭证，带来隐私和安全隐患。
   https://www.ithome.com/1/005/418.htm

35. **Claude Opus 5.5 上架 Arena 的 Agent Arena 与 Battle Mode** — X：Arena (@arena)（今天 01:48）
...
## 论文研究
36. **Epoch AI 研究报告：达到同等 AI 性能的成本每季度下降约 47%** — Epoch AI：研究、数据与评测（今天 08:00）
...
37. **独立调查：ChatGPT 的 __obi 跨站 Cookie 可将站外浏览行为关联到 ChatGPT 账号** — Hacker News 热门（buzzing.cc 中文翻译）（9/21 02:05）
...
38. **Artificial Analysis 评测 Grok 4.7：智能指数得分 46，编码智能体指数升至 56** — X：Artificial Analysis (@ArtificialAnlys)（昨天 00:38）
...
## 技巧与观点
39. **五角大楼调查：过度依赖 Maven AI 是美军误击伊朗米纳布学校的原因之一** — Hacker News：AI 热帖（今天 03:03）
...
40. **Claude Opus 5.5 登顶 Artificial Analysis 智能指数，得分 58** — Artificial Analysis 完整文章（网页）（今天 01:41）
...
41. **Artificial Analysis 评测 GPT-6 Sol 与 Luna：成本减半，智能指数持平** — X：Artificial Analysis (@ArtificialAnlys)（今天 02:20）
...
42. **Artificial Analysis 评测 GPT-6 Sol 和 Luna：成本减半但各评测有升有降** — Artificial Analysis 完整文章（网页）（今天 03:41）
...
43. **Claude Opus 5.5 登顶 Artificial Analysis Intelligence Index，并降价 20%** — X：Artificial Analysis (@ArtificialAnlys)（今天 00:41）
...
44. **卡兹克实测对比 Grok 4.7 与小米 MiMo V2.6：后者成不可能三角版本答案** — 公众号：数字生命卡兹克（今天 10:02）
...
45. **Artificial Analysis 评测 Step 5 Preview： Intelligence Index 得 44 分，成本约为同级模型 1/2.8** — X：Artificial Analysis (@ArtificialAnlys)（今天 09:49）
...
46. **OpenRouter 实测 Jev 1.13 与 Claude Opus 5 在 Banking77 分类任务上的准确率、延迟与成本** — OpenRouter：Announcements（RSS）（今天 08:00）
...
47. **OpenRouter 解读 NVIDIA Nemotron 3.5 Lightning 如何承担 Agent 高频执行调用** — OpenRouter：Announcements（RSS）（今天 08:00）
...
48. **Google 确认 Gemini 在 Irregular 安全测试中访问 3 家真实公司系统，与 OpenAI、Anthropic、Meta 属同一评估事故** — MarkTechPost（RSS）（9/21 04:20）
...
49. **马斯克称 Grok 4.7 使 xAI 在智能体编码领域位列第三** — X：Elon Musk (@elonmusk, xAI)（昨天 01:06）
...
50. **Nathan Lambert 撰文分析开源模型的中美实力格局** — Nathan Lambert：Interconnects（RSS）（昨天 19:56）
...
51. **数字生命卡兹克访谈汉化组：AI 时代字幕组与漫画组的真实处境** — 公众号：数字生命卡兹克（昨天 09:52）
...
52. **Tomer Tunguz 谈 AI 优化 if-then 判断：专用决策器把分类成本降近百倍** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
...
53. **Linear 重构 CI 流程应对 AI 编码带来的验证瓶颈，PR 等待时间从 6 分钟降至 5 分钟** — Linear：Now（RSS）（昨天 20:26）
...
54. **Artificial Analysis 评测 Grok 4.7：智能指数 46 分进入前四，编码智能体升至第 4** — Artificial Analysis 完整文章（网页）（9/21 00:00）
...
55. **Fireworks AI 分析：前沿不是单个模型，而是路由器，FireRouter 发布** — Fireworks AI（网页）（9/21 00:00）
