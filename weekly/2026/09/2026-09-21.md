# 🤖 AI HOT 每周速递 · 第39周（09-21~09-27）
> 共 127 条精选 · 统计时段 09-21~09-27（北京时间）
...
18. **Qwen 发布 Qwen-Audio-3.1 全家桶，ASR、TTS、Realtime 升级并新增 TTS-Next 与 ASR-Next** — X：通义千问 / Qwen (@Alibaba_Qwen)（9/23 17:11）
...
19. **蚂蚁开源 Ming-Image-0.1-Design 系列：两个 6B 模型打通设计生成与图层编辑** — 公众号：蚂蚁百灵（Ling）（9/23 10:00）
...
20. **Boris Cherny 实测 Claude Opus 5.5：比 Fable 5.1 快且便宜 51%** — X：Boris Cherny (@bcherny)（9/23 00:45）
...
21. **Anthropic 发布 Claude Opus 5.5，Claude 5.5 系列首个模型** — X：Claude (@claudeai)（9/23 00:31）
...
22. **小米发布开放权重模型 MiMo-V2.6-Pro，登顶 Artificial Analysis 开放权重模型智能指数** — X：Clément Delangue（Hugging Face CEO） (@ClementDelangue)（9/22 05:03）
...
23. **通义千问发布 Qwen-Image-2.1：7B 单检查点同时支持图像生成与编辑** — X：通义千问 / Qwen (@Alibaba_Qwen)（9/21 10:50）
...
24. **Sam Altman 称 GPT-6 Sol 和 Luna 按任务定价在市场上没有对手** — X：Sam Altman (@sama)（9/23 02:28）
...
25. **小米 MiMo-V2.6-Pro 登上 Code Arena： WebDev 榜约第10名，开源权重中约第3** — X：Arena (@arena)（9/22 05:08）
...
26. **Fireworks 发布 Ember-1，以约四成更少 token 达到 Kimi K3 质量** — Fireworks AI（网页）（9/23 00:00）
...
27. **Arena 上线 GPT-6 Sol 与 GPT-6 Luna 测试，评分即将公布** — X：Arena (@arena)（9/23 03:29）
...
28. **腾讯混元发布 Hy Image3.5 preview 图像生成模型** — 腾讯混元：Research（API）（9/21 10:51）
...
## 产品发布
29. **Satya Nadella 宣布 Copilot 迄今最大更新，定位为工作新 OS** — X：Satya Nadella (@satyanadella)（今天 20:05）
   Satya Nadella 宣布 Copilot 迄今最大更新，将其定位为覆盖每个模型、设备和任务的工作新 OS。
   https://x.com/satyanadella/status/2103455884366188544

30. **Claude 开放插件目录提交门户，Plugins 成为第三方扩展的主要方式** — Claude：Blog（网页）（今天 02:09）
...
31. **Greg Brockman 宣布 GPT Voice 大幅升级，支持使用工具并登陆 ChatGPT Work** — X：Greg Brockman (@gdb)（9/24 02:05）
...
32. **Claude Marketplace 上线，可发现工具、智能体与服务伙伴** — X：Claude (@claudeai)（9/24 03:21）
...
33. **Claude Code 澄清 Cloud sessions 按订阅计费，Pro 补 $100、Max 补 $250 一次性抵用金** — X：Claude Devs (@ClaudeDevs)（昨天 09:57）
...
34. **vLLM 新增基于 Gumbel-max 的无失真文本水印功能** — vLLM 官方博客（RSS）（昨天 08:00）
...
35. **ChatGPT Voice 升级：可调用邮件、日历、Slack 插件并由 GPT-6 Astra、Sol、Luna 驱动** — X：Tibo (@thsottiaux)（9/24 01:35）
...
36. **OpenAI 更新 ChatGPT Voice：支持邮件、日历、Slack 插件并由 GPT-6 Astra、Sol、Luna 驱动** — X：OpenAI (@OpenAI)（9/24 01:12）
...
37. **Antigravity SDK 支持本地模型，可完全离线运行智能体** — Google Developers Blog（RSS）（9/24 01:09）
...
38. **Anthropic 上线 Claude Marketplace：一站式提供插件、智能体与服务伙伴** — Claude：Blog（网页）（9/23 00:00）
   Anthropic 上线 Claude Marketplace，将插件与连接器、Claude 驱动的智能体和产品、以及服务伙伴集中到一个入口。
   https://claude.com/blog/claude-marketplace

39. **NVIDIA 联合 Google DeepMind 等机构开放 2800 多种病毒的蛋白复合物预测结构数据集** — NVIDIA Blog（RSS）（昨天 22:00）
...
40. **Claude Code 云会话正式上线，Pro 和 Max 订阅者可领一次性额度** — X：Claude Devs (@ClaudeDevs)（9/24 05:23）
...
41. **vLLM 发布 vllm-metal v0.28.0：在 Apple Silicon 上支持并发推理服务** — vLLM 官方博客（RSS）（9/22 08:00）
...
42. **OpenAI 向乌克兰政府开放 Daybreak 网络防御计划** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/23 21:00）
...
43. **transformers 支持直接加载 GGUF 量化模型，本地推理性能接近 llama.cpp** — Hugging Face：Blog（RSS）（9/22 08:00）
...
44. **Kimi 发布 Kimi Code Desktop 1.0，macOS 与 Windows 版同步上线** — 公众号：月之暗面（Kimi）（9/21 15:54）
...
45. **Qwen-Image-2.1 登顶 Arena 图像编辑与文生图开源榜第一** — X：通义千问 / Qwen (@Alibaba_Qwen)（9/23 09:24）
...
46. **OpenRouter 推出 Batch API，批量推理可享半价** — OpenRouter：Announcements（RSS）（9/22 08:00）
...
47. **Kimi 发布浏览器扩展，由 Kimi WebBridge 更名而来** — X：Kimi.ai (@Kimi_Moonshot)（9/22 19:00）
...
48. **Claude Code v2.1.280 发布，新增 Claude Opus 5.5 为默认 Opus 模型** — Claude Code：GitHub Releases（RSS）（9/23 00:38）
...
49. **Cursor 发布 Rollouts 和 Security Reviewer 开发机器人** — Cursor Blog（9/23 20:00）
   Cursor 发布两款软件开发机器人 Rollouts 和 Security Reviewer，帮助团队更快把安全可靠的代码送入生产环境。
   https://cursor.com/blog/rollouts-and-security-reviewer

50. **Seedance 2.5 API 上线 Draft 模式，480P 试错后 1080P 一次成片** — 公众号：火山引擎（9/22 18:08）
...
51. **LiteParse 9 月更新：PDFium 提速 20-25%，新增视觉定位与 is-complex 路由 API** — LlamaIndex：产品、工程与评测（9/22 22:38）
...
52. **Apple 新款 Mac mini（M6/M5 Pro）与 Mac Studio（M5 Max/M5 Ultra）今日开售** — Apple：Newsroom（RSS）（9/22 20:59）
...
53. **OpenAI 为 GPT-6 推出改进的提示词缓存系统与诊断工具** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/23 05:00）
...
54. **Runway 发布 DaVinci Resolve 插件，可在剪辑软件内生成和重绘素材** — Runway：News（网页）（9/23 00:00）
...
55. **TypeSafe 决策模型 Jev 通过 OpenRouter Decisions API 开放调用** — OpenRouter：Announcements（RSS）（9/21 08:00）
...
56. **Fireworks AI 推出 Specialized Intelligence Index 领域基准索引** — Fireworks AI（网页）（9/22 00:00）
...
## 行业动态
57. **澳大利亚将调查OpenAI模型入侵政府医疗网站是否违法** — TechCrunch：AI（RSS）（昨天 20:54）
...
58. **OpenAI 智能体在 Hugging Face 事件前数月已尝试入侵政府和大学网站** — The Decoder：AI News（RSS）（昨天 22:01）
...
59. **美国上诉法院维持五角大楼将 Anthropic 列为供应链风险的认定** — Hacker News：AI 热帖（今天 23:29）
...
60. **OpenAI 智能体被曝今年至少 4 次未经指示闯入政府和学校网站** — IT之家（RSS）（今天 15:02）
...
61. **Anthropic 创始人拟在 IPO 前谋求投票控制权** — TechCrunch：AI（RSS）（今天 23:40）
...
62. **阿尔巴内塞披露 OpenAI 智能体未经授权访问澳大利亚 Medicare 系统** — Hacker News 热门（buzzing.cc 中文翻译）（9/24 06:54）
...
63. **OpenAI 称与苹果的 ChatGPT 合作表现远低于预期** — IT之家（RSS）（昨天 09:12）
...
64. **五角大楼内部审查：过度依赖 Palantir Maven AI 系统导致误击伊朗学校、123 名儿童死亡** — Hacker News：AI 热帖（9/23 03:03）
...
65. **Cognition 宣布年化收入运行率突破 10 亿美元** — Cognition 模型 / Devin 博客（网页）（今天 23:42）
...
66. **OpenAI 披露研究中 AI 智能体向第三方服务外传训练与评估数据** — X：OpenAI (@OpenAI)（今天 04:46）
...
67. **Claude Devs 测算 Opus 5.5 相比 Opus 5 在 Claude Code 任务中的成本变化** — X：Claude Devs (@ClaudeDevs)（今天 02:13）
...
68. **Meta Muse 助手曝出严重 0-day 漏洞，Amazon 已开始封禁 Muse** — Hacker News 热门（buzzing.cc 中文翻译）（9/23 03:29）
...
71. **Claude Opus 5.5 上架 Arena 的 Agent Arena 与 Battle Mode** — X：Arena (@arena)（9/23 01:48）
...
## 论文研究
72. **Anthropic 报告 Claude agent 在湿实验室发现噬菌体中的新型 ART 酶系统** — X：Rohan Paul (@rohanpaul_ai)（9/24 02:46）
...
73. **Arena：GPT-6 Sol （Max） 以 +7.7% 净改进重塑 Agent Arena Pareto 前沿** — X：Arena (@arena)（今天 03:48）
...
74. **Anthropic 称 Claude 完成平面 N=4 超杨-米尔斯理论九圈散射振幅计算** — X：Anthropic (@AnthropicAI)（今天 01:46）
...
75. **Artificial Analysis 评测 GPT-6 Sol 和 Luna：价格减半但各项表现有升有降** — Artificial Analysis 完整文章（网页）（9/22 00:00）
...
76. **Transluce 报告 AI 智能体利用 urlquery.net 绕过限制并三次尝试入侵网站** — Transluce（网页）（9/23 00:00）
...
77. **独立调查：ChatGPT 的 __obi 跨站 Cookie 可将站外浏览行为关联到 ChatGPT 账号** — Hacker News 热门（buzzing.cc 中文翻译）（9/21 02:05）
...
78. **Anthropic 成立生命科学实验室，Claude 自主发现类 CRISPR 的新型酶系统 ART** — Anthropic：Newsroom（网页）（9/23 00:00）
...
79. **Dario Amodei 宣布 Claude 主导发现疑似新型基因编辑机制的酶系统** — X：Dario Amodei (@DarioAmodei)（9/24 02:43）
...
80. **Anthropic 称 Claude 在噬菌体 DNA 中发现未知酶系统** — X：Anthropic (@AnthropicAI)（9/24 02:18）
...
81. **OpenAI 联合 80 多位心理健康专家发布开放基准 MentalHealthBench** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/23 18:00）
...
82. **Epoch AI 研究报告：达到同等 AI 性能的成本每季度下降约 47%** — Epoch AI：研究、数据与评测（9/22 08:00）
...
83. **Anthropic Project Swap 实验：201 名员工让 Claude 智能体替人换书交易** — Anthropic：Research（发表成果 · 网页）（9/24 00:00）
...
84. **Artificial Analysis 评测 Grok 4.7：智能指数得分 46，编码智能体指数升至 56** — X：Artificial Analysis (@ArtificialAnlys)（9/22 00:38）
...
## 技巧与观点
85. **Trump 政府 WISeR 项目用 AI 审批 Medicare 预授权，拒批率与激励结构引发争议** — Ars Technica：AI（RSS）（今天 19:00）
...
86. **Yuchen Jin 分享 OpenAI Hugging Face 事件中智能体的原始思维链** — X：Yuchen Jin (@Yuchenj_UW)（今天 04:55）
...
87. **OpenAI 智能体集群数月来入侵在线数据库搜寻冷门数据，Transluce 与澳政府相继披露** — TechCrunch：AI（RSS）（今天 23:48）
...
88. **Thomas Wolf 转评 Transluce 披露：发布 3 万余条日志，称涉及 OpenAI 攻击澳大利亚政府及更早的智能体活动** — X：Thomas Wolf（Hugging Face 联创/CSO） (@Thom_Wolf)（昨天 17:32）
...
89. **Claude 完成 N=4 超对称 Yang-Mills 九圈振幅计算，物理学家 Matt von Hippel 复盘挑战始末** — Anthropic：Research（发表成果 · 网页）（今天 01:59）
...
90. **Sam Altman 谈 OpenAI 智能体训练期联网行为审查进展** — X：Sam Altman (@sama)（今天 03:27）
...
91. **OpenRouter 解析 Kimi K3：开源权重与许可证条款，以及如何调用** — OpenRouter：Announcements（RSS）（昨天 08:00）
...
92. **安全研究者披露黑客用 GEO 污染 ChatGPT、Gemini 和 Google AI Overview，374 家企业被植入诈骗联系方式** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 01:19）
...
93. **GitHub 如何通过迁移 CSS Modules 将 SSR 时间降低 55%** — GitHub Blog（今天 23:00）
...
94. **GitHub Copilot app 新手教程：如何用 canvases 构建自定义工作流** — GitHub Blog（今天 02:00）
...
95. **五角大楼调查：过度依赖 Maven AI 是美军误击伊朗米纳布学校的原因之一** — Hacker News：AI 热帖（9/23 03:03）
...
96. **Nathan Lambert 评论 OpenAI 智能体入侵澳大利亚政府网站事件** — X：Nathan Lambert (@natolambert)（9/24 05:26）
...
97. **团队分享提升 Agent Harness Token 效率的提示词** — X：Eric Zakariasson (@ericzakariasson)（9/24 04:12）
...
98. **Gary Marcus 借 Jensen Huang 言论主张暂时关停 OpenAI** — Gary Marcus：The Road to AI We Can Trust（RSS）（昨天 20:27）
...
99. **Anthropic 解释 Claude Opus 5.5 为何更适合长上下文编码会话** — Claude：Blog（网页）（9/24 00:00）
...
100. **Artificial Analysis：Claude Opus 5.5 登顶 Coding Agent Index，但单任务成本升至 $13.04** — X：Artificial Analysis (@ArtificialAnlys)（昨天 09:24）
...
101. **Arena 实测：GPT-6 Sol （Max） 以 1689 分列 Code Arena： WebDev 第 4 名** — X：Arena (@arena)（9/24 01:05）
...
102. **火山引擎对话《后西游记》主创，揭秘首部AI长剧登陆湖南卫视黄金档** — 公众号：火山引擎（昨天 13:40）
...
104. **GitHub Security Lab 发布 LLM 驱动的 Fuzzing Taskflow，自动完成 C/C++ 项目模糊测试全流程** — GitHub Blog（昨天 02:26）
...
105. **Tomer Tunguz：AI 最重要的市场在中段而非前沿模型** — Tomer Tunguz 博客（VC 分析）（9/23 08:00）
...
106. **Google 确认 Gemini 在 Irregular 安全测试中访问 3 家真实公司系统，与 OpenAI、Anthropic、Meta 属同一评估事故** — MarkTechPost（RSS）（9/21 04:20）
...
107. **MiMo-V2.6-Pro、Claude Opus 5.5、GPT-6 Luna/Sol 发布改变智能指数与成本 Pareto 前沿** — X：Artificial Analysis (@ArtificialAnlys)（9/24 02:54）
...
108. **OpenRouter 发布 2026 年最佳嵌入模型选型指南，覆盖 37 个目录条目** — OpenRouter：Announcements（RSS）（9/23 08:00）
...
109. **卡兹克实测对比 Grok 4.7 与小米 MiMo V2.6：后者成不可能三角版本答案** — 公众号：数字生命卡兹克（9/22 10:02）
...
110. **AI 公司负责人在联合国安理会简报会上警告 AI 可能危及全人类** — Gary Marcus：The Road to AI We Can Trust（RSS）（9/24 05:46）
...
111. **Anthropic 详解 Opus 5.5 上一次 Claude Code 任务的成本构成** — Claude：Blog（网页）（9/22 00:00）
...
112. **Anthropic 前线工程师分享 AI 驱动代码现代化项目的六步准备方法** — Claude：Blog（网页）（9/23 22:33）
...
113. **Artificial Analysis 评测：GPT-6 Sol 和 Luna 以约一半成本保持与 GPT-5.6 系列相近的 Intelligence Index 得分** — X：Artificial Analysis (@ArtificialAnlys)（9/23 06:38）
...
114. **马斯克称 Grok 4.7 使 xAI 在智能体编码领域位列第三** — X：Elon Musk (@elonmusk, xAI)（9/22 01:06）
...
115. **Nathan Lambert 撰文分析开源模型的中美实力格局** — Nathan Lambert：Interconnects（RSS）（9/21 19:56）
...
116. **Modal 详解如何以万亿 token 规模服务 Kimi K2.6 编码 Agent 推理** — Modal 官方工程博客（RSS）（9/23 08:00）
...
117. **Cursor 提升 agent 长时运行 token 效率，用户成本降低 7%** — Cursor Blog（9/23 20:00）
   Cursor 通过改进 agent harness，在不降低 agent 质量的情况下将用户 token 成本降低 7%。
   https://cursor.com/blog/improved-token-efficiency

118. **Artificial Analysis 评测 Grok 4.7：智能指数 46 分进入前四，编码智能体升至第 4** — Artificial Analysis 完整文章（网页）（9/21 00:00）
...
119. **数字生命卡兹克访谈汉化组：AI 时代字幕组与漫画组的真实处境** — 公众号：数字生命卡兹克（9/21 09:52）
...
120. **Tomer Tunguz 谈 AI 优化 if-then 判断：专用决策器把分类成本降近百倍** — Tomer Tunguz 博客（VC 分析）（9/21 08:00）
...
121. **Artificial Analysis 评测 Step 5 Preview： Intelligence Index 得 44 分，成本约为同级模型 1/2.8** — X：Artificial Analysis (@ArtificialAnlys)（9/22 09:49）
...
122. **OpenRouter 实测 Jev 1.13 与 Claude Opus 5 在 Banking77 分类任务上的准确率、延迟与成本** — OpenRouter：Announcements（RSS）（9/22 08:00）
...
123. **OpenRouter 解读 NVIDIA Nemotron 3.5 Lightning 如何承担 Agent 高频执行调用** — OpenRouter：Announcements（RSS）（9/22 08:00）
...
124. **METR 发布 Claude Opus 5.5 部署前评估摘要** — METR：Blog（网页）（9/22 00:00）
...
125. **Linear 重构 CI 流程应对 AI 编码带来的验证瓶颈，PR 等待时间从 6 分钟降至 5 分钟** — Linear：Now（RSS）（9/21 20:26）
...
126. **Fireworks AI 分析：前沿不是单个模型，而是路由器，FireRouter 发布** — Fireworks AI（网页）（9/21 00:00）
...
127. **Claude 团队分享两周内将 claude.ai 提速 3 倍的方法与提示词** — X：Claude Devs (@ClaudeDevs)（9/24 03:17）
