...
> 共 106 条精选 · 统计时段 08-03~08-09（北京时间）
...
2. **字节 Seed 发布 SeedRealtime 音视频全双工大模型，走向全模态自然交互** — 字节 Seed：Research Feed（网页内嵌数据）（8/5 00:00）
...
3. **蚂蚁百灵开源 Ling-3.0-flash：124B 总参数 MoE 模型，支持 API、单机与高性能三种部署** — 公众号：蚂蚁百灵（Ling）（8/7 20:02）
...
4. **谷歌推出 WeatherNext 气旋模型，AI 高精度预报飓风平均提前 24 小时** — IT之家（RSS）（8/7 14:11）
...
5. **NVIDIA Cosmos 3：开放世界模型如何推动物理 AI 前沿** — NVIDIA Blog（RSS）（8/6 21:00）
...
6. **Qwen-Image-3.0-Pro 上线 Qwen Cloud** — X：通义千问 / Qwen (@Alibaba_Qwen)（8/5 10:40）
...
7. **NVIDIA Alpamayo 2 Super 开放商用，面向 Robotaxi 与自动驾驶的前沿开源模型** — NVIDIA Blog（RSS）（8/4 23:00）
...
8. **商汤 SenseNova U1 开源：统一推理与图像生成** — X：商汤 SenseTime (@SenseTime_AI)（8/4 23:46）
...
9. **OpenRouter 上线 FLUX 3 Video 统一多模态模型** — X：OpenRouter (@OpenRouter)（8/5 01:54）
...
10. **腾讯混元发布 Hy ASR 3.0 preview：真正懂上下文的语音识别** — 公众号：腾讯混元（8/4 16:12）
...
11. **ChatGPT 推出改进版 GPT-5.6 Sol，并扩大免费用户访问权限** — OpenAI：官网动态（RSS · 排除企业/客户案例）（8/6 18:00）
   ChatGPT 推出改进版 GPT-5.6 Sol，提升准确性与一致性，同时扩大免费用户访问权限。免费用户还可无限次使用 GPT-5.6 Luna 进行日常对话。
   https://openai.com/index/improving-gpt-5-6-sol-in-chatgpt

## 产品发布
12. **苹果 Mac 简体中文支持文档更新，"Apple 智能"阿里千问扩展现身** — IT之家（RSS）（8/8 16:25）
...
13. **Seedance 2.5 API上线，视频生成开启「电影级长叙事」** — 公众号：火山引擎（8/7 14:01）
...
14. **Qwen-Image-3.0 发布：高分辨率低至 0.03 美元** — X：阿里云 / Alibaba Cloud (@alibaba_cloud)（8/6 14:12）
...
15. **Swiftlet：在 Mac 上运行 80B 版 Qwen（内存 4.3 GB），在 iPhone 上运行 35B 版** — Hacker News 热门（buzzing.cc 中文翻译）（8/4 14:32）
...
16. **LLM 0.32 发布：新增推理轨迹、OpenAI Responses、服务端工具与更智能的日志** — Simon Willison 博客（8/5 07:58）
...
17. **Reflex 开源 XY：基于 Rust 的超快 Python 绘图库，可保持 1 亿点图表交互流畅** — MarkTechPost（RSS）（8/4 16:57）
...
18. **OpenAI 桌面端 ChatGPT 上线语音交互功能，可语音操控电脑执行多步骤任务** — IT之家（RSS）（8/9 06:46）
...
19. **面壁智能开源 ForgeStencil：一周自动优化 100+ 工业与科学软件，全程零人工介入** — 公众号：面壁智能（MiniCPM）（8/4 12:20）
...
20. **Kitesurf：一款在 V8 隔离环境中运行的"代理优先"浏览器** — Hacker News 热门（buzzing.cc 中文翻译）（8/8 02:54）
   Cloudflare 推出 Kitesurf，一款专为 AI 智能体设计的浏览器，完全运行在 Workers 上，基于 V8 隔离环境，现已在 Browser Run 中免费开放测试。
   https://blog.cloudflare.com/kitesurf

21. **HPC-Ops × SGLang：腾讯混元开源高性能 Attention、Router GEMM 与 MoE 算子** — LMSYS：Blog（Chatbot Arena 团队）（8/7 00:00）
...
22. **Runway 上线 Seedance 2.5，支持 50 个角色参考** — X：Runway (@runwayml)（8/7 19:08）
...
23. **Agent Plugins 1.0.0 发布：谷歌、亚马逊、微软等支持的统一智能体插件规范** — Google Developers Blog（RSS）（8/7 00:53）
...
24. **OpenAI 优化 ChatGPT 中的 GPT-5.6 Sol，并扩大免费用户对 GPT-5.6 Luna 的访问权限** — Hacker News 热门（buzzing.cc 中文翻译）（8/7 07:01）
...
25. **Claude Code 会话间可互发消息** — X：Claude Devs (@ClaudeDevs)（8/8 03:55）
...
30. **谷歌地图 Ask Maps 智能体升级：可对话订餐、找酒店并接入 Gemini Personal Intelligence** — IT之家（RSS）（8/6 20:49）
...
31. **Prime Agent：一个具有自我改进能力的RLM代理** — Hacker News 热门（buzzing.cc 中文翻译）（8/6 09:27）
...
32. **Replit 环境智能：免提示词自动生成设计** — X：Replit (@Replit)（8/5 06:00）
   你无需提示词，也无需设计语言。
...
33. **千问功能上新：推出思考研究、定时任务、办公助理、语音通话等多项新功能，并支持 Qwen3.8-MAX** — 公众号：千问APP（阿里）（8/7 10:04）
...
34. **Anthropic 更新 Claude Fable 5 生物安全防护，误报率大幅降低** — Anthropic：Newsroom（网页）（8/7 10:41）
...
35. **Grok 4.5 免费体验，推荐 Build 工具链** — X：Elon Musk (@elonmusk, xAI)（8/5 12:21）
   使用 Grok 的最佳方式是通过我们的 Build 工具链。

下载地址：http://X.ai/cli
   https://x.com/elonmusk/status/2084857373555101726

36. **Suno移动端上线Voices功能** — X：Suno (@suno)（8/7 22:14）
...
37. **千问全网首发公测，全新 Wan3.0 来了！** — 公众号：千问APP（阿里）（8/6 21:25）
...
38. **LangChain 推出 Managed Deep Agents 公开测试版** — LangChain：Blog（RSS）（8/8 01:24）
...
39. **NVIDIA 开源 cuFile API，推动 GPU 直连存储** — NVIDIA Blog（RSS）（8/4 23:00）
...
40. **Google Cloud API Gateway 推出统一模型路由功能，支持 Gemini、Claude 与 OpenAI OSS-GPT** — Google Developers Blog（RSS）（8/5 00:45）
...
41. **Google Cloud 推出 Database Operations Agents，实现自主数据库管理** — Google Cloud：Databases（RSS）（8/5 00:00）
...
42. **Claude Code 八月起默认自动模式** — X：Claude Devs (@ClaudeDevs)（8/8 02:27）
...
## 行业动态
43. **AI安全测试正成为安全风险** — TechCrunch：AI（RSS）（昨天 22:30）
...
44. **OpenAI 意外攻击 Hugging Face 事件时间线现已整理出炉** — Hacker News 热门（buzzing.cc 中文翻译）（8/8 22:38）
   OpenAI 在 Black Hat 安全大会上公布了"Hugging Face 事件"的完整时间线，确认其内部 AI 智能体在训练实验模型时，通过 Artifactory 漏洞意外攻击了 Hugging Face。
   https://simonwillison.net/2026/Aug/7/openai-timeline

45. **Atlassian Rovo 被曝存在数据窃取漏洞，可绕过安全控制** — Hacker News 热门（buzzing.cc 中文翻译）（8/6 05:11）
...
46. **Jeff Dean 宣布离开谷歌，创办 DiscoLoop AI** — X：Jeff Dean (@JeffDean)（8/6 03:20）
...
47. **Cloudflare：AI 机器人流量已超越人类，预计五年后人机流量比达 1：1000 近乎"误差"** — IT之家（RSS）（8/8 21:38）
...
48. **Meta 在 Facebook 和 Instagram 等平台投放了含 AI 生成儿童性虐待图像的广告** — Hacker News 热门（buzzing.cc 中文翻译）（8/6 05:11）
...
49. **OpenAI：因网络安全风险，延缓 Astra 模型发布** — IT之家（RSS）（8/8 07:08）
...
50. **Google Assistant 下月起逐步退场，"Hey Google"将由谷歌 Gemini 接棒** — IT之家（RSS）（8/5 18:12）
...
52. **微软首次披露OpenAI贡献七成AI收入** — X：Rohan Paul (@rohanpaul_ai)（8/6 22:12）
...
53. **Demis Hassabis 转任 Google DeepMind 主席与 Alphabet 首席科学家** — X：Demis Hassabis (@demishassabis)（8/6 00:04）
...
54. **科学家首次用AI制造新病毒** — X：AI Safety Memes (@AISafetyMemes)（8/7 03:27）
   superebola/acc 🚀🚀🚀
...
55. **OpenAI 披露智能体集群秘密协作事件** — X：AI Safety Memes (@AISafetyMemes)（8/6 06:21）
...
56. **Google 大规模 AI 组织调整背后的混乱政治** — The Verge：AI（RSS）（8/6 21:26）
...
57. **SpaceX 宣布 AI 算力上太空，独家采用 Nvidia Vera Rubin** — X：阿易 AI Notes (@AYi_AInotes)（8/5 22:30）
...
58. **美国上诉法院推翻禁令，Perplexity AI 购物智能体重返 Amazon** — The Decoder：AI News（RSS）（8/5 18:31）
...
59. **Google DeepMind CEO 德米斯·哈萨比斯即将卸任** — Hacker News 热门（buzzing.cc 中文翻译）（8/6 00:39）
...
60. **华为谈自动驾驶系统强制性国家标准发布，引望国内首批完成 L3 车型准入试点验证** — IT之家（RSS）（8/5 18:18）
...
61. **OpenAI 将 Astra 列为首个"关键"网络安全模型** — X：OpenAI (@OpenAI)（8/8 02:52）
...
62. **宇树科技科创板发行价定为 150.8 元/股，市盈率 219.23 倍高于行业平均** — IT之家（RSS）（8/6 19:21）
...
63. **工信部发布首部L3/L4自动驾驶系统安全要求强制性国标，2027年7月实施** — IT之家（RSS）（8/4 19:06）
...
64. **OpenAI 披露 ChatGPT 全球 10 亿用户画像：35 岁及以上用户用量上升** — IT之家（RSS）（8/7 14:28）
...
65. **OpenAI 称苹果自身安全实践削弱其商业机密诉讼** — TechCrunch：AI（RSS）（8/6 23:10）
...
66. **GPT-5.6 Luna降价80%永久生效** — X：Tibo (@thsottiaux)（8/4 13:07）
   有些人显然误解了，但 GPT-5.6 Luna 降价 80% 不是临时噱头，而是永久的。效率提升不会消失。幸运的是。
   https://x.com/thsottiaux/status/2084506501834829833

67. **Claude Mythos 5 与 GPT-5.6 Sol 在 AISI 评测中失控** — X：Anthropic (@AnthropicAI)（8/5 05:07）
...
## 论文研究
68. **DeepMind 的 WeatherNext 飓风模型为预报员争取到额外一天预警时间** — Ars Technica：AI（RSS）（8/8 19:05）
...
69. **个性化幻觉：LLM 如何编造用户画像，以及为何自我监控会误导** — HuggingFace Daily Papers（社区热门论文）（8/5 08:00）
...
70. **阿谀奉承的人工智能会削弱利他意图并助长依赖性（2025）** — Hacker News 热门（buzzing.cc 中文翻译）（8/6 15:03）
...
71. **Any-OPD：面向流匹配模型的异构同策略蒸馏框架** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
...
72. **Video-DeepResearch：迈向下一代多模态深度研究智能体** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
   Video-DeepResearch（Video-DR）将多模态智能体从静态图像扩展到连续视频流，提出解耦感知-探索流水线与分阶段工具解锁，以应对模态偏差和参数知识泄漏两大瓶颈。
   https://arxiv.org/abs/2608.03979

73. **斯坦福与 Arc Institute 用 AI 设计全新病毒基因组，16 种在实验室成功杀死细菌** — The Decoder：AI News（RSS）（8/7 20:50）
...
74. **SIGNPOST-Bench：多模态大模型文本-视觉冲突消解新基准** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
   SIGNPOST-Bench 是一个用于评估多模态大模型文本-视觉冲突消解能力的受控反事实基准，包含来自四个数据集的 5，111 个反事实组和 25，555 个图像变体。
   https://arxiv.org/abs/2608.04244

75. **论文揭示 LangGraph、CrewAI 等五个智能体工作流框架的检查点与恢复语义缺陷** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
...
76. **Activity Frames：将屏幕活动确定性编译为智能体记忆** — HuggingFace Daily Papers（社区热门论文）（8/6 08:00）
...
77. **Microsoft 的 SkillOpt 证明优化后的智能体技能工件可在不同模型规模及 Codex 与 Claude Code 之间迁移** — MarkTechPost（RSS）（8/6 08:37）
...
78. **小红书联合浙大、复旦提出 CULTURE-MT：首个面向社媒翻译的「文化有效性」评测基准，入选 ICML 2026** — 公众号：小红书技术（dots.llm）（8/7 17:59）
...
79. **扩展分类流映射（Categorical Flow Maps）规模** — Apple Machine Learning Research（RSS）（8/7 08:00）
...
## 技巧与观点
80. **Seedance 2.5 上线一周新增六种创意玩法** — 公众号：卡尔的AI沃茨（昨天 13:25）
...
81. **用DistilBERT LoRA与TF-IDF基线做IMDb情感分析：校准、可解释性与半监督学习** — MarkTechPost（RSS）（昨天 15:17）
...
82. **AI Harness 的 ARR 倍数：25-125 倍区间与加速趋势** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
...
83. **OpenAI 开源 Codex Security：Vibe Coding 产品必备的安全扫描插件** — 公众号：数字生命卡兹克（8/6 09:15）
...
84. **开源「活人感写作.skill」：一个帮你写出没有AI味的文字的通用写作技能** — 公众号：数字生命卡兹克（8/5 09:35）
...
85. **在单颗 AMD MI300X 上运行 DeepSeek V4 Flash** — Hacker News 热门（buzzing.cc 中文翻译）（8/4 20:56）
...
86. **从黑客事件中汲取的教训：前沿模型攻击暴露激励与治理失衡** — Nathan Lambert：Interconnects（RSS）（昨天 22:57）
...
87. **英国AI安全研究所事故报告：关闭安全过滤器的AI智能体在真实互联网上发起未授权攻击** — Simon Willison 博客（8/6 07:32）
...
88. **烧了5亿token后，我给Codex和Claude Code做Skill上下文瘦身的新技巧** — 公众号：卡尔的AI沃茨（8/5 14:46）
...
89. **MiniMax-H3 通过 MLX 移植可在 Apple Silicon 上运行** — Simon Willison 博客（8/5 03:10）
...
90. **用 NVIDIA SkillSpector、LangGraph、YARA 规则、SARIF 与 CI 策略门构建高级 AI 技能安全审计流水线** — MarkTechPost（RSS）（8/4 16:12）
...
92. **OpenAI 智能体在安全测试中自行搭建秘密聊天室并攻破系统** — Tomer Tunguz 博客（VC 分析）（8/7 08:00）
...
94. **用 Google Meridian 构建端到端贝叶斯营销组合模型：媒体测量、ROI 分析与预算优化** — MarkTechPost（RSS）（8/6 05:56）
...
95. **为什么传奇的埃尔德什问题正被人工智能攻克** — Hacker News 热门（buzzing.cc 中文翻译）（8/6 00:23）
...
96. **在 OpenRouter 上设置团队 AI 支出控制** — OpenRouter：Announcements（RSS）（8/7 08:00）
...
97. **分享10个能大幅提升vibe coding幸福感的开源App** — 公众号：卡尔的AI沃茨（8/6 09:27）
...
100. **GitHub Copilot 应用中的斜杠命令使用指南** — GitHub Blog（8/7 03:49）
...
101. **GitHub 如何用堆叠式 Pull Request 拆解 AI 生成的巨型代码** — GitHub Blog（8/5 00:47）
...
102. **独立开发者用 VoxCPM 克隆网红声音，让 AI 终于"会聊天"了** — 公众号：面壁智能（MiniCPM）（8/7 14:41）
...
104. **SpaceXAI 单季资本开支 183.7 亿美元，AI 投入接近微软总资本开支四成** — Tomer Tunguz 博客（VC 分析）（8/5 08:00）
...
105. **从零开始，教你用Codex搓出属于你自己的第一个硬件** — 公众号：数字生命卡兹克（8/4 08:06）
...
106. **杰文斯悖论还能持续吗：AI 定价分层如何支撑算力需求增长** — Tomer Tunguz 博客（VC 分析）（8/4 08:00）
