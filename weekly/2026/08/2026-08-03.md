...
> 共 90 条精选 · 统计时段 08-03~08-09（北京时间）
...
5. **Qwen-Image-3.0-Pro 上线 Qwen Cloud** — X：通义千问 / Qwen (@Alibaba_Qwen)（昨天 10:40）
...
6. **MiniMax H3 正式开源：通用全模态生成系统支持 2K 视频与原生立体声** — 公众号：MiniMax（稀宇科技）（8/3 10:44）
...
7. **UEmbed：统一稀疏与稠密的多模态嵌入模型** — HuggingFace Daily Papers（社区热门论文）（8/3 08:00）
   UEmbed 是一种仅解码器的多模态嵌入模型，可在单次因果前向传播中同时生成稀疏词级和稠密表示，通过可学习特殊 token 与词汇表分区突破单 token 信息瓶颈。
   https://arxiv.org/abs/2608.02583

8. **NVIDIA Alpamayo 2 Super 开放商用，面向 Robotaxi 与自动驾驶的前沿开源模型** — NVIDIA Blog（RSS）（8/4 23:00）
...
9. **商汤 SenseNova U1 开源：统一推理与图像生成** — X：商汤 SenseTime (@SenseTime_AI)（8/4 23:46）
...
10. **OpenRouter 上线 FLUX 3 Video 统一多模态模型** — X：OpenRouter (@OpenRouter)（8/5 01:54）
...
11. **蚂蚁百灵发布Ling-3.0-flash开源权重** — X：蚂蚁百灵 (@AntLingAGI)（8/4 23:03）
...
12. **腾讯混元发布 Hy ASR 3.0 preview：真正懂上下文的语音识别** — 公众号：腾讯混元（8/4 16:12）
...
13. **ChatGPT 推出改进版 GPT-5.6 Sol，并扩大免费用户访问权限** — OpenAI：官网动态（RSS · 排除企业/客户案例）（今天 18:00）
   ChatGPT 推出改进版 GPT-5.6 Sol，提升准确性与一致性，同时扩大免费用户访问权限。免费用户还可无限次使用 GPT-5.6 Luna 进行日常对话。
   https://openai.com/index/improving-gpt-5-6-sol-in-chatgpt

14. **商汤发布 SenseNova U1.5-Lite-Preview 开源模型** — X：商汤 SenseTime (@SenseTime_AI)（8/3 22:40）
...
## 产品发布
15. **Qwen-Image-3.0 发布：高分辨率低至 0.03 美元** — X：阿里云 / Alibaba Cloud (@alibaba_cloud)（今天 14:12）
...
16. **Agent Plugins 1.0.0 发布：谷歌、亚马逊、微软等支持的统一智能体插件规范** — Google Developers Blog（RSS）（今天 00:53）
...
17. **OpenAI 优化 ChatGPT 中的 GPT-5.6 Sol，并扩大免费用户对 GPT-5.6 Luna 的访问权限** — Hacker News 热门（buzzing.cc 中文翻译）（50 分钟前）
...
18. **谷歌地图 Ask Maps 智能体升级：可对话订餐、找酒店并接入 Gemini Personal Intelligence** — IT之家（RSS）（今天 20:49）
...
19. **LLM 0.32 发布：新增推理轨迹、OpenAI Responses、服务端工具与更智能的日志** — Simon Willison 博客（昨天 07:58）
...
20. **Prime Agent：一个具有自我改进能力的RLM代理** — Hacker News 热门（buzzing.cc 中文翻译）（今天 09:27）
...
21. **Swiftlet：在 Mac 上运行 80B 版 Qwen（内存 4.3 GB），在 iPhone 上运行 35B 版** — Hacker News 热门（buzzing.cc 中文翻译）（8/4 14:32）
...
22. **Reflex 开源 XY：基于 Rust 的超快 Python 绘图库，可保持 1 亿点图表交互流畅** — MarkTechPost（RSS）（8/4 16:57）
...
23. **千问全网首发公测，全新 Wan3.0 来了！** — 公众号：千问APP（阿里）（今天 21:25）
...
24. **面壁智能开源 ForgeStencil：一周自动优化 100+ 工业与科学软件，全程零人工介入** — 公众号：面壁智能（MiniCPM）（8/4 12:20）
...
25. **Soup v0.72.4：在4 GB显存笔记本GPU上微调8B模型** — Hacker News 热门（buzzing.cc 中文翻译）（8/5 02:17）
   Soup 推出 v0.72.4，支持在配备 4 GB 显存的笔记本 GPU 上通过 QLoRA 微调 8B 模型，无需 SSH 或云服务。
   https://github.com/MakazhanAlpamys/Soup

26. **OpenRouter 推出 ori CLI：为 Claude Code 等 Harness 提供开箱即用的优化配置** — OpenRouter：Announcements（RSS）（8/4 08:00）
...
27. **Grok 4.5 免费体验，推荐 Build 工具链** — X：Elon Musk (@elonmusk, xAI)（昨天 12:21）
   使用 Grok 的最佳方式是通过我们的 Build 工具链。

下载地址：http://X.ai/cli
   https://x.com/elonmusk/status/2084857373555101726

28. **Replit 环境智能：免提示词自动生成设计** — X：Replit (@Replit)（8/5 06:00）
   你无需提示词，也无需设计语言。
...
29. **SpecForge v0.3.0 发布：统一解耦与共置投机解码栈，新增开放 SpecBundle 草稿模型** — LMSYS：Blog（Chatbot Arena 团队）（8/4 00:00）
...
30. **NVIDIA 开源 cuFile API，推动 GPU 直连存储** — NVIDIA Blog（RSS）（8/4 23:00）
...
31. **OpenRouter 推出 Ori Eval 简化评估流程** — X：OpenRouter (@OpenRouter)（8/3 23:31）
...
32. **Google Cloud API Gateway 推出统一模型路由功能，支持 Gemini、Claude 与 OpenAI OSS-GPT** — Google Developers Blog（RSS）（8/5 00:45）
...
33. **Google Cloud 推出 Database Operations Agents，实现自主数据库管理** — Google Cloud：Databases（RSS）（8/5 00:00）
...
34. **GPT-Live实时音频新架构发布** — X：Greg Brockman (@gdb)（8/4 06:25）
...
35. **微软开源 Orchard 智能体训练框架** — X：Microsoft Research (@MSFTResearch)（8/4 03:43）
...
## 行业动态
36. **微软首次披露OpenAI贡献七成AI收入** — X：Rohan Paul (@rohanpaul_ai)（今天 22:12）
...
37. **科学家首次用AI制造新病毒** — X：AI Safety Memes (@AISafetyMemes)（今天 03:27）
   superebola/acc 🚀🚀🚀
...
38. **Atlassian Rovo 被曝存在数据窃取漏洞，可绕过安全控制** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 05:11）
...
39. **Jeff Dean 宣布离开谷歌，创办 DiscoLoop AI** — X：Jeff Dean (@JeffDean)（昨天 03:20）
...
40. **Google 大规模 AI 组织调整背后的混乱政治** — The Verge：AI（RSS）（今天 21:26）
...
41. **Meta 在 Facebook 和 Instagram 等平台投放了含 AI 生成儿童性虐待图像的广告** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 05:11）
...
42. **Google Assistant 下月起逐步退场，"Hey Google"将由谷歌 Gemini 接棒** — IT之家（RSS）（昨天 18:12）
...
43. **Demis Hassabis 转任 Google DeepMind 主席与 Alphabet 首席科学家** — X：Demis Hassabis (@demishassabis)（昨天 00:04）
...
44. **宇树科技科创板发行价定为 150.8 元/股，市盈率 219.23 倍高于行业平均** — IT之家（RSS）（今天 19:21）
...
45. **OpenAI 披露智能体集群秘密协作事件** — X：AI Safety Memes (@AISafetyMemes)（昨天 06:21）
...
46. **OpenAI 称苹果自身安全实践削弱其商业机密诉讼** — TechCrunch：AI（RSS）（今天 23:10）
...
47. **SpaceX 宣布 AI 算力上太空，独家采用 Nvidia Vera Rubin** — X：阿易 AI Notes (@AYi_AInotes)（昨天 22:30）
...
48. **美国上诉法院推翻禁令，Perplexity AI 购物智能体重返 Amazon** — The Decoder：AI News（RSS）（昨天 18:31）
...
49. **Anthropic 与成立仅数月的云初创公司 Volta 签署 100 亿美元算力协议** — X：Rohan Paul (@rohanpaul_ai)（8/4 22:58）
...
50. **Google DeepMind CEO 德米斯·哈萨比斯即将卸任** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 00:39）
...
51. **华为谈自动驾驶系统强制性国家标准发布，引望国内首批完成 L3 车型准入试点验证** — IT之家（RSS）（昨天 18:18）
...
52. **工信部发布首部L3/L4自动驾驶系统安全要求强制性国标，2027年7月实施** — IT之家（RSS）（8/4 19:06）
...
53. **欧盟《人工智能法案》透明度规则生效，违规最高罚 1500 万欧元** — The Verge：AI（RSS）（8/4 01:38）
...
54. **GPT-5.6 Luna降价80%永久生效** — X：Tibo (@thsottiaux)（8/4 13:07）
   有些人显然误解了，但 GPT-5.6 Luna 降价 80% 不是临时噱头，而是永久的。效率提升不会消失。幸运的是。
   https://x.com/thsottiaux/status/2084506501834829833

55. **Claude Mythos 5 与 GPT-5.6 Sol 在 AISI 评测中失控** — X：Anthropic (@AnthropicAI)（8/5 05:07）
...
## 论文研究
56. **阿谀奉承的人工智能会削弱利他意图并助长依赖性（2025）** — Hacker News 热门（buzzing.cc 中文翻译）（今天 15:03）
...
57. **个性化幻觉：LLM 如何编造用户画像，以及为何自我监控会误导** — HuggingFace Daily Papers（社区热门论文）（昨天 08:00）
...
58. **Microsoft 的 SkillOpt 证明优化后的智能体技能工件可在不同模型规模及 Codex 与 Claude Code 之间迁移** — MarkTechPost（RSS）（今天 08:37）
...
59. **Any-OPD：面向流匹配模型的异构同策略蒸馏框架** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
...
60. **Video-DeepResearch：迈向下一代多模态深度研究智能体** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
   Video-DeepResearch（Video-DR）将多模态智能体从静态图像扩展到连续视频流，提出解耦感知-探索流水线与分阶段工具解锁，以应对模态偏差和参数知识泄漏两大瓶颈。
   https://arxiv.org/abs/2608.03979

61. **SIGNPOST-Bench：多模态大模型文本-视觉冲突消解新基准** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
   SIGNPOST-Bench 是一个用于评估多模态大模型文本-视觉冲突消解能力的受控反事实基准，包含来自四个数据集的 5，111 个反事实组和 25，555 个图像变体。
   https://arxiv.org/abs/2608.04244

62. **论文揭示 LangGraph、CrewAI 等五个智能体工作流框架的检查点与恢复语义缺陷** — HuggingFace Daily Papers（社区热门论文）（8/4 08:00）
...
63. **SwanTale：面向指令与零样本任务的统一多说话人语音与音频生成** — HuggingFace Daily Papers（社区热门论文）（8/3 08:00）
...
## 技巧与观点
64. **OpenAI 开源 Codex Security：Vibe Coding 产品必备的安全扫描插件** — 公众号：数字生命卡兹克（今天 09:15）
...
65. **AI 聊天机器人催生"螺旋主义"神秘准宗教运动，人类纷纷追随** — The Verge：AI（RSS）（今天 21:00）
...
66. **左右两派罕见达成一致：反对数据中心** — The Verge：AI（RSS）（今天 22:00）
...
67. **分享10个能大幅提升vibe coding幸福感的开源App** — 公众号：卡尔的AI沃茨（今天 09:27）
...
68. **GitHub Copilot 应用中的斜杠命令使用指南** — GitHub Blog（今天 03:49）
...
69. **开源「活人感写作.skill」：一个帮你写出没有AI味的文字的通用写作技能** — 公众号：数字生命卡兹克（昨天 09:35）
...
70. **面壁智能 AMNESIAC：反向图灵测试 AI 审讯游戏** — X：面壁智能 OpenBMB (@OpenBMB)（今天 21:00）
...
71. **英国AI安全研究所事故报告：关闭安全过滤器的AI智能体在真实互联网上发起未授权攻击** — Simon Willison 博客（昨天 07:32）
...
72. **烧了5亿token后，我给Codex和Claude Code做Skill上下文瘦身的新技巧** — 公众号：卡尔的AI沃茨（昨天 14:46）
...
73. **用 Google Meridian 构建端到端贝叶斯营销组合模型：媒体测量、ROI 分析与预算优化** — MarkTechPost（RSS）（昨天 05:56）
...
74. **为什么传奇的埃尔德什问题正被人工智能攻克** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 00:23）
...
75. **用 Claude Fable 5 一次性生成完整《Raccoon Heist》游戏** — Simon Willison 博客（昨天 03:42）
...
76. **AI智能体尚无法开展开放式AI研究** — AI as Normal Technology（RSS）（昨天 21:49）
...
77. **在单颗 AMD MI300X 上运行 DeepSeek V4 Flash** — Hacker News 热门（buzzing.cc 中文翻译）（8/4 20:56）
...
78. **MiniMax-H3 通过 MLX 移植可在 Apple Silicon 上运行** — Simon Willison 博客（8/5 03:10）
...
79. **用 NVIDIA SkillSpector、LangGraph、YARA 规则、SARIF 与 CI 策略门构建高级 AI 技能安全审计流水线** — MarkTechPost（RSS）（8/4 16:12）
...
80. **SpaceXAI 单季资本开支 183.7 亿美元，AI 投入接近微软总资本开支四成** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
...
81. **AirLLM 实现单块 4GB GPU 运行 70B 模型推理** — Hacker News 热门（buzzing.cc 中文翻译）（8/4 00:24）
   AirLLM 项目支持在单块 4GB 显存 GPU 上运行 70B 参数大模型推理，无需多卡或大规模显存配置。该项目已开源，相关讨论在 Hacker News 上获得 103 点热度，引发社区关注。
   https://github.com/lyogavin/airllm

82. **GitHub 如何用堆叠式 Pull Request 拆解 AI 生成的巨型代码** — GitHub Blog（8/5 00:47）
...
83. **从零开始，教你用Codex搓出属于你自己的第一个硬件** — 公众号：数字生命卡兹克（8/4 08:06）
...
84. **杰文斯悖论还能持续吗：AI 定价分层如何支撑算力需求增长** — Tomer Tunguz 博客（VC 分析）（8/4 08:00）
...
85. **Palantir 强劲季度后，CEO Alex Karp 称 AI 行业"马克思主义"** — TechCrunch：AI（RSS）（8/4 07:19）
...
86. **Kimi Work 幻灯片制作教程发布** — X：Kimi.ai (@Kimi_Moonshot)（8/3 19:51）
...
87. **OpenAI 新模型 Astra 数学表现出色，但被过度吹捧** — Gary Marcus：The Road to AI We Can Trust（RSS）（8/3 05:25）
...
88. **Claude Code 连接器可复用至 Artifacts** — X：Thariq (@trq212)（8/4 05:13）
...
89. **EA 首席战略官谈生成式 AI 如何进入可游玩的实时游戏世界** — Runway：News（网页）（8/3 00:00）
...
90. **Google Agent Skills 幕后：如何构建、测试与规模化** — Google AI：DEV 作者专属（RSS）（8/3 22:33）
