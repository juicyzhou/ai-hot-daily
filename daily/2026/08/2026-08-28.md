...
## 模型发布
1. **腾讯混元发布 Hy4 preview：770B 总参数、1M 上下文，开源上线** — 公众号：腾讯混元（8/28 14:03）
...
2. **GLM-5.3 开源权重，智能体编码与网防最强** — X：智谱 Z.ai (@Zai_org)（8/28 23:04）
   GLM-5.3 现已开放权重。
...
3. **Midjourney 开放 V8.2 图像编辑模型测试** — Midjourney：Updates（RSS）（8/28 07:32）
...
## 产品发布
4. **Open ASR 排行榜新增首个全球南方语言：印地语与印度英语评测集** — Hugging Face：Blog（RSS）（8/28 08:00）
...
## 行业动态
5. **联邦法官裁定特朗普政府将 Anthropic 列入黑名单违法** — Ars Technica：AI（RSS）（8/29 02:07）
...
6. **OpenAI 决定终止向 Cursor 提供模型，因 SpaceX 收购后合规风险** — OpenAI：官网动态（RSS · 排除企业/客户案例）（8/28 14:00）
...
## 论文研究
7. **Terminal-Bench-Science 0.1：评估科研工作流中的 AI 智能体** — Hacker News 热门（buzzing.cc 中文翻译）（8/28 20:04）
...
## 技巧与观点
8. **AI 工程师笔记本：在 Colab 上免费、无需框架即可使用 RAG/智能体/评估工具** — Hacker News 热门（buzzing.cc 中文翻译）（8/28 16:36）
...
9. **Gemini 3.5 Transcribe 完整指南：告别 ASR 转录难题** — Google AI：DEV 作者专属（RSS）（8/28 21:34）
...
10. **OpenAI 攻击 Hugging Face 事件的 5 个教训** — Gary Marcus：The Road to AI We Can Trust（RSS）（8/29 02:24）
