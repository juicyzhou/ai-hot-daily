...
> 共 10 条精选 · 统计时段 2026-08-12 07:30 ~ 2026-08-13 07:30（北京时间）

## 模型发布
1. **xAI 发布 Grok 4.6，强化长时运行智能体能力** — xAI：News（网页）（昨天 08:00）
...
2. **LTX-2.5 模型登场：AI 生成 10 秒 720P 视频仅需 6.8 秒，原生集成 ComfyUI** — IT之家（RSS）（昨天 14:46）
...
3. **Cursor 与 SpaceXAI 联合发布 Grok 4.6** — Cursor Blog（昨天 08:00）
...
4. **微软首发自研推理模型MAI-Thinking-1** — X：Mustafa Suleyman（Microsoft AI CEO） (@mustafasuleyman)（昨天 00:00）
...
## 产品发布
5. **Meta 开源 Muse Glimmer 登陆 OpenRouter** — X：OpenRouter (@OpenRouter)（昨天 20:00）
...
6. **OpenRouter 推出实时网页搜索基准测试：如何为智能体选择引擎、深度与模型** — OpenRouter：Announcements（RSS）（昨天 08:00）
...
## 行业动态
7. **Research Gold 号称"100%人类撰写、绝不使用AI"，实则全程由AI驱动** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 13:41）
...
## 技巧与观点
8. **零基础用户半天上手AI的12步实操流程** — 公众号：数字生命卡兹克（昨天 08:08）
...
9. **AutoGPT 如何用 AGENTS.md 和技能门控管理 AI 生成的拉取请求** — GitHub Blog（昨天 02:00）
...
10. **我写了一本 AI 教科书--AI 还要多久才能写得更好？** — Nathan Lambert：Interconnects（RSS）（昨天 21:01）
