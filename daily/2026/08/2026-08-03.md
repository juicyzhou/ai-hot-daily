...
> 共 18 条精选 · 统计时段 2026-08-03 07:30 ~ 2026-08-04 07:30（北京时间）
...
6. **Cloudflare 推出 Billable Usage API：为自助账户提供按产品与计费周期的程序化成本可见性** — Cloudflare Blog（8/3 21:00）
...
7. **OpenRouter 推出 Ori Eval 简化评估流程** — X：OpenRouter (@OpenRouter)（8/3 23:31）
...
8. **Cloudflare 推出 @cloudflare/computer 预览版：为智能体提供虚拟文件系统与多执行环境** — Cloudflare Blog（8/3 21:15）
...
9. **GPT-Live实时音频新架构发布** — X：Greg Brockman (@gdb)（8/4 06:25）
...
10. **微软开源 Orchard 智能体训练框架** — X：Microsoft Research (@MSFTResearch)（8/4 03:43）
...
11. **Cloudflare Workers 与 Containers 现已支持入站 TCP 连接和 gRPC** — Cloudflare Blog（8/3 21:00）
...
## 行业动态
12. **欧盟《人工智能法案》透明度规则生效，违规最高罚 1500 万欧元** — The Verge：AI（RSS）（8/4 01:38）
...
## 论文研究
13. **SwanTale：面向指令与零样本任务的统一多说话人语音与音频生成** — HuggingFace Daily Papers（社区热门论文）（8/3 08:00）
...
## 技巧与观点
14. **AirLLM 实现单块 4GB GPU 运行 70B 模型推理** — Hacker News 热门（buzzing.cc 中文翻译）（8/4 00:24）
   AirLLM 项目支持在单块 4GB 显存 GPU 上运行 70B 参数大模型推理，无需多卡或大规模显存配置。该项目已开源，相关讨论在 Hacker News 上获得 103 点热度，引发社区关注。
   https://github.com/lyogavin/airllm

15. **Palantir 强劲季度后，CEO Alex Karp 称 AI 行业"马克思主义"** — TechCrunch：AI（RSS）（8/4 07:19）
...
16. **Kimi Work 幻灯片制作教程发布** — X：Kimi.ai (@Kimi_Moonshot)（8/3 19:51）
...
17. **Claude Code 连接器可复用至 Artifacts** — X：Thariq (@trq212)（8/4 05:13）
...
18. **Google Agent Skills 幕后：如何构建、测试与规模化** — Google AI：DEV 作者专属（RSS）（8/3 22:33）
