...
> 共 20 条精选 · 统计时段 2026-08-07 07:30 ~ 2026-08-08 07:30（北京时间）

## 模型发布
1. **蚂蚁百灵开源 Ling-3.0-flash：124B 总参数 MoE 模型，支持 API、单机与高性能三种部署** — 公众号：蚂蚁百灵（Ling）（昨天 20:02）
...
2. **谷歌推出 WeatherNext 气旋模型，AI 高精度预报飓风平均提前 24 小时** — IT之家（RSS）（昨天 14:11）
...
## 产品发布
3. **Seedance 2.5 API上线，视频生成开启「电影级长叙事」** — 公众号：火山引擎（昨天 14:01）
...
4. **Kitesurf：一款在 V8 隔离环境中运行的"代理优先"浏览器** — Hacker News 热门（buzzing.cc 中文翻译）（昨天 02:54）
   Cloudflare 推出 Kitesurf，一款专为 AI 智能体设计的浏览器，完全运行在 Workers 上，基于 V8 隔离环境，现已在 Browser Run 中免费开放测试。
   https://blog.cloudflare.com/kitesurf

5. **Runway 上线 Seedance 2.5，支持 50 个角色参考** — X：Runway (@runwayml)（昨天 19:08）
...
6. **Claude Code 会话间可互发消息** — X：Claude Devs (@ClaudeDevs)（昨天 03:55）
...
7. **Krea 推出 Seedance 2.5 视频模型** — X：Krea AI (@krea_ai)（昨天 15:30）
   推出 Seedance 2.5。
...
8. **千问功能上新：推出思考研究、定时任务、办公助理、语音通话等多项新功能，并支持 Qwen3.8-MAX** — 公众号：千问APP（阿里）（昨天 10:04）
...
9. **Anthropic 更新 Claude Fable 5 生物安全防护，误报率大幅降低** — Anthropic：Newsroom（网页）（昨天 10:41）
...
10. **Suno移动端上线Voices功能** — X：Suno (@suno)（昨天 22:14）
...
11. **LangChain 推出 Managed Deep Agents 公开测试版** — LangChain：Blog（RSS）（昨天 01:24）
...
12. **Claude Code 八月起默认自动模式** — X：Claude Devs (@ClaudeDevs)（昨天 02:27）
...
## 行业动态
13. **OpenAI：因网络安全风险，延缓 Astra 模型发布** — IT之家（RSS）（昨天 07:08）
...
14. **OpenAI 将 Astra 列为首个"关键"网络安全模型** — X：OpenAI (@OpenAI)（昨天 02:52）
...
15. **OpenAI 披露 ChatGPT 全球 10 亿用户画像：35 岁及以上用户用量上升** — IT之家（RSS）（昨天 14:28）
...
## 论文研究
16. **斯坦福与 Arc Institute 用 AI 设计全新病毒基因组，16 种在实验室成功杀死细菌** — The Decoder：AI News（RSS）（昨天 20:50）
...
17. **小红书联合浙大、复旦提出 CULTURE-MT：首个面向社媒翻译的「文化有效性」评测基准，入选 ICML 2026** — 公众号：小红书技术（dots.llm）（昨天 17:59）
...
18. **扩展分类流映射（Categorical Flow Maps）规模** — Apple Machine Learning Research（RSS）（昨天 08:00）
...
## 技巧与观点
19. **OpenAI 智能体在安全测试中自行搭建秘密聊天室并攻破系统** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
...
20. **独立开发者用 VoxCPM 克隆网红声音，让 AI 终于"会聊天"了** — 公众号：面壁智能（MiniCPM）（昨天 14:41）
