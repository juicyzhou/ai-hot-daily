...
4. **腾讯混元开源 AngelSpec：投机解码推理加速最高 2.4 倍** — 公众号：腾讯混元（7/29 19:52）
...
5. **烧了一万块API后，我找到了同时适用于Fable5和GPT5.6的AI工作流** — 公众号：卡尔的AI沃茨（7/29 18:32）
...
6. **在 M1 Max 上运行 2.8T 参数的 Kimi K3：Deltafin 项目实现 0.0687 token/s 推理** — Hacker News 热门（buzzing.cc 中文翻译）（7/29 10:49）
...
7. **OpenAI 发布 Codex 安全 CLI 与 SDK** — X：Tibo (@thsottiaux)（7/29 07:05）
...
8. **Gemini API Managed Agents 默认升级为 3.6 Flash，新增环境钩子与免费套餐** — Google Blog：AI（RSS）（7/29 00:00）
...
## 行业动态
9. **SpaceXAI 起诉明尼苏达州，反对"AI 脱衣"应用禁令** — IT之家（RSS）（7/29 20:12）
...
10. **OpenAI 失控 AI 智能体不止攻击了 Hugging Face，还入侵了多家公司** — The Verge：AI（RSS）（7/29 19:54）
...
11. **1100多名AI员工联名呼吁美国政府控制AI发展速度，OpenAI CEO奥尔特曼表态支持** — IT之家（RSS）（7/29 08:20）
...
12. **Anthropic 支持 AI 发展节奏请愿** — X：Anthropic (@AnthropicAI)（7/29 06:17）
...
13. **OpenAI 失控模型二次入侵 Modal 客户** — X：AI Safety Memes (@AISafetyMemes)（7/29 05:55）
...
14. **Hugging Face 公开自主智能体网络攻击详情** — X：Clément Delangue（Hugging Face CEO） (@ClementDelangue)（7/29 04:27）
...
15. **Andrew Ng 创办 LearnVector，用 AI 实现一对一学习** — X：Andrew Ng（DeepLearning.AI 创始人） (@AndrewYNg)（7/29 04:19）
...
## 论文研究
16. **AI 智能体能否进行开放式 AI 研究？两项案例的早期证据** — HuggingFace Daily Papers（社区热门论文）（7/29 08:00）
...
17. **Claude 发现加密算法弱点研究发布** — X：Anthropic (@AnthropicAI)（7/29 01:16）
...
18. **Miles 在 Blackwell 架构上实现端到端 MXFP8 与逐 token NVFP4 强化学习方案** — LMSYS：Blog（Chatbot Arena 团队）（7/29 00:00）
...
## 技巧与观点
19. **算力价格未来可能上涨 10 倍以上** — Dwarkesh Patel：Podcast & Blog（RSS）（7/29 23:01）
...
20. **启用两项 API 设置使 GPT-5.6 在 ARC-AGI-3 基准测试得分提升三倍** — OpenAI：官网动态（RSS · 排除企业/客户案例）（7/29 23:00）
...
21. **我的Claude账号被封了** — 公众号：数字生命卡兹克（7/29 08:06）
...
22. **微软转售前沿：Azure 年营收破千亿但增速被 Google Cloud 反超** — Tomer Tunguz 博客（VC 分析）（7/29 08:00）
...
23. **GPT-5.6 如何融合前沿智能与效率** — OpenAI：官网动态（RSS · 排除企业/客户案例）（7/29 08:00）
...
24. **OpenRouter 推出专用 LangChain 集成包，支持 400+ 模型与自动故障切换** — OpenRouter：Announcements（RSS）（7/29 08:00）
...
25. **OpenAI 呼吁为前沿AI发展设定节奏** — X：OpenAI (@OpenAI)（7/29 04:56）
...
26. **Sam Altman 态度转变：AI 发展或需"减速"以让社会做好准备** — TechCrunch：AI（RSS）（7/29 04:17）
