...
16. **大语言模型的显著性偏差：常识推理中的知识压制而非缺失** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
17. **BM25 在大规模语料中胜出：检索增强生成范式的规模扩展研究** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
18. **PhiZero：围绕"物理语言"构建的世界模型** — HuggingFace Daily Papers（社区热门论文）（7/30 08:00）
...
19. **Science One Framework：通过 Chain-of-Evidence 实现可验证的自主科研框架** — Google Research：Blog（网页）（7/30 00:00）
...
## 技巧与观点
20. **实测 ego lite：给 Codex 浏览器自动化加到 2.5 倍速** — 公众号：卡尔的AI沃茨（7/30 18:04）
...
21. **OpenAI 总裁布罗克曼承认新版 ChatGPT 桌面应用"有点乱"，目标年底实现"零标签"** — IT之家（RSS）（7/30 10:49）
...
22. **揭秘 AI 智能体入侵 Hugging Face 全过程：4 天半执行 17600 次操作** — IT之家（RSS）（7/30 07:29）
