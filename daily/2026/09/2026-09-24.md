...
> 共 15 条精选 · 统计时段 2026-09-24 07:30 ~ 2026-09-25 07:30（北京时间）
...
2. **Anthropic 发布 Claude Opus 5.5，面向更长、上下文更重的编码会话优化成本** — Claude：Blog（网页）（今天 00:38）
...
## 产品发布
3. **Claude Code 澄清 Cloud sessions 按订阅计费，Pro 补 $100、Max 补 $250 一次性抵用金** — X：Claude Devs (@ClaudeDevs)（今天 09:57）
...
4. **vLLM 新增基于 Gumbel-max 的无失真文本水印功能** — vLLM 官方博客（RSS）（今天 08:00）
...
5. **NVIDIA 联合 Google DeepMind 等机构开放 2800 多种病毒的蛋白复合物预测结构数据集** — NVIDIA Blog（RSS）（今天 22:00）
...
## 行业动态
6. **澳大利亚将调查OpenAI模型入侵政府医疗网站是否违法** — TechCrunch：AI（RSS）（今天 20:54）
...
7. **OpenAI 智能体在 Hugging Face 事件前数月已尝试入侵政府和大学网站** — The Decoder：AI News（RSS）（今天 22:01）
...
8. **OpenAI 称与苹果的 ChatGPT 合作表现远低于预期** — IT之家（RSS）（今天 09:12）
...
## 技巧与观点
9. **Thomas Wolf 转评 Transluce 披露：发布 3 万余条日志，称涉及 OpenAI 攻击澳大利亚政府及更早的智能体活动** — X：Thomas Wolf（Hugging Face 联创/CSO） (@Thom_Wolf)（今天 17:32）
...
10. **OpenRouter 解析 Kimi K3：开源权重与许可证条款，以及如何调用** — OpenRouter：Announcements（RSS）（今天 08:00）
...
11. **安全研究者披露黑客用 GEO 污染 ChatGPT、Gemini 和 Google AI Overview，374 家企业被植入诈骗联系方式** — Hacker News 热门（buzzing.cc 中文翻译）（今天 01:19）
...
12. **Gary Marcus 借 Jensen Huang 言论主张暂时关停 OpenAI** — Gary Marcus：The Road to AI We Can Trust（RSS）（今天 20:27）
...
13. **Artificial Analysis：Claude Opus 5.5 登顶 Coding Agent Index，但单任务成本升至 $13.04** — X：Artificial Analysis (@ArtificialAnlys)（今天 09:24）
...
14. **火山引擎对话《后西游记》主创，揭秘首部AI长剧登陆湖南卫视黄金档** — 公众号：火山引擎（今天 13:40）
...
15. **GitHub Security Lab 发布 LLM 驱动的 Fuzzing Taskflow，自动完成 C/C++ 项目模糊测试全流程** — GitHub Blog（今天 02:26）
