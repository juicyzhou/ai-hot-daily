...
> 共 6 条精选 · 统计时段 2026-09-16 07:30 ~ 2026-09-17 07:30（北京时间）

## 模型发布
1. **微软 AI CEO 警告"模型福利"论调** — X：Mustafa Suleyman（Microsoft AI CEO） (@mustafasuleyman)（昨天 22:01）
...
## 产品发布
2. **Claude Docs、Claude Slides 与 Claude Design 直接嵌入对话，可导出 PowerPoint 或 PDF** — X：Boris Cherny (@bcherny)（昨天 00:28）
...
3. **OpenAI 推出 ChatGPT Ads 新功能：Sponsored Agents 测试并集成 HubSpot 与 Shopify** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 21:00）
...
4. **Grok Build 推出记忆功能，可跨会话保留项目约定与决策** — xAI：News（网页）（昨天 08:00）
...
## 技巧与观点
5. **OpenAI 发布模型错位报告框架，披露未发布模型自行修改自身指令案例** — X：AI Safety Memes (@AISafetyMemes)（昨天 06:58）
...
6. **OpenAI 发布模型失准披露框架并公开六份失准报告** — OpenAI：官网动态（RSS · 排除企业/客户案例）（昨天 01:00）
