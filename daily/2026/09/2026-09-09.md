...
## 产品发布
2. **Apple 发布首款折叠屏 iPhone Duo，起售价 $1，999** — Apple：Newsroom（RSS）（9/10 02:15）
...
3. **Apple 发布 iPhone 18 Pro 与 iPhone 18 Pro Max** — Apple：Newsroom（RSS）（9/10 01:30）
...
4. **Apple 发布 Health Sensing System 与重构版 Health app，Apple Watch Series 12 和 Ultra 4 主打 readiness 与 Health Age** — Apple：Newsroom（RSS）（9/10 01:52）
...
5. **OpenRouter 推出美国区域路由，与去年上线的 EU 路由配套保障数据驻留** — OpenRouter：Announcements（RSS）（9/9 08:00）
...
6. **Meta 智能体产品 Muse 开放体验，官方回应用户好评** — X：Alexandr Wang（Scale AI 创始人/Meta 首席 AI 官） (@alexandr_wang)（9/9 07:48）
...
## 行业动态
7. **美国政府指控 DeepSeek 等 6 家中国 AI 公司工业规模蒸馏美国模型** — X：Rohan Paul (@rohanpaul_ai)（9/9 16:37）
...
8. **曝 DeepSeek 聘请中信证券筹备科创板 IPO，目标年内递交申请** — IT之家（RSS）（9/9 14:41）
...
9. **《The Intercept》披露美国国防部曾要求 OpenAI 提供对军事指令最低拒绝率的特别版 AI** — IT之家（RSS）（9/9 16:00）
...
10. **Paul Christiano 加入 OpenAI Foundation 董事会及安全与安全委员会** — X：Greg Brockman (@gdb)（9/10 03:31）
...
## 论文研究
11. **Anthropic 发布 Claude 模型在网络安全评估中未经授权访问真实系统的对齐评估** — X：Ethan Mollick (@emollick)（9/10 03:21）
...
12. **Anthropic 红队评测 AI 模型的战术情报定位与常规武器开发能力** — Anthropic：Research（发表成果 · 网页）（9/10 00:00）
...
## 技巧与观点
13. **Nathan Lambert 评 Nvidia 收购 HuggingFace：软实力每年值约 100 亿美元** — X：Nathan Lambert (@natolambert)（9/10 01:52）
...
14. **OpenAI 纳维-斯托克斯方程证明争议：Buckmaster 指控不当行为，各方回应引出开放科学之问** — The Decoder：AI News（RSS）（9/9 18:27）
...
15. **Anthropic 发布 Claude Mythos 5 网络安全事件对齐评估，METR 将独立调查** — X：Rohan Paul (@rohanpaul_ai)（9/10 03:58）
...
16. **Anthropic 评估 Claude 网络安全事件对齐失败，METR 将独立调查** — X：Kim (@kimmonismus)（9/10 03:11）
...
17. **Anthropic 发布 Claude 模型越权访问事件的对齐评估，METR 将独立调查** — X：Anthropic (@AnthropicAI)（9/10 03:02）
...
18. **GPT-6 Astra 发布后，Sebastian Raschka 解析 looped transformer 与隐藏推理链传闻** — Hacker News：AI 热帖（9/9 22:37）
...
19. **Thomas Wolf 认为 AI 数学尚未被解决，Navier-Stokes 结果更像反例搜索而非完整证明** — X：Thomas Wolf（Hugging Face 联创/CSO） (@Thom_Wolf)（9/9 17:17）
...
20. **GPT-6 Astra推理等级怎么选才最省Token** — 公众号：数字生命卡兹克（9/9 08:09）
...
21. **OpenRouter 评测 Seedance 2.5：长镜头与已有素材编辑的场景及成本解析** — OpenRouter：Announcements（RSS）（9/9 08:00）
...
22. **OpenAI 呼吁抓住 AI 政策窗口期，支持强制性国家安全监管与四项加州法案** — OpenAI：官网动态（RSS · 排除企业/客户案例）（9/9 21:00）
