...
> 共 12 条精选 · 统计时段 2026-09-15 07:30 ~ 2026-09-16 07:30（北京时间）

## 模型发布
1. **Google DeepMind 发布 Gemini 3.8 Live 和 3.8 Live Extended Thinking** — Google DeepMind：Blog（RSS）（昨天 01:05）
...
2. **生数科技发布 Vidu S2：含 Avatar 与 Editing 双模型，探索空间视频** — 公众号：生数科技（Vidu·视频）（昨天 22:30）
...
3. **阶跃星辰发布 StepAudio 3 系列语音大模型，多款在 Artificial Analysis 榜单全球第一** — 公众号：阶跃星辰（Step）（昨天 15:29）
...
## 产品发布
4. **Google 发布 TranslateGemma 等多语言 AI 成果，语言技术覆盖 300 多种语言** — Google Blog：AI（RSS）（昨天 00:00）
...
## 行业动态
5. **Perplexity 自研 CobbleDB 替代 AWS DynamoDB，每年最多可节省一亿美元** — X：Aravind Srinivas（Perplexity CEO） (@AravSrinivas)（昨天 04:23）
...
## 技巧与观点
6. **Gergely Orosz 探访 OpenAI：Codex 驱动的智能体软件工厂** — Pragmatic Engineer（RSS）（昨天 23:41）
...
7. **Anthropic 与 OpenAI 提议协调放缓前沿 AI 开发，Cohere CEO 等批评者质疑其真实动机** — The Decoder：AI News（RSS）（昨天 17:04）
...
8. **404 Media 曝光 OpenAI 莉莉计划：人工审核 ChatGPT 聊天记录以优化模型** — IT之家（RSS）（昨天 19:48）
...
9. **Arena 更新 Image-to-WebDev 榜单：GPT-6 Astra 以 1733 分登顶** — X：Arena (@arena)（昨天 05:20）
...
10. **Artificial Analysis 评测：GPT-Live-1 以 81.5 分登顶 Speech to Speech Index** — X：Artificial Analysis (@ArtificialAnlys)（昨天 11:14）
...
11. **Trail of Bits 批评 1Password 的 AI 补丁基准存在误导，并发布两个补丁验证 Agent 技能** — Trail of Bits：AI安全研究（昨天 19:00）
...
12. **Vercel 将 inbound 销售团队从 10 人压缩至 1.25 人，AI 销售开发智能体年成本仅数千美元** — Tomer Tunguz 博客（VC 分析）（昨天 08:00）
